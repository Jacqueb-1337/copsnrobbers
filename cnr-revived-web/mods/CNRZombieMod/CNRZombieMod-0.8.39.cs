...
namespace CNRZombieMod
{
    ...
    public class ZombieHook : MonoBehaviour
    {
        ...
        private const bool  VERBOSE_TARGET_LOGS = true;
        ...
        private void KillZombie(byte id, GameObject go, string attackerId)
        {
            ...
            {
                ZombieModEntry.Log("KillZombie: syncing attacker=" + attackerId + " id=" + id);
                var ht = new System.Collections.Hashtable();
                ht["kc"] = attackerId;
                ht["zk"] = (int)id;
                RaiseZombieEvent(ht, true);
        ...
        private void Broadcast()
        {
            ...
            foreach (var kv in _drivers)
            {
                ...
                float hp;
                if (!_zombieHealth.TryGetValue(kv.Key, out hp)) hp = 100f;
                int h = idx * 2;
                hpData[h] = kv.Key;
                hpData[h + 1] = hp;
                idx++;
        ...
        public void OnZombieEvent(EventData ev)
        {
            try
            {
                ...
                if (authority)
                {
                    ...
                    {
                        ZombieModEntry.Log("OnZombieEvent: authority zh=" + DumpValue(ht["zh"]));
                        ApplyZombieDamageRequest(ht["zh"]);
                    }
                    if (ht.ContainsKey("pd"))
                    {
                        ZombieModEntry.Log("OnZombieEvent: authority pd=" + DumpValue(ht["pd"]));
                        ApplyPlayerDownRequest(ht["pd"]);
                    }
                    if (ht.ContainsKey("kc"))
                    {
                        ZombieModEntry.Log("OnZombieEvent: authority kc=" + DumpValue(ht["kc"]));
                        ApplyKillCredit(ht["kc"]);
                    }
                    if (ht.ContainsKey("zk"))
                    {
                        ZombieModEntry.Log("OnZombieEvent: authority zk=" + DumpValue(ht["zk"]));
                        ApplyZombieKillSync(ht["zk"]);
                    }
                    if (ht.ContainsKey("zhp"))
                    {
                        ZombieModEntry.Log("OnZombieEvent: authority zhp=" + DumpValue(ht["zhp"]));
                        ApplyZombieHealthSync(NormalizeFloatArray(ht["zhp"]));
                    }
                    if (ht.ContainsKey("gd"))
                    {
                        ZombieModEntry.Log("OnZombieEvent: authority gd=" + DumpValue(ht["gd"]));
                ...
                {
                    ZombieModEntry.Log("OnZombieEvent: client zs=" + DumpValue(ht["zs"]));
                    ApplySyncedModeState(NormalizeFloatArray(ht["zs"]));
                }
                if (ht.ContainsKey("zk"))
                {
                    ZombieModEntry.Log("OnZombieEvent: client zk=" + DumpValue(ht["zk"]));
                    ApplyZombieKillSync(ht["zk"]);
                }
                if (ht.ContainsKey("zhp"))
                {
        ...
        public void ReportZombieDamage(byte zombieId, int damage)
        {
            ...
            ZombieModEntry.Log("ReportZombieDamage: id=" + zombieId + " dmg=" + damage +
        ...
        internal bool IsPeerDowned(string peerId)
        {
            ...
            if (!string.IsNullOrEmpty(localId) && string.Equals(localId, peerId, StringComparison.Ordinal))
            {
                bool actuallyDowned = IsLocalPlayerActuallyDowned();
                if (_localPlayerDowned && !actuallyDowned)
                {
                    ZombieModEntry.Log("IsPeerDowned: clearing stale local downed flag peer=" + peerId);
                    _localPlayerDowned = false;
                }
                return actuallyDowned;
            }
    ...
    public class ZombieDriver : MonoBehaviour
    {
        ...
        private static float      _nextRemoteTargetDiagTime;
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (logicType != null)
                    {
                        ...
                        ZombieModEntry.Log("ZombieDriver[" + ZombieId + "]: logic refs logic=" + (_singleEnemyLogic != null) +
                            " blood=" + (_fLogicBlood != null) + " bDied=" + (_fLogicBDied != null));
                    }
                }
        ...
        internal void ApplySharedHealth(float hp)
        {
            try
            {
                if (_singleEnemyLogic == null) return;
                if (_fLogicBlood != null) _fLogicBlood.SetValue(_singleEnemyLogic, Mathf.Max(0, Mathf.RoundToInt(hp)));
        ...
        internal PlayerTarget GetNearestPlayerTarget()
        {
            try
            {
                ...
                if (ec != null && (Hook == null || !Hook.IsPeerDowned(localPeer)))
                {
                    ...
                    bestSq = sq;
                    ZombieModEntry.Log("GetNearestPlayerTarget: candidate local target=" + ec.name +
                        " peer=" + (localPeer ?? "null") + " distSq=" + sq.ToString("F2"));
                }
                ...
                {
                    ZombieModEntry.Log("GetNearestPlayerTarget: no live targets at " + pos);
                    return null;
                }
                ZombieModEntry.Log("GetNearestPlayerTarget: selected=" + best.Transform.name +
                    " peer=" + (best.PeerId ?? "null") +
                    " zombiePos=" + pos +
                    " playerPos=" + best.Transform.position +
                    " distSq=" + best.DistanceSq.ToString("F2"));
                return best;
            }
        ...
        private void AddRemotePlayerTargets(Vector3 zombiePos, ref PlayerTarget best, ref float bestSq)
        {
            try
            {
                ...
                bool logDiag = ShouldLogRemoteTargetDiag();
                FieldInfo objField = mgrType.GetField("otherPlayerObject", instFlags);
                FieldInfo infoField = mgrType.GetField("otherPlayersInfoList", instFlags);
                ...
                {
                    if (logDiag)
                        ZombieModEntry.Log("AddRemotePlayerTargets: missing arrays objField=" + (objField != null) +
                            " infoField=" + (infoField != null) +
                            " objects=" + (objects != null) + " infos=" + (infos != null));
                    return;
                }

                if (logDiag)
                    ZombieModEntry.Log("AddRemotePlayerTargets: objects=" + objects.Length + " infos=" + infos.Length);
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    {
                        if (logDiag)
                            ZombieModEntry.Log("AddRemotePlayerTargets: skip idx=" + i +
                                " go=" + (go != null) + " info=" + (info != null));
                        continue;
                    }

                    Type infoType = info.GetType();
                    FieldInfo idField = infoType.GetField("mId", instFlags);
                    string peerId = idField != null ? Convert.ToString(idField.GetValue(info)) : null;
                    if (string.IsNullOrEmpty(peerId) || peerId == "null")
                    {
                        if (logDiag)
                            ZombieModEntry.Log("AddRemotePlayerTargets: skip idx=" + i +
                                " missing peer idField=" + (idField != null) + " obj=" + go.name);
                        continue;
                    }
                    if (Hook.IsPeerDowned(peerId))
                    {
                        if (logDiag)
                            ZombieModEntry.Log("AddRemotePlayerTargets: skip idx=" + i +
                                " peer downed=" + peerId + " obj=" + go.name);
                        continue;
                    }

                    float sq = (go.transform.position - zombiePos).sqrMagnitude;
                    ZombieModEntry.Log("GetNearestPlayerTarget: candidate remote idx=" + i +
                        " obj=" + go.name + " peer=" + peerId + " distSq=" + sq.ToString("F2"));
                    if (sq < bestSq)
                    {
                        bestSq = sq;
        ...
        private static bool ShouldLogRemoteTargetDiag()
        {
            if (Time.time < _nextRemoteTargetDiagTime) return false;
            _nextRemoteTargetDiagTime = Time.time + 1f;
            return true;
        }
    ...
    public class ZombieProxy : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            try
            {
                ...
                if (logicType != null)
                {
                    _singleEnemyLogic = GetComponent(logicType) ?? GetComponentInChildren(logicType);
                    _fLogicBlood = logicType.GetField("blood", BindingFlags.Public | BindingFlags.Instance) ??
                                   logicType.GetField("Blood", BindingFlags.Public | BindingFlags.Instance);
                    _fLogicBDied = logicType.GetField("bDied", BindingFlags.Public | BindingFlags.Instance) ??
                                  logicType.GetField("bDead", BindingFlags.Public | BindingFlags.Instance);
                    ZombieModEntry.Log("ZombieProxy[" + ZombieId + "]: logic=" + (_singleEnemyLogic != null) +
                        " bloodField=" + (_fLogicBlood != null) + " diedField=" + (_fLogicBDied != null));
                }
            }
        ...
        private void ApplyBloodBar(float hp)
        {
            ...
            float num = Mathf.Clamp01(hp / 100f);
