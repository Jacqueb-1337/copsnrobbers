...
namespace CNRZombieMod
{
    ...
    public class ZombieHook : MonoBehaviour
    {
        ...
        private void AttachSpectatorCamera()
        {
            try
            {
                ...
                Transform target = FindAlivePlayerTransform();
                if (target == null) return;

                _spectateTarget = target;
                _spectateCamParent = cam.transform.parent;
                _spectateCamLocalPos = cam.transform.localPosition;
                _spectateCamLocalRot = cam.transform.localRotation;
                cam.transform.SetParent(target, false);
                cam.transform.localPosition = new Vector3(0f, 1.6f, -3.5f);
                cam.transform.localRotation = Quaternion.identity;
                ZombieModEntry.Log("ZombieMod: spectating " + target.name);
            }
            catch (Exception ex) { ZombieModEntry.Log("AttachSpectatorCamera err: " + ex.Message); }
        }
        ...
        private void RestoreSpectatorCamera()
        {
            try
            {
                Camera cam = Camera.main;
                if (cam == null) return;
                if (_spectateTarget == null && _spectateCamParent == null) return;
                cam.transform.SetParent(_spectateCamParent, false);
                cam.transform.localPosition = _spectateCamLocalPos;
                cam.transform.localRotation = _spectateCamLocalRot;
                _spectateTarget = null;
                _spectateCamParent = null;
                ZombieModEntry.Log("ZombieMod: restored camera");
            }
            catch (Exception ex) { ZombieModEntry.Log("RestoreSpectatorCamera err: " + ex.Message); }
        }
        ...
        private Transform FindAlivePlayerTransform()
        {
            ...
            {
                GameObject ec = GameObject.Find("ExampleCharacter");
                Transform fallback = ec != null ? ec.transform : null;
            ...
            catch (Exception ex) { ZombieModEntry.Log("FindAlivePlayerTransform err: " + ex.Message); return null; }
        }
        ...
        internal PlayerTarget GetNearestPlayerTarget()
        {
            try
            {
                ...
                if (ec != null)
                {
                    ...
                    best = new PlayerTarget(ec.transform, Hook != null ? Hook.GetLocalPeerId() : null, sq);
                    bestSq = sq;
                }

                Type infoType = FindType("CRPlayerInfo");
                if (infoType != null)
                {
                    UnityEngine.Object[] infos = FindObjectsOfType(infoType);
                    for (int i = 0; i < infos.Length; i++)
                    {
                        Component c = infos[i] as Component;
                        if (c == null || c.transform == null) continue;
                        FieldInfo fiId = infoType.GetField("mId", BindingFlags.Public | BindingFlags.Instance);
                        FieldInfo fiState = infoType.GetField("mStatus", BindingFlags.Public | BindingFlags.Instance);
                        string peerId = fiId != null ? Convert.ToString(fiId.GetValue(c)) : null;
                        if (string.IsNullOrEmpty(peerId) || peerId == "null") continue;
                        bool alive = true;
                        if (fiState != null)
                        {
                            object st = fiState.GetValue(c);
                            alive = st == null || !st.ToString().ToLowerInvariant().Contains("dead");
                        }
                        if (!alive) continue;
                        ...
                        {
                            bestSq = sq;
                            best = new PlayerTarget(c.transform, peerId, sq);
                        }
                    }
                }
