    ...
    public class ZombieDriver : MonoBehaviour
    {
        ...
        internal PlayerTarget GetNearestPlayerTarget()
        {
            try
            {
                ...
                if (ec == null)
                {
                    ZombieModEntry.Log("GetNearestPlayerTarget: ExampleCharacter missing at " + pos);
                    return null;
                }
                float sq = (ec.transform.position - pos).sqrMagnitude;
                ZombieModEntry.Log("GetNearestPlayerTarget: target=" + ec.name +
                    " peer=" + (Hook != null ? Hook.GetLocalPeerId() ?? "null" : "null") +
                    " zombiePos=" + pos +
                    " playerPos=" + ec.transform.position +
                    " distSq=" + sq.ToString("F2"));
