...
namespace CNRZombieMod
{
    ...
    public class ZombieHook : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (_diagTimer <= 0f)
            {
                ...
                ZombieModEntry.Log(string.Format(
                    "Diag: scene={0} gameScene={1} inRoom={2} isMaster={3} hasTemplate={4} player={5} spawned={6} {7}",
                    scene, IsGameScene(scene), inRoom, isMaster, hasTemplate,
                    ec != null ? ec.name : "null", _masterSpawned, GetRoomDebug()));
        ...
        public void OnZombieEvent(EventData ev)
        {
            try
            {
                bool master = IsMasterClient();
                ZombieModEntry.Log("OnZombieEvent: code=" + ev.Code + " master=" + master + " " + GetRoomDebug());
                if (master)
        ...
        {
            string serverId = GetAppServerId();
            string localId = GetLocalPeerId();
            if (!string.IsNullOrEmpty(serverId) && !string.IsNullOrEmpty(localId))
                return string.Equals(serverId, localId, StringComparison.Ordinal);
