...
namespace CNRMods
{
    ...
    public class CNRArenaBotManager : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            // Human PlayerInfo stores the actual CharacterBody world rotation. Vanilla
            // NetPlayerController reconstructs its root from that value during Update,
            // but the virtual bots have no local CharacterBody driving the source value.
            // Use the visuals cached by BindDamageReceivers so this correction costs no
            // per-frame FindObjectsOfType scan on the old Unity runtime.
            for (int i = 0; i < _bots.Count; i++)
            {
                CNRArenaBotState bot = _bots[i];
                if (bot == null || bot.Status == PlayerStatus.dead) continue;
                NetPlayerController npc;
                if (!_botVisuals.TryGetValue(bot.Id, out npc) || npc == null || npc.bodyTransform == null) continue;
                try { npc.bodyTransform.rotation = bot.BodyRotation; }
                catch { }
            }
        }
