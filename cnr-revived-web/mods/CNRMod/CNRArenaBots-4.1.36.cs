...
namespace CNRMods
{
    ...
    public class CNRArenaBotManager : MonoBehaviour
    {
        ...
        private void ReconcileRoster(CNRMatchSettingsData rules)
        {
            ...
            if (rules.Mode == "tdm")
            {
                desiredCop = rules.TdmCopBots < 0 ? Mathf.Max(0, 5 - realCop) : rules.TdmCopBots;
                desiredRobber = rules.TdmRobberBots < 0 ? Mathf.Max(0, 5 - realRobber) : rules.TdmRobberBots;
            }
            else if (rules.Mode == "kc")
            {
                int count = Mathf.Clamp(rules.KcBotCount, 1, MAX_BOTS);
            ...
            // The stock scoreboard has seven entries per team. Explicit bot counts
            // therefore shrink as needed when real players already occupy those rows.
            desiredCop = Mathf.Clamp(desiredCop, 0, Mathf.Max(0, 7 - realCop));
            desiredRobber = Mathf.Clamp(desiredRobber, 0, Mathf.Max(0, 7 - realRobber));
