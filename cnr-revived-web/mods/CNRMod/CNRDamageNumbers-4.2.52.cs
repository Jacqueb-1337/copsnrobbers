...
namespace CNRMods
{
    ...
    public static class CNRDamageNumbers
    {
        ...
        public static bool IsNetworkPlayerTarget(Transform target)
        {
            Transform current = target;
            for (int i = 0; current != null && i < 10; i++, current = current.parent)
            {
                if (current.GetComponent<HitRegRelay>() != null) return true;
            }
            return false;
        }
    }
            ...
            if (distance > 0.0001f)
            {
                ...
                if (Physics.Raycast(_lastPosition, delta / distance, out hit, distance + 0.08f, 19))
                {
                    ...
                    if (IsDamageTag(tag))
                    {
                        ...
                        if (!CNRDamageNumbers.IsNetworkPlayerTarget(hit.transform))
