...
namespace CNRMods
{
    ...
    internal class CNRMinecraftLadderController : MonoBehaviour
    {
        ...
        private const float MaxClimbDeltaTime = 0.05f;
        ...
        void LateUpdate()
        {
            ...
            if (liveContact) _contactGraceUntil = Time.time + 0.20f;
            bool onClimbable = liveContact || (_wasOnClimbable && Time.time <= _contactGraceUntil);
            bool hudJumpHeld = (onClimbable || _jumpTouchIds.Count > 0 || _mouseJumpHeld) && DetectHudJumpTouch();
            if (!onClimbable)
            {
                if (_wasOnClimbable)
                {
            ...
            float dt = Mathf.Min(Mathf.Max(Time.deltaTime, 0.0001f), MaxClimbDeltaTime);
            ...
            bool jumpPressed = DetectJumpClimbIntent(hudJumpHeld);
            ...
            bool likelyRealTeleport = rawDelta.y > teleportGuard ||
                Mathf.Abs(rawDelta.x) > teleportGuard || Mathf.Abs(rawDelta.z) > teleportGuard;
            if (!likelyRealTeleport && _controller != null && _controller.enabled)
            {
                ...
                // The climbable controller owns Y completely. Capping this correction let
                // vanilla gravity win whenever it had already moved the player downward by
                // more than the cap, producing the tiny-hop / slide-back ladder behavior.
                float correctionY = desiredDelta.y - rawDelta.y;
        ...
        private bool DetectJumpClimbIntent(bool hudJumpHeld)
        {
            bool held = hudJumpHeld || (_fps != null && _fps.inputJump);
        ...
        private bool DetectHudJumpTouch()
        {
            try
            {
                ...
                if (Input.GetMouseButtonUp(0)) _mouseJumpHeld = false;
                if (_mouseJumpHeld && Input.GetMouseButton(0)) held = true;
                if (Input.GetMouseButtonDown(0) && IsScreenPointOnJump(Input.mousePosition))
                {
                    _mouseJumpHeld = true;
                    held = true;
                }
                ...
                for (int i = 0; i < Input.touchCount; i++)
                {
                    ...
                    if (!IsScreenPointOnJump(touch.position)) continue;
        ...
        private bool IsScreenPointOnJump(Vector3 screenPosition)
        {
            if (_jumpButton == null || _jumpUiCamera == null) return false;
            Ray ray = _jumpUiCamera.ScreenPointToRay(screenPosition);
            RaycastHit hit;
            if (!Physics.Raycast(ray, out hit, 100f) || hit.collider == null) return false;

            Transform t = hit.collider.transform;
            Transform jump = _jumpButton.transform;
            while (t != null)
            {
                if (t == jump) return true;
                t = t.parent;
            }
            return false;
        }
    ...
    internal class CNRMinecraftWaterController : MonoBehaviour
    {
        ...
        private const float SurfaceControlSubmersion = 0.55f;
        private const float SurfaceTargetSubmersion = 0.28f;
        private const float SurfaceBobAmplitude = 0.035f;
        private const float SurfaceBobFrequency = 4.0f;
        private const float SurfaceSpring = 3.0f;
        private const float SurfaceMaxRiseSpeed = 0.75f;
        private const float SurfaceMaxSinkSpeed = 0.45f;
        private const float SurfaceResponse = 6.0f;
        private const float MinimumSwimSubmersion = 0.12f;
        private const float MaxWaterDeltaTime = 0.05f;
        ...
        {
            CacheComponents();
            bool hudJumpHeld = DetectHudJumpTouch();
            Vector3 now = transform.position;
            if (!_hasLastPosition)
            {
                _lastPosition = now;
            ...
            // Swimming is deliberately based on direct hitbox/water overlap. Ignore only
            // the lowest 10% of the controller (the feet); touching even a sliver of water
            // anywhere in the upper 90% immediately hands vertical physics to this controller.
            ...
            {
                // Climbables take absolute priority. Do not re-enable gravity or rewrite
                // vertical velocity while the ladder/vine controller owns movement.
                LeaveWater(now, false);
                return;
            }
            if (!touchingWater)
            {
            ...
            float scale = blockScale > 0f ? blockScale : 1f;
            float rawDt = Mathf.Max(Time.deltaTime, 0.0001f);
            float dt = Mathf.Min(rawDt, MaxWaterDeltaTime);
            ...
            bool jumpHeld = DetectJumpIntent(hudJumpHeld);
            float surfaceY;
            bool hasSurface = TryGetWaterSurface(out surfaceY);
            float submersion = hasSurface ? (surfaceY - body.min.y) / bodyHeight : 2f;

            if (jumpHeld)
            {
                if (hasSurface && submersion <= SurfaceControlSubmersion)
                {
                    // Near the top, transition from full swim-up thrust to a lightly
                    // underdamped surface target. This keeps the upper hitbox in the water
                    // and produces a small smooth bob instead of hopping completely out,
                    // re-enabling vanilla gravity, and slamming back down.
                    float targetSubmersion = SurfaceTargetSubmersion +
                        Mathf.Sin(Time.time * SurfaceBobFrequency) * SurfaceBobAmplitude;
                    float error = submersion - targetSubmersion;
                    float desiredSpeed = Mathf.Clamp(
                        error * bodyHeight * SurfaceSpring,
                        -SurfaceMaxSinkSpeed * scale,
                        SurfaceMaxRiseSpeed * scale);
                    _verticalSpeed = Mathf.MoveTowards(
                        _verticalSpeed,
                        desiredSpeed,
                        SurfaceResponse * scale * dt);
                }
                else
                {
                    _verticalSpeed = Mathf.MoveTowards(
            ...
            // Vanilla movement already ran earlier this frame. Replace its Y displacement.
            // Clamp our own integration dt so a long frame cannot turn water velocity into
            // a multi-block step. A large Y delta on a normal frame is treated as a real
            // teleport/respawn; a large delta during a hitch is corrected instead, because
            // that is exactly when vanilla gravity can otherwise drop the player to the floor.
            Vector3 rawDelta = now - _lastPosition;
            float teleportGuard = Mathf.Max(1.5f * scale, 1.5f);
            bool likelyRealTeleport = rawDelta.y > teleportGuard ||
                Mathf.Abs(rawDelta.x) > teleportGuard || Mathf.Abs(rawDelta.z) > teleportGuard;
            // A large DOWNWARD delta while already overlapping water is never accepted as
            // a teleport. It is the failure mode that occasionally dumped the player onto
            // the pool floor when vanilla movement/hitches got a frame ahead of us.
            if (!likelyRealTeleport && _controller.enabled)
            {
                float wantedY = _verticalSpeed * dt;
                if (jumpHeld && hasSurface && wantedY > 0f)
                {
                    // Never let a held surface swim overshoot far enough that only the feet
                    // remain in water. That preserves the strict upper-90%-overlap swim rule.
                    float maxSurfaceRise = (submersion - MinimumSwimSubmersion) * bodyHeight;
                    if (wantedY > maxSurfaceRise) wantedY = Mathf.Max(maxSurfaceRise, 0f);
                }
        ...
        private void LeaveWater(Vector3 now, bool restoreGravity)
        {
            // Do not clear the claimed jump finger here. Surface bobbing can briefly leave
            // water between frames; clearing it loses the still-held finger because it will
            // not emit another TouchPhase.Began until the user releases and presses again.
            _localPlayerInWater = false;
            if (_wasSwimming && restoreGravity)
            {
                RestoreNormalGravity();
                if (_fps != null && _fps.movement != null)
                {
        ...
        private bool DetectJumpIntent(bool hudJumpHeld)
        {
            bool held = hudJumpHeld || (_fps != null && _fps.inputJump);
            try { if (Input.GetButton("Jump")) held = true; }
            catch { }
        ...
        private bool DetectHudJumpTouch()
        {
            try
            {
                ...
                // WSA/desktop pointer input can arrive as a mouse press instead of a stable
                // Input.touch stream. Claim the press when it begins on Jump and keep it
                // claimed until the physical mouse button is released.
                if (Input.GetMouseButtonUp(0)) _mouseJumpHeld = false;
                if (_mouseJumpHeld && Input.GetMouseButton(0)) held = true;
                if (Input.GetMouseButtonDown(0) && IsScreenPointOnJump(Input.mousePosition))
                {
    ...
    internal class CNRDLCMapLoader : MonoBehaviour
    {
        ...
        private static bool _prebuiltForJoin;
        private static int _prebuiltWaterParts;
        ...
        internal static bool IsBuildInProgress
        {
            get { return _buildInProgress; }
        }

        internal static bool IsActiveOrBuildingMap(string id)
        {
            if (!IsActive || string.IsNullOrEmpty(id)) return false;
            string activeId = PlayerPrefs.GetString(PrefId, "") ?? "";
            // Once the authoritative active ID matches, a repeated mapUrl event is stale
            // join synchronization, not a request to rebuild the map. OnLevelWasLoaded owns
            // the one legitimate scene build, including the short pre-build state where
            // _mapRoot has not been created yet.
            return string.Equals(activeId, id, StringComparison.OrdinalIgnoreCase);
        }
        ...
        internal static void ClearActive()
        {
            ...
            _prebuiltForJoin = false;
            _prebuiltWaterParts = 0;
            if (_mapRoot != null)
            {
                Destroy(_mapRoot);
                _mapRoot = null;
        ...
        internal static bool TryBuildActiveInCurrentScene(out string reason)
        {
            ...
            try
            {
                ...
                if (_mapRoot != null)
                {
                    ModEntry.Log("DLCMap: duplicate late-build request ignored; map root already exists");
                    return true;
                }
                if (_buildInProgress)
                {
                    ModEntry.Log("DLCMap: duplicate late-build request ignored; build already in progress");
                    return true;
                }
                ...
                loader.StartCoroutine(loader.BuildScene(false));
        ...
        {
            string scene = Application.loadedLevelName;
            bool bootstrapScene = scene == BootstrapScene || scene == "FreeRun5_1" || scene == "FreeRun8_1";

            // A pre-join DLC root is marked DontDestroyOnLoad so it can cross from
            // MultiplayerSelect into the donor scene. Once FinalizePrebuiltScene clears
            // _prebuiltForJoin, that persistent root must not survive a later trip back to
            // menus. Otherwise the menu camera can render the old map through its own fog.
            if (_mapRoot != null && !bootstrapScene && (!_prebuiltForJoin || scene == "MultiplayerSelect"))
            {
                Destroy(_mapRoot);
                _mapRoot = null;
                _prebuiltForJoin = false;
                ...
                ReleaseRenderResources();
                CNRMinecraftWaterRegistry.Clear();
                ModEntry.Log("DLCMap: cleared persistent map root after leaving gameplay, scene=" + scene);
            }
            if (!IsActive) return;
            if (!bootstrapScene) return;
            ...
            RemapVanillaSpawnPoints();
            if (_prebuiltForJoin && _mapRoot != null)
                StartCoroutine(FinalizePrebuiltScene());
            else
                StartCoroutine(BuildScene(false));
        }

        private IEnumerator BuildScene(bool prejoin)
        {
            if (_buildInProgress)
            {
                ModEntry.Log("DLCMap: duplicate build request ignored");
                yield break;
            }
            _buildInProgress = true;

            // Build before the first donor-scene frame is presented. Waiting here lets the
            // bootstrap map visibly flash, then freezes the player while the DLC is built.
            // Spawn remapping is repeated a couple of frames after the build instead.
            string reason;
            string path = PlayerPrefs.GetString(PrefPath, "");
            if (_prepared == null && !PrepareFile(path, out reason))
            {
            ...
            // A join prebuild happens in MultiplayerSelect. It creates the expensive map
            // hierarchy now, while spawn/player binding remains deferred until the donor scene.
            if (!prejoin) RemapVanillaSpawnPoints();
            CharacterController cc = prejoin ? null : ConfigurePlayerForDlcMap();
            ...
            try
            {
                ...
                if (prejoin)
                {
                    _prebuiltForJoin = true;
                    _prebuiltWaterParts = waterParts;
                    ModEntry.Log("DLCMap prebuilt before Photon join: chunks=" + _prepared.chunks.Length + " renderParts=" + renderParts + " collisionParts=" + collisionParts + " waterVolumes=" + waterParts);
                }
                else
                {
                    ...
                    // Re-apply spawn remapping now that the donor scene has finished constructing
                    // its named spawn objects. If the local player already spawned on the donor
                    // map, move it onto a valid DLC spawn before removing donor collision.
                    RemapVanillaSpawnPoints();
                    MoveExistingLocalPlayerOntoDlcMap();

                    // Keep donor collision alive until the DLC collision is complete and the
                    // existing local player (if any) has been moved onto the custom map. If the
            ...
            if (!prejoin)
            {
                // RoomMultiplayerMenu/spawn holders can appear just after OnLevelWasLoaded.
                // Repeat the remap after the DLC is already visible so late-created vanilla
                // spawn lists still point at the custom map without exposing donor geometry.
                yield return null;
                yield return null;
                RemapVanillaSpawnPoints();
            }
        ...
        internal static IEnumerator PrebuildActiveForJoin(Action<string, float> progress, Action<bool, string> completed)
        {
            if (!IsActive)
            {
                if (completed != null) completed(false, "DLC map is not active.");
                yield break;
            }
            if (_prebuiltForJoin && _mapRoot != null)
            {
                if (progress != null) progress("DLC map ready", 1f);
            ...
            if (_buildInProgress)
            {
                while (_buildInProgress) yield return null;
                bool existingOk = _prebuiltForJoin && _mapRoot != null;
                if (completed != null) completed(existingOk, existingOk ? "" : "DLC map build did not complete.");
                yield break;
            }
            ...
            if (loader == null)
            {
                if (completed != null) completed(false, "DLC map loader component is unavailable.");
                yield break;
            }

            if (progress != null) progress("Building DLC map geometry...", 0.12f);
            yield return loader.StartCoroutine(loader.BuildScene(true));
            bool ok = _prebuiltForJoin && _mapRoot != null;
            if (progress != null) progress(ok ? "DLC map ready" : "DLC map build failed", ok ? 1f : 0f);
            if (completed != null) completed(ok, ok ? "" : "DLC map could not be prebuilt before joining.");
        }

        internal static void CancelPrebuiltJoin()
        {
            // A failed pre-join build can leave a partial root before _prebuiltForJoin is set.
            // It is always safe to discard that root while still in MultiplayerSelect.
            if (!_prebuiltForJoin && Application.loadedLevelName != "MultiplayerSelect") return;
            _prebuiltForJoin = false;
            _prebuiltWaterParts = 0;
            if (_mapRoot != null)
            {
            ...
            ReleaseRenderResources();
            CNRMinecraftWaterRegistry.Clear();
            ModEntry.Log("DLCMap: cancelled prebuilt join map");
        }

        private IEnumerator FinalizePrebuiltScene()
        {
            if (!_prebuiltForJoin || _mapRoot == null) yield break;
            CharacterController cc = ConfigurePlayerForDlcMap();
            if (cc != null) cc.enabled = false;
            try
            {
                _mapRoot.transform.position = OriginVector(_prepared.origin);
                ...
                RemapVanillaSpawnPoints();
                MoveExistingLocalPlayerOntoDlcMap();
                if (_prebuiltWaterParts > 0) CNRZombieMod.ZombieNavGrid.Invalidate();
                StripBootstrapGeometry();
                ModEntry.Log("DLCMap: finalized prebuilt map after Photon join");
                _prebuiltForJoin = false;
                _prebuiltWaterParts = 0;
            }
            finally
        ...
        private static bool MoveExistingLocalPlayerOntoDlcMap()
        {
            if (_prepared == null) return false;
            ...
            try
            {
                ...
                {
                    TeamType team = CNRMultiplayerManager.mInstance.myPlayerInfo.mTeam;
                    if (team == TeamType.Cop)
                        targets = BuildSpawnTargets(_prepared.copSpawns, origin);
                    else if (team == TeamType.Robber)
                        targets = BuildSpawnTargets(_prepared.robberSpawns, origin);
                }
            }
            catch { }

            if (targets == null || targets.Count == 0)
                targets = BuildSpawnTargets(_prepared.spawns, origin);
            if (targets.Count == 0)
            {
                targets.AddRange(BuildSpawnTargets(_prepared.copSpawns, origin));
                targets.AddRange(BuildSpawnTargets(_prepared.robberSpawns, origin));
            }
            if (targets.Count == 0)
                targets.Add(origin + new Vector3(0f, DefaultSpawnHeight, 0f));

            Vector3 target = targets[UnityEngine.Random.Range(0, targets.Count)];
            player.transform.position = target;
            ModEntry.Log("DLCMap: moved existing local player onto DLC spawn " + target);
            return true;
        }

        private static CharacterController ConfigurePlayerForDlcMap()
        {
            GameObject player = GameObject.Find("ExampleCharacter");
        ...
        private static void StripBootstrapGeometry()
        {
            ...
            for (int i = 0; i < all.Length; i++)
            {
                ...
                // Never strip a live player/network hierarchy. Remote player PhotonView and
                // NetPlayerController components are not guaranteed to live on the top-level root.
                if (go.GetComponentInChildren<PhotonView>() != null) continue;
                if (go.GetComponentInChildren<NetPlayerController>() != null) continue;
                Renderer[] renderers = go.GetComponentsInChildren<Renderer>(true);
                for (int r = 0; r < renderers.Length; r++)
                {
                    Renderer renderer = renderers[r];
                    if (renderer != null && !IsLivePlayerObject(renderer.transform)) renderer.enabled = false;
                }
                Collider[] colliders = go.GetComponentsInChildren<Collider>(true);
                for (int c = 0; c < colliders.Length; c++)
                {
                    Collider collider = colliders[c];
                    if (collider != null && !IsLivePlayerObject(collider.transform)) collider.enabled = false;
                }
                cleared++;
            }
        ...
        private static bool IsLivePlayerObject(Transform t)
        {
            // Bootstrap geometry can contain remote-player objects beneath otherwise generic
            // roots. Never disable renderers/colliders belonging to a live local/network
            // character while stripping the donor scene.
            for (int i = 0; t != null && i < 16; i++, t = t.parent)
            {
                if (t.name == "ExampleCharacter") return true;
                try
                {
                    if (t.GetComponent<PhotonView>() != null) return true;
