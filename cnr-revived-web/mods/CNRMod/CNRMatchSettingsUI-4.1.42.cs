...
namespace CNRMods
{
    ...
    public class CNRMatchSettingsUIHook : MonoBehaviour
    {
        ...
        private void TryInstallGear()
        {
            ...
            try
            {
                ...
                if (existing != null)
                {
                    ...
                    if (_settingsIcon == null) _settingsIcon = LoadSharedSettingsIcon();
                ...
                if (donor == null)
                {
                    ModEntry.Log("MatchSettingsUI: no room control available to anchor settings icon");
                    return;
                }

                // Deliberately do NOT clone the donor's visible sprites. The SettingsMod
                // button on MainMenu is a bare IMGUI icon, not an NGUI +/- style button.
                // This object exists only as a stable NGUI-space anchor and input blocker.
                _gear = new GameObject(GearName);
                _gear.layer = donor.layer;
                ...
                _gear.AddComponent<CNRMatchSettingsBlocker>();

                _settingsIcon = LoadSharedSettingsIcon();
                _roomUiCamera = FindRoomUiCamera();
                _gear.SetActive(true);
                ModEntry.Log("MatchSettingsUI: bare SettingsMod-style gear installed icon="
                    + (_settingsIcon != null ? "ok" : "missing")
                    + " camera=" + (_roomUiCamera != null ? _roomUiCamera.name : "missing"));
            }
            catch (Exception ex)
            {
        ...
        {
            if (Application.loadedLevelName != "MultiplayerSelect") return;
            if (_gear == null || !_gear.activeInHierarchy || _roomPanel == null || !_roomPanel.activeInHierarchy) return;
            if (_overlay != null || _settingsIcon == null) return;

            Rect iconRect;
            if (!TryGetGearGuiRect(out iconRect)) return;

            if (_gearIconStyle == null)
            {
                if (_gearHoverTexture == null) _gearHoverTexture = MakeGearHighlight(new Color(1f, 1f, 1f, 0.2f));
                if (_gearActiveTexture == null) _gearActiveTexture = MakeGearHighlight(new Color(1f, 1f, 1f, 0.4f));
                _gearIconStyle = new GUIStyle();
                _gearIconStyle.imagePosition = ImagePosition.ImageOnly;
                _gearIconStyle.normal.background = null;
                _gearIconStyle.hover.background = _gearHoverTexture;
                _gearIconStyle.active.background = _gearActiveTexture;
                _gearIconStyle.border = new RectOffset(0, 0, 0, 0);
                _gearIconStyle.padding = new RectOffset(0, 0, 0, 0);
            }

            // Match the SettingsMod/MainMenu icon treatment exactly: transparent at rest,
            // subtle white hover/press feedback, and no green native-button surround.
            ...
            try
            {
                ...
                if (GUI.Button(iconRect, new GUIContent(_settingsIcon), _gearIconStyle))
                    HandleAction("open");
            }
            finally
            {
