...
namespace CNRZombieMod
{
    ...
    {
        public const  string Version = "0.8.51";
    ...
    public class ZombieHook : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (!inRoom)             return;
            if (!CNRMods.ZombieMode.IsZombieRoom && !CNRMods.ZombieMode.PendingZombie)
                return;
        ...
        void OnGUI()
        {
            ...
            if (!IsGameScene(scene)) return;
            if (!IsInRoom()) return;
            if (!CNRMods.ZombieMode.IsZombieRoom && !CNRMods.ZombieMode.PendingZombie) return;
            DrawZombieHud();
        ...
        private void SpawnZombies()
        {
            ...
            for (int i = 0; i < ZOMBIE_COUNT; i++)
            {
                ...
                try
                {
                    ...
                    _zombieVariant[(byte)(i + 1)] = variant;
        ...
        private void KillZombie(byte id, GameObject go, string attackerId)
        {
            ...
            if (string.IsNullOrEmpty(attackerId) || string.Equals(attackerId, GetLocalPeerId(), StringComparison.Ordinal))
                _points += award;
        ...
        public void OnZombieEvent(EventData ev)
        {
            try
            {
                ...
                {
                    ZombieModEntry.Log("OnZombieEvent: client zv=" + DumpValue(ht["zv"]));
