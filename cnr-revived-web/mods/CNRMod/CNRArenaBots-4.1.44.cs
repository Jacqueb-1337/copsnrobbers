...
namespace CNRMods
{
    ...
    public class CNRArenaBotManager : MonoBehaviour
    {
        ...
        private void ThinkBot(CNRArenaBotState bot)
        {
            ...
            // Face the target while engaging/searching. NetPlayerController applies a
            // -90 degree correction internally, so FaceDirection stores the compensated
            // CharacterBody-style yaw expected by vanilla remote-player rendering.
            FaceDirection(bot, dir);
            ...
            if (!visible)
            {
                // Search the last known location. Do not strafe or aim at the target's
                // hidden live position because the bot no longer knows it.
                move = dir;
        ...
        private void WanderBot(CNRArenaBotState bot)
        {
            ...
            if (now >= bot.NextWanderAt || bot.WanderDirection.sqrMagnitude < 0.01f)
            {
                float angle = UnityEngine.Random.Range(0f, 360f) * Mathf.Deg2Rad;
                bot.WanderDirection = new Vector3(Mathf.Sin(angle), 0f, Mathf.Cos(angle));
                bot.WanderUntil = now + UnityEngine.Random.Range(1.4f, 3.8f);
                bot.NextWanderAt = bot.WanderUntil + UnityEngine.Random.Range(0.25f, 1.1f);
            }
            ...
            FaceDirection(bot, bot.WanderDirection);
            bot.GunRotation = Quaternion.identity;
            bot.FirePoint = bot.Position + new Vector3(0f, 1.15f, 0f) + GetBotForward(bot) * 0.45f;
            bot.Status = PlayerStatus.walk;
            MoveBot(bot, bot.WanderDirection.normalized, WANDER_SPEED * THINK_INTERVAL);
        }
