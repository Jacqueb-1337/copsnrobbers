...
namespace CNRMods
{
    ...
    public class CNRDLCWeaponRuntime : MonoBehaviour
    {
        ...
        private const float BussinRecoilDrag = 7.5f;
        private const float BussinRecoilStopSpeed = 0.05f;
        ...
        private Vector3 _bussinRecoilVelocity = Vector3.zero;
        ...
        {
            if (_bussinRecoilVelocity.sqrMagnitude <= BussinRecoilStopSpeed * BussinRecoilStopSpeed)
            {
                _bussinRecoilVelocity = Vector3.zero;
                return;
            }
            ...
            // Add a second collision-aware motion layer instead of replacing the FPS
            // motor's own velocity. Jump/run/fall momentum therefore survives intact and
            // Bussin recoil stacks on top of it, including the full vertical component.
            CollisionFlags hit = controller.Move(_bussinRecoilVelocity * Time.deltaTime);
            if ((hit & CollisionFlags.Above) != 0 && _bussinRecoilVelocity.y > 0f)
                _bussinRecoilVelocity.y = 0f;
            if ((hit & CollisionFlags.Below) != 0 && _bussinRecoilVelocity.y < 0f)
                _bussinRecoilVelocity.y = 0f;

            _bussinRecoilVelocity = Vector3.MoveTowards(
                _bussinRecoilVelocity, Vector3.zero, BussinRecoilDrag * Time.deltaTime);
        }
        ...
        private void ApplyBussinRecoil()
        {
            ...
            // Add the full 3D camera-opposite impulse to a dedicated recoil velocity.
            // This is deliberately separate from FPScontroller.movement.velocity, so
            // existing jump/fall/run momentum is preserved and recoil stacks on top.
            Vector3 impulse = -cam.transform.forward.normalized * BussinRecoilVelocity;
            _bussinRecoilVelocity += impulse;
            ModEntry.Log("CNR DLC weapons: Bussin' additive recoil " + impulse + " total=" + _bussinRecoilVelocity);
        }
        ...
        private void UnbindBussinWeapon()
        {
            ...
            _weaponManager = null;
            _bussinSlotIndex = -1;
            // Do not clear _bussinRecoilVelocity here. Once a shot has imparted
            // momentum it should survive weapon switching/rebinding until it decays.
        }
