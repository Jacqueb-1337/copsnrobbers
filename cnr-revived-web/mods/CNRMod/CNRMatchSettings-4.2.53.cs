...
namespace CNRMods
{
    ...
    internal class CNRMatchSettingsData
    {
        ...
        public int BussinVelocityTenths = 10;    // DEV setting: 10 = 1.0x, range 0-50
        ...
        public CNRMatchSettingsData Clone()
        {
            ...
            d.BussinVelocityTenths = BussinVelocityTenths;
            return d;
        }
    }
    ...
    internal static class CNRMatchSettings
    {
        ...
        // The live Photon room is authoritative once joined. Pre-join queuing is an
        // optimization/gate, not the only way clients learn the host's rules.
        private static string _liveRoomKey = "";
        private static string _lastLiveSyncError = "";
        ...
        internal static bool SelectMode(MultiplayerSelectDirector msd, string mode, out string reason)
        {
            ...
            FreeForAllMode.PendingFfa = mode == "ffa";
            FreeForAllMode.IsFfaRoom = false;
        ...
        {
            mode = NormalizeMode(mode);
            if (mode == "tdm" || mode == "ffa") { vanilla = GrowthGameModeTag.tTeamDeathMatch; return true; }
        ...
        // Once room creation has started, the MultiplayerSelect UI is no longer a safe
        // source of truth. Loading can change its selected map/mode before Photon joins.
        // Prefer the frozen pending snapshot, then active state, then the host draft.
        internal static string PackAuthoritativeRoom()
        {
            CNRMatchSettingsData source = _pending != null ? _pending : (_active != null ? _active : _host);
            if (source == null) source = NewDefaults("tdm");
            return Pack(source);
        }

        internal static string GetAuthoritativeRoomMode()
        {
            CNRMatchSettingsData source = _pending != null ? _pending : (_active != null ? _active : _host);
            return source != null ? NormalizeMode(source.Mode) : "";
        }
        ...
        internal static string Pack(CNRMatchSettingsData d)
        {
            ...
            return d.Schema + "|" + NormalizeMode(d.Mode) + "|" + d.RoundSeconds + "|" + d.KcKillLimit + "|" +
                ...
                d.TdmBotsEnabled + "|" + d.TdmCopBots + "|" + d.TdmCopBotsAuto + "|" + d.TdmRobberBots + "|" + d.TdmRobberBotsAuto + "|" + d.KcBotsEnabled + "|" + d.KcBotCount + "|" + d.KcBotsAuto + "|" + d.BussinVelocityTenths;
        }
        ...
        internal static bool TryUnpack(string packed, out CNRMatchSettingsData data, out string reason)
        {
            ...
            try
            {
                ...
                else if (sourceSchema >= 3)
                {
                    ...
                    if (sourceSchema >= 4 && (p.Length < 19 || !int.TryParse(p[18], out d.BussinVelocityTenths)))
                    {
                        reason = "Room DEV settings contain invalid values.";
                        return false;
                    }
                }
        ...
        internal static void PromotePending(string roomName)
        {
            ...
            FreeForAllMode.IsFfaRoom = _active.Mode == "ffa";
            FreeForAllMode.PendingFfa = false;
            ModEntry.Log("MatchSettings: activated " + Pack(_active));
        }

        // Re-read the authoritative room properties after Photon has actually joined.
        // This makes rule replication self-healing if the legacy NGUI click path ever
        // bypasses the pre-join queue or if pending state is lost during scene loading.
        internal static bool SyncFromCurrentRoom(out string reason)
        {
            reason = "";
            Room room;
            try { room = PhotonNetwork.room; }
            catch (Exception ex) { reason = "Photon room unavailable: " + ex.Message; return false; }
            if (room == null) { reason = "Photon room is unavailable."; return false; }

            string roomName = room.name ?? "";
            string raw = CNRCompatibility.GetRoomProp(room, PropSettings) ?? "";
            string canonical = NormalizeMode(CNRCompatibility.GetRoomProp(room, CNRMatchMetadata.PropGameMode));
            string liveKey = roomName + "|" + raw + "|" + canonical;
            if (_active != null && string.Equals(_liveRoomKey, liveKey, StringComparison.Ordinal))
                return true;

            if (!QueueFromRoom(room, out reason))
            {
                if (!string.Equals(_lastLiveSyncError, reason, StringComparison.Ordinal))
                {
                    _lastLiveSyncError = reason ?? "";
                    ModEntry.Log("MatchSettings: live room sync waiting/failed: " + _lastLiveSyncError);
                }
                return false;
            }

            PromotePending(roomName);
            if (_active == null)
            {
                reason = "Room match settings could not be activated.";
                return false;
            }

            _liveRoomKey = liveKey;
            _lastLiveSyncError = "";
            ModEntry.Log("MatchSettings: synced from live Photon room=" + roomName + " settings=" + Pack(_active));
            return true;
        }
        ...
        internal static void ClearActive()
        {
            ...
            _lastLiveSyncError = "";
            CtfMode.WinScore = 30;
            // Clear only live-room FFA state here. PendingFfa belongs to the room-create
            // selection and must survive while PhotonNetwork.room is still null.
            FreeForAllMode.IsFfaRoom = false;
        }
        ...
        internal static string NormalizeMode(string mode)
        {
            ...
            if (mode == "freeforall" || mode == "free for all" || mode == "free-for-all") return "ffa";
            return mode;
        }
        ...
        internal static bool ValidateRanges(CNRMatchSettingsData d, out string reason)
        {
            ...
            if (d.BussinVelocityTenths < 0 || d.BussinVelocityTenths > 50) { reason = "Bussin velocity multiplier must be 0.0x-5.0x."; return false; }
            return true;
        }
        ...
        internal static void Sanitize(CNRMatchSettingsData d)
        {
            ...
            d.BussinVelocityTenths = Mathf.Clamp(d.BussinVelocityTenths, 0, 50);
        }
    }
    ...
    public class CNRMatchSettingsRuntimeHook : MonoBehaviour
    {
        ...
        void OnLevelWasLoaded(int level)
        {
            ...
            try
            {
                ...
                {
                    string ignored;
                    CNRMatchSettings.SyncFromCurrentRoom(out ignored);
