...
namespace CNRMods
{
    ...
    public class CNRBussinRemoteWeaponRenderer : MonoBehaviour
    {
        ...
        private void EnsureVisual(int actorId)
        {
            ...
            Transform donor = FindRemoteDonorWeapon(actor);
        ...
        private static Transform FindRemoteDonorWeapon(GameObject actor)
        {
            if (actor == null) return null;

            // Cops N Robbers multiplayer does not use the generic FPS-kit AnimationSync
            // hierarchy for its live remote characters. NetPlayerController drives a
            // WeaponManagerOnline object instead, so resolve the donor from that manager.
            NetPlayerController npc = actor.GetComponent<NetPlayerController>();
            if (npc == null) npc = actor.GetComponentInChildren<NetPlayerController>();
            if (npc != null && npc.myWeaponManager != null)
            {
                WeaponManager manager = npc.myWeaponManager.GetComponent<WeaponManager>();
                Transform donor = FindDonorInWeaponManager(manager);
                if (donor != null) return donor;

                donor = FindNamedChild(npc.myWeaponManager.transform, CNRDLCWeaponSystem.BussinDonor);
                if (donor != null) return donor;
            }

            // Keep the old generic Photon hierarchy as a compatibility fallback for
            // scenes or prefabs that really do use AnimationSync.
            AnimationSync sync = actor.GetComponentInChildren<AnimationSync>();
            if (sync != null && sync.thirdPersonWeapons != null)
            {
                Transform donor = FindNamedChild(sync.thirdPersonWeapons, CNRDLCWeaponSystem.BussinDonor);
                if (donor != null) return donor;
            }
            return null;
        }

        private static Transform FindDonorInWeaponManager(WeaponManager manager)
        {
            if (manager == null) return null;
            if (manager.allWeapons != null)
            {
                for (int i = 0; i < manager.allWeapons.Count; i++)
                {
                    WeaponScript weapon = manager.allWeapons[i];
                    if (weapon != null && weapon.weaponName == CNRDLCWeaponSystem.BussinDonor)
                        return weapon.transform;
                }
            }
            if (manager.allWeaponsTotal != null)
            {
                for (int i = 0; i < manager.allWeaponsTotal.Count; i++)
                {
                    WeaponScript weapon = manager.allWeaponsTotal[i];
                    if (weapon != null && weapon.weaponName == CNRDLCWeaponSystem.BussinDonor)
                        return weapon.transform;
                }
            }
    ...
    public class CNRDLCWeaponRuntime : MonoBehaviour
    {
        ...
        private float _bussinNextFireAt = -10f;
        ...
        private bool _hudPrepared;
        private bool _hudUpdateFaulted;
        ...
        private void UpdateBussinHudIcon(bool active)
        {
            ...
            try
            {
                ...
                UISprite sprite = _hudIconObject.GetComponent<UISprite>();
                if (sprite == null) return;

                if (CNRDLCWeaponSystem.IsBussinEquipped() && !_hudPrepared)
                {
                    CNRDLCWeaponThumbnail.PrepareHud(_hudIconObject);
                    _hudPrepared = true;
                }

                if (active)
                {
                    if (!sprite.enabled || sprite.spriteName != "CNR_Bussin_Hud")
                        CNRDLCWeaponThumbnail.ApplyHud(_hudIconObject);
                }
                else if (sprite.spriteName == "CNR_Bussin_Hud")
                {
                    CNRDLCWeaponThumbnail.ClearHud(_hudIconObject);
                }
            }
            catch (Exception ex)
            {
                _hudUpdateFaulted = true;
                ModEntry.Log("CNR DLC weapons: disabled Bussin HUD reassertion after exception: "
        ...
        private void BindActiveBussinWeapon()
        {
            ...
            // Keep the runtime donor WeaponScript disabled. CNR owns Bussin input itself,
            // which prevents the clone from consuming the same HUD flags and entering the
            // WSA-unsafe vanilla shotgun firing path.
            _bussinWeapon.enabled = false;
        ...
        private void BeginBussinReload()
        {
            ...
            if (_bussinWeapon.ShotGun.bulletsLeft >= _bussinWeapon.ShotGun.bulletsPerClip) return;
            if (_bussinWeapon.ShotGun.clips <= 0) return;
            StartCoroutine(BussinReloadRoutine(_bussinWeapon));
        }

        private IEnumerator BussinReloadRoutine(WeaponScript weapon)
        {
            if (weapon == null || weapon.ShotGun == null) yield break;
            weapon.isReload = true;
            weapon.aimed = false;
            float reloadTime = Mathf.Max(0.1f, weapon.ShotGun.reloadTime);
            try
            {
                if (weapon.audio != null && weapon.ShotGun.reloadSound != null)
                {
                    weapon.audio.clip = weapon.ShotGun.reloadSound;
                    weapon.audio.Play();
                }
                weapon.gameObject.BroadcastMessage("Reloading", reloadTime, SendMessageOptions.DontRequireReceiver);
            }
            catch (Exception ex) { ModEntry.Log("CNR DLC weapons: Bussin' reload start failed: " + ex.Message); }

            yield return new WaitForSeconds(reloadTime);
            if (_bussinWeapon != weapon || weapon == null || weapon.ShotGun == null) yield break;

            int missing = Mathf.Max(0, weapon.ShotGun.bulletsPerClip - weapon.ShotGun.bulletsLeft);
            int loaded = Mathf.Min(missing, Mathf.Max(0, weapon.ShotGun.clips));
            weapon.ShotGun.bulletsLeft += loaded;
            weapon.ShotGun.clips -= loaded;
            weapon.noBullets = false;
            weapon.isReload = false;
            weapon.canFire = true;
            weapon.canAim = false;
            weapon.reloadFlag = true;
        }
        ...
        private void DriveBussinFireInput()
        {
            ...
            if (_bussinWeapon.ShotGun == null || _bussinWeapon.ShotGun.bulletsLeft <= 0) return;
            if (Time.time < _bussinNextFireAt) return;

            float fireRate = Mathf.Max(0.03f, _bussinWeapon.ShotGun.fireRate);
            _bussinNextFireAt = Time.time + fireRate;
            FireBussinShell();
        }

        private void FireBussinShell()
        {
            WeaponScript weapon = _bussinWeapon;
            if (weapon == null || weapon.ShotGun == null || weapon.ShotGun.bulletsLeft <= 0) return;

            Transform firePoint = weapon.firePoint;
            Camera cam = Camera.main;
            if (firePoint == null || cam == null) return;

            Vector3 rayStart = cam.ScreenToWorldPoint(new Vector3(
                Screen.width * 0.5f, Screen.height * 0.5f, cam.nearClipPlane));
            Vector3 baseDirection = cam.transform.forward.normalized;
            Transform donorBullet = weapon.ShotGun.bullet;
            Bullet donor = donorBullet != null ? donorBullet.GetComponent<Bullet>() : null;
            float maxDistance = donor != null
                ? Mathf.Max(1f, Mathf.Abs((float)donor.speed) * Mathf.Max(0.01f, donor.life) * 10f)
                : 15000f;

            int level = Mathf.Clamp(CNRDLCWeaponSystem.GetBussinLevel(), 1, 3);
            int pellets = Mathf.Max(1, weapon.ShotGun.fractions);
            for (int i = 0; i < pellets; i++)
            {
                Vector3 spreadEuler = UnityEngine.Random.insideUnitSphere * weapon.ShotGun.errorAngle;
                Vector3 direction = Quaternion.Euler(spreadEuler) * baseDirection;
                direction.Normalize();

                RaycastHit hit;
                Vector3 endpoint = rayStart + direction * maxDistance;
                if (Physics.Raycast(rayStart, direction, out hit, maxDistance, 19))
                {
                    endpoint = hit.point;
                    ApplyBussinHitscanHit(hit, rayStart, level, donor);
                }
                CNRBulletTrailSystem.EmitRayTracer(rayStart, direction, endpoint);
            }

            weapon.ShotGun.bulletsLeft = Mathf.Max(0, weapon.ShotGun.bulletsLeft - 1);
            _bussinLastShotAt = Time.time;
            PlayBussinFireSound();
            ApplyBussinRecoil();

            if (weapon.ShotGun.bulletsLeft <= 0 && weapon.ShotGun.clips > 0)
            {
                _bussinReloadQueued = true;
                _bussinReloadAt = Time.time + BussinReloadAudioGuard;
            }
        }

        private void ApplyBussinHitscanHit(RaycastHit hit, Vector3 spawnPos, int level, Bullet donor)
        {
            if (hit.transform == null) return;
            if (hit.rigidbody != null && donor != null)
                hit.rigidbody.AddForce((hit.point - spawnPos).normalized * donor.impactForce, ForceMode.Impulse);

            float distance = Vector3.Distance(spawnPos, hit.point);
            float baseDamage = CNRDLCWeaponSystem.RollBussinPelletDamage(level);
            float scaledDamage = baseDamage * CNRDLCWeaponSystem.GetBussinDamageScale(level, distance);
            int bodyDamage = Mathf.Max(1, Mathf.RoundToInt(scaledDamage));
            int headDamage = Mathf.Max(1, Mathf.RoundToInt(scaledDamage * CNRDLCWeaponSystem.BussinHeadshotMultiplier));
            ...
            if (tag == "EnemyTag")
            {
                hit.transform.SendMessageUpwards("decreaseBlood", bodyDamage, SendMessageOptions.DontRequireReceiver);
                hit.transform.SendMessageUpwards("setTargetIsPlayer", true, SendMessageOptions.DontRequireReceiver);
                hit.transform.SendMessageUpwards("OnDamaged", bodyDamage, SendMessageOptions.DontRequireReceiver);
            }
            else if (tag == "EnemyHeadTag")
            {
                hit.transform.SendMessageUpwards("OnDamaged", headDamage, SendMessageOptions.DontRequireReceiver);
            }
            else if (tag == "EnemyBodyTag")
            {
                hit.transform.SendMessageUpwards("OnDamaged", bodyDamage, SendMessageOptions.DontRequireReceiver);
            ...
            {
                int footDamage = Mathf.Max(1, Mathf.RoundToInt(scaledDamage * 0.7f));
                hit.transform.SendMessageUpwards("OnDamaged", footDamage, SendMessageOptions.DontRequireReceiver);
            }
            else if (tag == "Player")
            {
                hit.transform.SendMessage("PlayerDamage", bodyDamage, SendMessageOptions.DontRequireReceiver);
            }
            else
            {
                hit.transform.SendMessageUpwards("setTargetIsPlayer", false, SendMessageOptions.DontRequireReceiver);
            }
        }
