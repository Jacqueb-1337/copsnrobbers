...
namespace CNRMods
{
    ...
    public class ModEntry
    {
        ...
        public const  string Version = "3.2.25";
    ...
    public class ContentManager : MonoBehaviour
    {
            ...
            try
            {
                ...
                if (tex.LoadImage(bytes))
                {
                    if (tex.width == 64 && tex.height == 64)
                        tex = MakeLegacyPlayerSkinBase(tex) ?? tex;
                    tex.name       = id;
        ...
        static Texture2D MakeLegacyPlayerSkinBase(Texture2D src)
        {
            if (src == null || src.width != 64 || src.height != 64) return src;

            var dst = new Texture2D(64, 32, TextureFormat.ARGB32, false);
            dst.filterMode = FilterMode.Point;
            dst.wrapMode   = TextureWrapMode.Clamp;
            Color32[] basePx = src.GetPixels32();
            Color32[] crop = new Color32[64 * 32];
            for (int y = 0; y < 32; y++)
            {
                for (int x = 0; x < 64; x++)
                {
                    int srcIdx = y * 64 + x;
                    int dstIdx = y * 64 + x;
                    if (srcIdx >= 0 && srcIdx < basePx.Length)
                        crop[dstIdx] = basePx[srcIdx];
                }
            }
            dst.SetPixels32(crop);
            dst.Apply(false, false);
            dst.name = src.name;
            return dst;
        }
