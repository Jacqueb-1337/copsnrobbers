...
namespace CNRMods
{
    ...
    internal static class CNRRoomResources
    {
            ...
            if (CNRMatchMetadata.TryGetSelectedCustomMap(out id, out displayName, out thumb))
            {
                ...
                if (official != null && !string.IsNullOrEmpty(official.Url))
                {
                    list.Add(new CNRRoomResource {
                        kind = official.IsDlcMap ? "dlcmap" : "map",
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(activeUrl))
                    {
                        ...
                        bool selectedIsActiveDlc = CNRDLCMapLoader.IsActive && !string.IsNullOrEmpty(id)
                            && string.Equals(activeDlcId, id, StringComparison.OrdinalIgnoreCase);
                        list.Add(new CNRRoomResource {
                            kind = selectedIsActiveDlc ? "dlcmap" : "map",
        ...
        internal static IEnumerator PrepareRoom(RoomInfo room, Action<string> progress, Action<bool, string> completed)
        {
            ...
            string mapType = (CNRCompatibility.GetRoomProp(room, CNRMatchMetadata.PropMapType) ?? "").Trim().ToLowerInvariant();
            if ((mapType == "dlc" || mapType == "legacy") && string.IsNullOrEmpty(mapId) && string.IsNullOrEmpty(mapName))
            {
                completed(false, "Cannot join: host custom-map identity is incomplete.");
                yield break;
            }
            if (mapType == "dlc" && string.IsNullOrEmpty(mapId))
            {
                completed(false, "Cannot join: host DLC map ID is missing.");
                yield break;
            }
            if (mapType == "vanilla" && (!string.IsNullOrEmpty(mapId) || !string.IsNullOrEmpty(mapName)))
            {
                completed(false, "Cannot join: room map metadata is contradictory.");
                yield break;
            }
