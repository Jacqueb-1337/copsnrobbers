...
namespace CNRMods
{
    ...
    public class ModEntry
    {
        ...
        public const  string Version = "3.2.51";
        ...
        public static void ForceLabelText(GameObject root, string text)
        {
            if (root == null || string.IsNullOrEmpty(text)) return;
            try
            {
                foreach (UILabel lbl in root.GetComponentsInChildren<UILabel>(true))
                {
                    if (lbl == null) continue;
                    if (!lbl.gameObject.activeSelf) lbl.gameObject.SetActive(true);
                    lbl.enabled = true;
                    string n = lbl.gameObject.name.ToLowerInvariant();
                    if (n.Contains("name") || n.Contains("skin") || n.Contains("label"))
                        lbl.text = text;
                }
            }
            catch (Exception ex) { Log("ForceLabelText error: " + ex.Message); }
        }
        ...
        internal static string ResolvePeerName(Type mgrType, object mgr, string peerId, System.Array others)
        {
            string nick = null;
            Log("ResolvePeerName: mgr=" + mgrType.Name + " peerId=" + peerId + " others=" + others.Length);
            for (int j = 0; j < others.Length; j++)
            {
                object p = others.GetValue(j);
                if (p == null) continue;
                Type pt = p.GetType();
                FieldInfo idField = pt.GetField("mId", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                FieldInfo nickField = pt.GetField("mNickName", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                if (idField == null || nickField == null) continue;
                string id = idField.GetValue(p) as string;
                if (id != peerId) continue;
                nick = nickField.GetValue(p) as string;
                Log("ResolvePeerName: matched otherPlayersInfoList idx=" + j + " nick='" + (nick ?? "null") + "'");
                if (!string.IsNullOrEmpty(nick) && nick != "PlayerX" && nick != "New Player") return nick;
            }

            try
            {
                foreach (object photonPlayer in GetPhotonOtherPlayers())
                {
                    if (photonPlayer == null) continue;
                    int pid = GetPhotonPlayerId(photonPlayer);
                    if (pid.ToString() != peerId) continue;
                    nick = GetPhotonPlayerName(photonPlayer);
                    Log("ResolvePeerName: photon match id=" + pid + " nick='" + (nick ?? "null") + "'");
                    if (!string.IsNullOrEmpty(nick)) return nick;
                }
            }
            catch (Exception ex) { Log("ResolvePeerName: photon lookup error " + ex.Message); }
            ...
            try
            {
                ...
                foreach (var asm in AppDomain.CurrentDomain.GetAssemblies())
                {
                    gk = asm.GetType("GameKitBinding");
                    if (gk != null) break;
                }
                if (gk != null)
                {
                    MethodInfo dm = gk.GetMethod("displayNameForPeer", BindingFlags.Static | BindingFlags.Public);
                    if (dm != null)
                    {
                        nick = dm.Invoke(null, new object[] { peerId }) as string;
                        Log("ResolvePeerName: GameKit displayNameForPeer -> '" + (nick ?? "null") + "'");
                        if (!string.IsNullOrEmpty(nick)) return nick;
                    }
                    else
                    {
                        Log("ResolvePeerName: displayNameForPeer missing");
                    }
                }
                else
                {
                    Log("ResolvePeerName: GameKitBinding missing");
                }
            }
            catch (Exception ex) { Log("ResolvePeerName: GameKit lookup error " + ex.Message); }
        ...
        public static void FixJoinFeedNames()
        {
            try
            {
                ...
                {
                    if (mgrType == null) { Log("FixJoinFeedNames: mgrType null"); continue; }
                    Log("FixJoinFeedNames: checking " + mgrType.Name);
                    FieldInfo instField = mgrType.GetField("mInstance", BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
                    if (instField == null) { Log("FixJoinFeedNames: missing mInstance on " + mgrType.Name); continue; }
                    object mgr = instField.GetValue(null);
                    if (mgr == null) { Log("FixJoinFeedNames: " + mgrType.Name + " instance null"); continue; }
                    ...
                    if (msgField == null || otherField == null)
                    {
                        Log("FixJoinFeedNames: missing msg/other fields on " + mgrType.Name);
                        continue;
                    }

                    var msgs = msgField.GetValue(mgr) as List<string>;
                    var others = otherField.GetValue(mgr) as System.Array;
                    if (msgs == null || others == null)
                    {
                        Log("FixJoinFeedNames: msgs or others null on " + mgrType.Name);
                        continue;
                    }
                    ...
                    {
                        string msg = msgs[i];
                        if (string.IsNullOrEmpty(msg) || !msg.Contains("New Player(id-")) continue;

                        int start = msg.IndexOf("New Player(id-") + "New Player(id-".Length;
                        int end = msg.IndexOf(")", start);
                        if (end <= start) continue;
                        string peerId = msg.Substring(start, end - start);
                        Log("FixJoinFeedNames: raw msg='" + msg + "' peerId=" + peerId);
        ...
        public static void Load()
        {
            ...
            LoadExternalMods();
            try { CNRZombieMod.ZombieModEntry.Load(); }
            catch (Exception ex) { Log("Load zombie mod error: " + ex.Message); }
        }
        ...
        private static void LoadExternalMods()
        {
            ...
            try
            {
                ...
                foreach (string path in files)
                {
                    ...
                    if (name.StartsWith("CNRZombieMod", StringComparison.OrdinalIgnoreCase)) continue;
                    try
                    {
    ...
    public static class CnrEventBus
    {
        ...
        public static void OnPhotonEventReceived(byte eventCode, object content, int senderId)
        {
            ...
            {
                if ((ht["ctf"] as string) == "1") CtfMode.IsCtfRoom = true;
            }
            if (ht.ContainsKey("zombie"))
            {
                if ((ht["zombie"] as string) == "1") ZombieMode.IsZombieRoom = true;
            }
            if (ht.ContainsKey("ctf_s"))
            {
                string ctfState = ht["ctf_s"] as string;
                if (!string.IsNullOrEmpty(ctfState))
                {
            ...
            ModEntry.Log("CnrEvent[" + senderId + "]:"
                ...
                + (ht.ContainsKey("ctf")     ? " ctf="     + ht["ctf"]     : "")
                + (ht.ContainsKey("zombie")  ? " zombie="  + ht["zombie"]  : "")
                + (ht.ContainsKey("ctf_s")   ? " ctf_s=<state>" : ""));
        }
    ...
    public class RedirectHook : MonoBehaviour
    {
        ...
        private static void InstallGameKitJoinHook()
        {
            if (ModEntry._gameKitJoinHookInstalled) return;
            try
            {
                foreach (var asm in AppDomain.CurrentDomain.GetAssemblies())
                {
                    Type gk = asm.GetType("GameKitManager");
                    if (gk == null) continue;
                    EventInfo ev = gk.GetEvent("peerConnected", BindingFlags.Static | BindingFlags.Public);
                    if (ev == null) { ModEntry.Log("InstallGameKitJoinHook: peerConnected event missing"); return; }
                    Delegate handler = Delegate.CreateDelegate(ev.EventHandlerType,
                        typeof(ModEntry).GetMethod("OnGameKitPeerConnected",
                            BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.Public));
                    ev.AddEventHandler(null, handler);
                    ModEntry._gameKitJoinHookInstalled = true;
                    ModEntry.Log("InstallGameKitJoinHook: installed");
                    return;
                }
                ModEntry.Log("InstallGameKitJoinHook: GameKitManager type missing");
            }
            catch (Exception ex)
            {
                ModEntry.Log("InstallGameKitJoinHook error: " + ex.Message);
            }
        }

        private static void OnGameKitPeerConnected(string peerId)
        {
            try
            {
            ...
            {
                ModEntry.Log("OnGameKitPeerConnected error: " + ex.Message);
            }
        }

        public static void ProcessPendingJoinFeedFixes()
        {
            try
            {
                while (true)
            ...
            {
                ModEntry.Log("ProcessPendingJoinFeedFixes error: " + ex.Message);
            }
        }

        private static bool TryRewriteJoinMessage(string peerId)
        {
            Type[] mgrTypes = new Type[2];
            foreach (var asm in AppDomain.CurrentDomain.GetAssemblies())
            {
        ...
        private void ProcessPendingCnrEvents()
        {
            ...
            if (_isMaster && CnrEventBus.NeedMapBroadcast)
            {
                ...
                ModEntry.RaiseCnrEvent(ht);
                    ModEntry.Log("Master: re-broadcast to new joiner (mapUrl=" + (string.IsNullOrEmpty(url) ? "(none)" : url) + " v" + ModEntry.Version + (CtfMode.IsCtfRoom ? " ctf=1" : "") + (ZombieMode.IsZombieRoom ? " zombie=1" : "") + ")");
            }
        ...
        private void OnEnteredRoom(Type pnt, bool asMaster)
        {
            ...
            // If this client created the room with CTF mode, confirm it immediately
            // so CtfHook knows it's a CTF room without waiting for the broadcast.
            if (asMaster && CtfMode.PendingCtf)
            {
                CtfMode.IsCtfRoom = true;
                ModEntry.Log("OnEnteredRoom: CTF room confirmed (master created)");
            }
            if (asMaster && ZombieMode.PendingZombie)
            {
                ZombieMode.IsZombieRoom = true;
                ModEntry.Log("OnEnteredRoom: Zombie room confirmed (master created)");
            }
        ...
        private IEnumerator BroadcastPropsDelayed(Type pnt)
        {
            ...
            if (ZombieMode.IsZombieRoom)
            {
                ht["zombie"] = "1";
            }
            ModEntry.RaiseCnrEvent(ht);
            ModEntry.Log("CnrEvent broadcast (delayed): v" + ModEntry.Version + " skin=" + (string.IsNullOrEmpty(skin) ? "(none)" : skin) + (CtfMode.IsCtfRoom ? " ctf=1" : "") + (ZombieMode.IsZombieRoom ? " zombie=1" : ""));
        }
    ...
    {
        private delegate void BlitRectDelegate(int sx, int sy, int w, int h, int dx, int dy, bool flipX, bool flipY);
        ...
        void OnLevelWasLoaded(int level)
        {
            ...
            // Spawn CTF hook in game scenes; self-destructs if not a CTF room or wrong mode.
            if (Array.IndexOf(gameScenes, Application.loadedLevelName) >= 0)
            {
                var ctfGo = new GameObject("CNRCtfHook");
                ctfGo.AddComponent<CtfHook>();
            }
            // Inject CTF checkbox into the multiplayer map-selection screen.
            if (Application.loadedLevelName == "MultiplayerSelect")
            {
                var injGo = new GameObject("CNRCtfInjector");
                injGo.AddComponent<CtfCheckBoxInjector>();
                var zombieGo = new GameObject("CNRZombieInjector");
                zombieGo.AddComponent<ZombieCheckBoxInjector>();
            }
        }
        ...
        static void EnsureDirs()
        {
            try
            {
                ...
                // Prevent Android's media scanner from indexing / re-encoding our cached images.
                // Place .nomedia in EVERY directory — Android's media scanner respects it per-directory,
                // not necessarily inherited from parent dirs on all Android versions.
                foreach (string d in dirs)
                {
                    string nm = d.TrimEnd('/') + "/.nomedia";
                    if (!File.Exists(nm))
                        try { File.WriteAllBytes(nm, new byte[0]); } catch {}
                }
            }
        ...
        static void LoadCachedManifest()
        {
            ...
            try
            {
                ...
                foreach (var sk in OfficialSkins)
                {
                    ...
                    if (File.Exists(p)) LoadSkinFile(sk.Id, p);
                }
                ...
                // Verify hashes; delete files whose hash has changed so they get re-downloaded.
                // We no longer clear VersionPref here — FetchAndSync will download any missing files.
                if (!VerifyAndClean())
                    ModEntry.Log("ContentManager: hash mismatch(es) found; bad files deleted — will re-download missing files on next sync");
            ...
            foreach (var sk in OfficialSkins)
            {
                if (!string.IsNullOrEmpty(sk.Hash))
                {
                    ...
                    if (File.Exists(p))
                    {
                        ...
                        {
                            ModEntry.Log("ContentManager: skin hash mismatch [" + sk.Id + "] server=" + sk.Hash.ToLower() + " local=" + got + " ? deleting");
                            try { File.Delete(p); } catch {}
                            allOk = false;
                        }
                    }
        ...
        IEnumerator FetchAndSync()
        {
            ...
            if (newVer != oldVer || newVer == "" || AnyContentMissing())
            {
                ModEntry.Log("ContentManager: syncing (" + oldVer + " -> " + newVer + ")");
        ...
        // Returns true if any expected content file is absent from the cache directory.
        // Used to detect files removed by VerifyAndClean() so we re-download only what's missing.
        static bool AnyContentMissing()
        {
            foreach (var te in OfficialTextures)
                if (!File.Exists(TexCacheDir + te.Id + ".png")) return true;
            foreach (var sk in OfficialSkins)
                if (!File.Exists(SkinCacheDir + sk.Id + ".png")) return true;
            foreach (var g in OfficialGuns)
            {
                // Gun files may be .json or .png depending on the URL
                bool gunExists = File.Exists(GunCacheDir + g.Id + ".json")
                              || File.Exists(GunCacheDir + g.Id + ".png")
                              || File.Exists(GunCacheDir + g.Id);
                if (!gunExists) return true;
            }
            foreach (var m in OfficialMaps)
                if (!File.Exists(MapCacheDir + m.Id + ".json")) return true;
            foreach (var d in OfficialData)
                if (!File.Exists(DataCacheDir + d.Id + ".json")) return true;
            return false;
        }

        // Download all content items (maps → MapCacheDir, textures → TexCacheDir, data → DataCacheDir)
        // Skips files that are already present and whose hash matches the manifest.
        IEnumerator DownloadItems()
        {
            foreach (var m in OfficialMaps)
            {
                string path = MapCacheDir + m.Id + ".json";
                // Skip if the file already exists and the hash is valid
                if (File.Exists(path) && (string.IsNullOrEmpty(m.Hash) || ComputeMD5(path) == m.Hash.ToLower()))
                { /* already cached */ }
                else
                {
                ...
                // Download thumbnail if URL is provided and not already cached
                if (!string.IsNullOrEmpty(m.ThumbnailUrl))
                {
                    ...
                    if (File.Exists(thumbPath) && (string.IsNullOrEmpty(m.ThumbnailHash) || ComputeMD5(thumbPath) == m.ThumbnailHash.ToLower()))
                    {
            ...
            foreach (var te in OfficialTextures)
            {
                ...
                if (File.Exists(path) && (string.IsNullOrEmpty(te.Hash) || ComputeMD5(path) == te.Hash.ToLower()))
                {
                    LoadTexFile(te.Id, path);
                }
                else
                {
            ...
            foreach (var d in OfficialData)
            {
                ...
                if (File.Exists(path) && (string.IsNullOrEmpty(d.Hash) || ComputeMD5(path) == d.Hash.ToLower()))
                { /* already cached */ }
                else
                {
            ...
            foreach (var sk in OfficialSkins)
            {
                ...
                if (File.Exists(path) && (string.IsNullOrEmpty(sk.Hash) || ComputeMD5(path) == sk.Hash.ToLower()))
                {
                    LoadSkinFile(sk.Id, path);
                }
                else
                {
            ...
            foreach (var g in OfficialGuns)
            {
                ...
                if (File.Exists(path) && (string.IsNullOrEmpty(g.Hash) || ComputeMD5(path) == g.Hash.ToLower()))
                { /* already cached */ }
                else
                {
        ...
        public static Texture2D CropPlayerBase(Texture2D src)
        {
            if (src == null) return null;
            if (src.width < 64 || src.height < 32) return src;
            if (src.height == 32) return src;

            Texture2D dst = new Texture2D(64, 32, TextureFormat.ARGB32, false);
            dst.filterMode = FilterMode.Point;
            dst.wrapMode = TextureWrapMode.Clamp;
            Color32[] srcPx = src.GetPixels32();
            Color32[] outPx = new Color32[64 * 32];
            int srcYOffset = src.height - 32;
            for (int y = 0; y < 32; y++)
            {
                for (int x = 0; x < 64; x++)
                {
                    int srcIdx = (srcYOffset + y) * src.width + x;
                    if (srcIdx >= 0 && srcIdx < srcPx.Length)
                        outPx[y * 64 + x] = srcPx[srcIdx];
                }
            }
            dst.SetPixels32(outPx);
            dst.Apply(false, false);
            dst.name = src.name + "_base";
            return dst;
        }

        public static Texture2D CropPlayerOverlay(Texture2D src)
        {
            if (src == null) return null;
            if (src.width < 64 || src.height < 64) return null;
            ...
            for (int i = 0; i < outPx.Length; i++) outPx[i] = transparent;

            for (int y = 0; y < 32; y++)
            {
                for (int x = 0; x < 64; x++)
                {
                    int srcIdx = y * src.width + x;
                    if (srcIdx >= 0 && srcIdx < srcPx.Length)
                        outPx[y * 64 + x] = srcPx[srcIdx];
                }
            }
            dst.SetPixels32(outPx);
            dst.Apply(false, false);
            dst.name = src.name + "_overlay";
            return dst;
        }

        public static void RemovePlayerOverlay(GameObject root)
        {
            try
            {
                if (root == null) return;
                Transform existing = root.transform.Find("CNR_SkinOverlay");
                if (existing != null) UnityEngine.Object.Destroy(existing.gameObject);
            }
            catch { }
        }

        public static void ApplyPlayerOverlayToModel(GameObject root, Texture2D sourceTex)
        {
            try
            {
                if (root == null) return;
                if (sourceTex == null || sourceTex.width != 64 || sourceTex.height != 64)
                {
                    RemovePlayerOverlay(root);
                    return;
                }

                Texture2D overlayTex = CropPlayerOverlay(sourceTex);
                if (overlayTex == null)
                {
                    RemovePlayerOverlay(root);
                    return;
                ...
                if (clone == null)
                {
                    clone = UnityEngine.Object.Instantiate(root) as GameObject;
                    if (clone == null) return;
                    clone.name = "CNR_SkinOverlay";
                    clone.transform.parent = root.transform;
                    clone.transform.localPosition = Vector3.zero;
                    clone.transform.localRotation = Quaternion.identity;
                    clone.transform.localScale = Vector3.one * 1.015f;

                    foreach (Collider c in clone.GetComponentsInChildren<Collider>(true))
                        UnityEngine.Object.Destroy(c);
                    foreach (MonoBehaviour mb in clone.GetComponentsInChildren<MonoBehaviour>(true))
                        if (mb != null) UnityEngine.Object.Destroy(mb);
                }

                Shader cutout = Shader.Find("Transparent/Cutout/Diffuse");
                Shader transparent = cutout != null ? cutout : Shader.Find("Transparent/Diffuse");
                foreach (Renderer r in clone.GetComponentsInChildren<Renderer>(true))
                {
                    ...
                    bool skinMat = mname.StartsWith("Skin_", StringComparison.OrdinalIgnoreCase);
                    r.enabled = skinMat;
                    if (!skinMat) continue;
                    if (transparent != null) r.material.shader = transparent;
                    r.material.mainTexture = overlayTex;
                    if (r.material.HasProperty("_Cutoff")) r.material.SetFloat("_Cutoff", 0.1f);
                    r.material.renderQueue = 3001;
                }
            }
            catch (Exception ex)
            {
                ModEntry.Log("ContentManager: ApplyPlayerOverlayToModel error: " + ex.Message);
            }
        }
        ...
        static void LoadTexFile(string id, string path)
        {
            try
            {
                ...
                {
                    tex.name       = id;
                    tex.filterMode = FilterMode.Point;   // no bilinear blur on pixel-art skins
        ...
        static void LoadSkinFile(string id, string path)
        {
            try
            {
                byte[] bytes = File.ReadAllBytes(path);
            ...
            catch (Exception ex) { ModEntry.Log("ContentManager: LoadSkinFile error [" + id + "]: " + ex.Message); }
        }
        ...
        IEnumerator ApplyTextureSwaps()
        {
            ...
            foreach (var sk in OfficialSkins)
            {
                ...
                Texture2D baseTex = CropPlayerBase(tex) ?? tex;
                matToTex[sk.MaterialName] = baseTex;                  // e.g. Skin_34_2
                if (!string.IsNullOrEmpty(sk.SkinName))
                    matToTex[sk.SkinName + "_1"] = baseTex;           // e.g. Skin_34_1 (head)
            }
            ...
            if (!string.IsNullOrEmpty(dlcId))
            {
                ...
                {
                    Texture2D baseDlcTex = CropPlayerBase(dlcTex) ?? dlcTex;
                    try
                    {
                        ...
                        {
                            matToTex[curSkinInfo.mBodyMaterialName] = baseDlcTex;
                            if (!string.IsNullOrEmpty(curSkinInfo.mHeadMaterialName))
                                matToTex[curSkinInfo.mHeadMaterialName] = baseDlcTex;
            ...
            try
            {
                ...
                if (curSkinInfo != null)
                {
                    Texture2D handTex = null;
                    if (!string.IsNullOrEmpty(curSkinInfo.mName))
                    {
                        string skinId = curSkinInfo.mName;
                        // Derive the equipped skin texture from the current material mapping first.
                        if (!string.IsNullOrEmpty(curSkinInfo.mBodyMaterialName) && matToTex.ContainsKey(curSkinInfo.mBodyMaterialName))
                            handTex = matToTex[curSkinInfo.mBodyMaterialName];
                    }

                    if (handTex == null && !string.IsNullOrEmpty(curSkinInfo.mBodyMaterialName))
                    {
                        foreach (var kv in _texCache)
                        {
                            // Fallback to any currently loaded texture; better than leaving vanilla hands.
                            handTex = CropPlayerBase(kv.Value) ?? kv.Value;
                            if (handTex != null) break;
                        }
                    }

                    if (handTex != null)
                    {
                        foreach (HandSkinControl hsc in (HandSkinControl[])FindObjectsOfType(typeof(HandSkinControl)))
                        {
                            if (hsc == null || hsc.curHandSkinMaterial == null) continue;
                            hsc.curHandSkinMaterial.mainTexture = handTex;
                        }
                    }
                }
            }
            catch (Exception ex) { ModEntry.Log("ContentManager: hand skin sync error: " + ex.Message); }
    ...
    // ─────────────────────────────────────────────────────────────────────────
    //  ICNRSkinHook  -  shared contract consumed by CNRSkinGridUI
    // ─────────────────────────────────────────────────────────────────────────
    public interface ICNRSkinHook
    {
        int         Total           { get; }
        int         EquippedListIdx { get; }  // list idx of currently-equipped skin (-1 unknown)
        string      GetDisplayName(int listIdx);
        int         GetPrice(int listIdx);
        bool        GetOwned(int listIdx);
        Texture2D   GetThumb(int listIdx);
        void        NavigateTo(int listIdx);  // preview this skin on the model
        void        EquipCurrent();           // equip or purchase the currently-previewed skin
        GameObject  RoleModel     { get; }
        GameObject  SkinTabMarker { get; }    // null = always show overlay
        Rect        SkinContentRect { get; }  // content area in GUI space (Y=0 top); full-screen fallback
        Rect        SkinBackButtonRect { get; } // screen rect for the vanilla BackToMenu button
        void        SetVanillaVisible(bool show);  // toggle vanilla skin panel children on/off
    }
    ...
    public class CNRSkinStoreHook : MonoBehaviour, ICNRSkinHook
    {
        // -- Reflection cache --------------------------------------------------
        static FieldInfo  _fi_curSkinId, _fi_skinUsingId, _fi_skinNameList,
                          _fi_gSkinItemInfo, _fi_hatSkin, _fi_bodySkin,
                          _fi_skinDisNameLabel, _fi_skinPriceLabel, _fi_skinSetUnlockLabel, _fi_skinTipLabel,
                          _fi_storeSkinTabBtn, _fi_storeCancelBtn, _fi_storeSkinPanel;
        ...
        static void EnsureReflection()
        {
            ...
            _fi_skinDisNameLabel = t.GetField("skinDisNameLabel", BindingFlags.Public | BindingFlags.Instance);
            _fi_skinPriceLabel   = t.GetField("skinPriceLabel",   BindingFlags.Public | BindingFlags.Instance);
            _fi_skinSetUnlockLabel = t.GetField("skinSetUnlockLabel", BindingFlags.Public | BindingFlags.Instance);
            _fi_skinTipLabel     = t.GetField("skinTipLabel",     BindingFlags.Public | BindingFlags.Instance);
            ...
            _fi_storeSkinTabBtn = t.GetField("skinPageBtn", BindingFlags.NonPublic | BindingFlags.Instance);
            _fi_storeCancelBtn  = t.GetField("cancelBtn",   BindingFlags.NonPublic | BindingFlags.Instance);
            _fi_storeSkinPanel  = t.GetField("skinPanel",   BindingFlags.NonPublic | BindingFlags.Instance);
            _reflReady = true;
        }
        ...
        // -- Vanilla panel children toggle ------------------------------------
        GameObject        _vanillaPanelGO;
        List<GameObject>  _vanillaHiddenChildren = new List<GameObject>();
        Rect              _cachedContentRect;
        Rect              _profileBackButtonRect;
        Rect              _storeBackButtonRect;
        ...
        // -- SkinTabMarker: one of the intercepted button GOs; active iff the skin tab is shown --
        public GameObject SkinTabMarker { get; private set; }

        // -- Thumbnail helper --
        public Texture2D GetSkinThumb(int idx)
        {
            if (idx >= 0 && _extBody != null && idx < _extBody.Length && _extBody[idx] != null)
                return _extBody[idx].mainTexture as Texture2D;
            return null;
        }

        // -- ICNRSkinHook explicit implementations --
        int    ICNRSkinHook.Total           { get { return _total; } }
        int    ICNRSkinHook.EquippedListIdx { get { return SkinUsingId; } }
        string ICNRSkinHook.GetDisplayName(int idx)
        {
            var info = GetExtInfo(idx);
            return info != null && !string.IsNullOrEmpty(info.mNameDisplay) ? info.mNameDisplay
                 : info != null ? info.mName : "???";
        }
        int  ICNRSkinHook.GetPrice(int idx) { var i = GetExtInfo(idx); return i != null ? i.mPrice : 0; }
        bool ICNRSkinHook.GetOwned(int idx) { var i = GetExtInfo(idx); return i != null && i.mIsEnabled; }
        Texture2D ICNRSkinHook.GetThumb(int idx) { return GetSkinThumb(idx); }
        void ICNRSkinHook.NavigateTo(int idx)   { CurSkinId = idx; RefreshSetSkinData(); }
        void ICNRSkinHook.EquipCurrent()        { OnSetUnlock(); }
        GameObject ICNRSkinHook.SkinTabMarker   { get { return SkinTabMarker; } }
        Rect ICNRSkinHook.SkinContentRect       { get { return _cachedContentRect; } }
        Rect ICNRSkinHook.SkinBackButtonRect    { get { return _storeBackButtonRect; } }
        void ICNRSkinHook.SetVanillaVisible(bool show)
        {
            if (show) ShowVanillaChildren(); else HideVanillaChildren();
        }
        GameObject ICNRSkinHook.RoleModel
        {
            get
            {
                if (_store == null) return null;
                var fi = typeof(UIStoreDirector).GetField("roleModel", BindingFlags.Public | BindingFlags.Instance);
                return fi != null ? (GameObject)fi.GetValue(_store) : null;
            }
        }

        void HideVanillaChildren()
        {
            if (_vanillaPanelGO == null) return;
            _vanillaHiddenChildren.Clear();
            for (int i = 0; i < _vanillaPanelGO.transform.childCount; i++)
            {
                GameObject child = _vanillaPanelGO.transform.GetChild(i).gameObject;
                if (child.activeSelf) { child.SetActive(false); _vanillaHiddenChildren.Add(child); }
            }
        }

        void ShowVanillaChildren()
        {
            foreach (GameObject child in _vanillaHiddenChildren)
                if (child != null) child.SetActive(true);
            _vanillaHiddenChildren.Clear();
            HideNativeSkinNavButtons();
        }

        public void HideNativeSkinNavButtons()
        {
            foreach (var evt in (UIStoreBtnEvent[])Resources.FindObjectsOfTypeAll(typeof(UIStoreBtnEvent)))
            {
                if (evt.buttonName == UIStoreBtnEvent.ButtonName.LeftSkin ||
                    evt.buttonName == UIStoreBtnEvent.ButtonName.RightSkin)
                    evt.gameObject.SetActive(false);
            }
        }

        static Rect ComputePanelScreenRect(GameObject panelGO, Camera cam)
        {
            if (panelGO == null || cam == null) return new Rect(0, 0, Screen.width, Screen.height);
            ...
            if (panel != null)
            {
                ...
                if (cr.z > 0f && cr.w > 0f)
                {
                    ...
                    Vector3 s0 = cam.WorldToScreenPoint(t.TransformPoint(cr.x - hw, cr.y - hh, 0f));
                    Vector3 s1 = cam.WorldToScreenPoint(t.TransformPoint(cr.x + hw, cr.y + hh, 0f));
            ...
            // Fallback: aggregate all BoxCollider children
            float xMin = float.MaxValue, xMax = float.MinValue;
            float yMin = float.MaxValue, yMax = float.MinValue;
            foreach (BoxCollider box in panelGO.GetComponentsInChildren<BoxCollider>())
            {
                Vector3 ctr = box.bounds.center, ext = box.bounds.extents;
                Vector3 sa = cam.WorldToScreenPoint(new Vector3(ctr.x - ext.x, ctr.y - ext.y, ctr.z));
                Vector3 sb = cam.WorldToScreenPoint(new Vector3(ctr.x + ext.x, ctr.y + ext.y, ctr.z));
                xMin = Mathf.Min(xMin, Mathf.Min(sa.x, sb.x)); xMax = Mathf.Max(xMax, Mathf.Max(sa.x, sb.x));
                yMin = Mathf.Min(yMin, Mathf.Min(sa.y, sb.y)); yMax = Mathf.Max(yMax, Mathf.Max(sa.y, sb.y));
            }
        ...
        // Compute the store skin-page content rect from SkinPage tab-button height + SkinCancel position.
        // top    = bottom-of-tab + 0.25 * tabH
        // left   = 0.75 * tabH from each side
        // bottom = top-of-SkinCancel-btn - 0.25 * tabH
        // DEBUG: log every UIStoreBtnEvent with its full hierarchy path and GUI-space Y extents.
        static void DumpStoreBtnEvents(Camera cam)
        {
            float sh = Screen.height;
            var allBtnEvents = (UIStoreBtnEvent[])Resources.FindObjectsOfTypeAll(typeof(UIStoreBtnEvent));
            ModEntry.Log("CNRSkinStoreHook DUMP: sh=" + sh + " total=" + allBtnEvents.Length);
            foreach (var evt in allBtnEvents)
            {
                // Build full path up to root
                string path = evt.gameObject.name;
                Transform p = evt.transform.parent;
                while (p != null) { path = p.name + "/" + path; p = p.parent; }

                float topGuiY = -1f, botGuiY = -1f;
                if (cam != null)
                {
                    var box = evt.GetComponent<BoxCollider>();
                    if (box != null)
                    {
                        Transform tr = box.transform;
                        Vector3 wc = tr.TransformPoint(box.center);
                        float hh = box.size.y * tr.lossyScale.y * 0.5f;
                        Vector3 sa = cam.WorldToScreenPoint(new Vector3(wc.x, wc.y - hh, wc.z));
                        Vector3 sb = cam.WorldToScreenPoint(new Vector3(wc.x, wc.y + hh, wc.z));
                        topGuiY = sh - Mathf.Max(sa.y, sb.y);
                        botGuiY = sh - Mathf.Min(sa.y, sb.y);
                    }
                }
                ModEntry.Log(string.Format("  [{0}] top={1:F0} bot={2:F0}  {3}", evt.buttonName, topGuiY, botGuiY, path));
            }
        }

        static Rect ComputeStoreContentRect(UIImageButton tabBtnComp, GameObject skinPanelGO, Camera cam)
        {
            float sw = Screen.width, sh = Screen.height;
            // Measure SkinPage tab button (Panel(Tool)/Image Button(SkinPage))
            float tabH = 0f, tabTopGuiY = 0f, tabBotGuiY = 0f;
            if (tabBtnComp != null && cam != null)
            {
                var box = ((Component)tabBtnComp).GetComponent<BoxCollider>();
                if (box != null)
                {
                    Transform tr = box.transform;
                    Vector3 wc = tr.TransformPoint(box.center);
                    ...
                    Vector3 sa = cam.WorldToScreenPoint(new Vector3(wc.x, wc.y - hh, wc.z));
                    Vector3 sb = cam.WorldToScreenPoint(new Vector3(wc.x, wc.y + hh, wc.z));
                    tabH       = Mathf.Abs(sb.y - sa.y);
                    tabTopGuiY = sh - Mathf.Max(sa.y, sb.y);
                    tabBotGuiY = tabTopGuiY + tabH;
                }
            }
            if (tabH < 1f) { tabH = sh * 0.07f; tabTopGuiY = sh - tabH; tabBotGuiY = sh; }

            bool lowerHalf = tabBotGuiY > sh * 0.5f;

            float rx = tabH * 0.75f;
            float rw = sw - 2f * tabH * 0.75f;
            float ry, rh;

            if (lowerHalf)
            {
                // Tab strip is at the bottom of the measured space.
                // Use symmetric margins: same tabH*1.25 padding from each end.
                float contentTop = tabH * 1.25f;
                float contentBot = tabTopGuiY - tabH * 1.25f;
                ry = contentTop;
                rh = contentBot - contentTop;
            }
            else
            {
                // Tab strip is at the top — content sits below it, above the back button footer.
                // Find Panel(Tool)/Image Button(Back) for the bottom boundary.
                float backTopGuiY = sh - tabH * 1.5f; // fallback
                var backBtnGO = GameObject.Find("UI Root (2D)/Camera/Anchor/Panel(Tool)/Image Button(Back)");
                if (backBtnGO != null && cam != null)
                {
                    var box = backBtnGO.GetComponent<BoxCollider>();
                    if (box != null)
                    {
                        Transform tr = box.transform;
                        Vector3 wc = tr.TransformPoint(box.center);
                        ...
                        Vector3 sa = cam.WorldToScreenPoint(new Vector3(wc.x, wc.y - hh, wc.z));
                        Vector3 sb = cam.WorldToScreenPoint(new Vector3(wc.x, wc.y + hh, wc.z));
                        float candidateY = sh - Mathf.Max(sa.y, sb.y);
                        if (candidateY > tabBotGuiY && candidateY < sh) backTopGuiY = candidateY;
                    }
                }
                else if (cam != null)
                {
                    ...
                    foreach (var evt in (UIStoreBtnEvent[])Resources.FindObjectsOfTypeAll(typeof(UIStoreBtnEvent)))
                    {
                        if (evt.buttonName != UIStoreBtnEvent.ButtonName.BackToMenu) continue;
                        var box = evt.GetComponent<BoxCollider>();
                        ...
                        Vector3 sa = cam.WorldToScreenPoint(new Vector3(wc.x, wc.y - hh, wc.z));
                        Vector3 sb = cam.WorldToScreenPoint(new Vector3(wc.x, wc.y + hh, wc.z));
                        float topY = sh - Mathf.Max(sa.y, sb.y);
                        if (topY > bestY && topY > tabBotGuiY && topY < sh) { bestY = topY; backTopGuiY = topY; }
                    }
                }
                ry = tabBotGuiY + tabH * 0.25f;
                rh = (backTopGuiY - tabH * 0.25f) - ry;
            }

            ModEntry.Log(string.Format("CNRSkinStoreHook: tabH={0:F0} tabTopY={1:F0} tabBotY={2:F0} lowerHalf={3} -> rect=({4:F0},{5:F0},{6:F0},{7:F0})",
                tabH, tabTopGuiY, tabBotGuiY, lowerHalf, rx, ry, rw, rh));
            if (rw < 50f || rh < 50f) return new Rect(0, 0, sw, sh);
            return new Rect(rx, ry, rw, rh);
        }

        static Rect ComputeStoreBackButtonRect(Camera cam)
        {
            ...
            foreach (var evt in (UIStoreBtnEvent[])Resources.FindObjectsOfTypeAll(typeof(UIStoreBtnEvent)))
            {
                ...
                Vector3 wc = tr.TransformPoint(box.center);
                float hh = box.size.y * tr.lossyScale.y * 0.5f;
                Vector3 sa = cam.WorldToScreenPoint(new Vector3(wc.x - box.size.x * 0.5f * tr.lossyScale.x, wc.y - hh, wc.z));
                Vector3 sb = cam.WorldToScreenPoint(new Vector3(wc.x + box.size.x * 0.5f * tr.lossyScale.x, wc.y + hh, wc.z));
                float topY = sh - Mathf.Max(sa.y, sb.y);
                if (topY > bestY)
                {
                    bestY = topY;
                    xMin = Mathf.Min(sa.x, sb.x);
                    xMax = Mathf.Max(sa.x, sb.x);
                    yMin = Mathf.Min(sa.y, sb.y);
                    yMax = Mathf.Max(sa.y, sb.y);
                }
            }
            if (xMax > xMin && yMax > yMin)
                return new Rect(xMin, Screen.height - yMax, xMax - xMin, yMax - yMin);
        ...
        IEnumerator Setup()
        {
            ...
            // Find vanilla skin panel, hide its children, compute content rect from tab/back anchors
            _vanillaPanelGO = _fi_storeSkinPanel != null ? _fi_storeSkinPanel.GetValue(_store) as GameObject : null;
            // Prefer the named UI camera; UICamera.mainCamera may return the wrong camera
            // in the store scene since the skin panel is inactive at setup time.
            Camera setupCam = null;
            var uiRootCamGO = GameObject.Find("UI Root (2D)/Camera");
            if (uiRootCamGO != null) setupCam = uiRootCamGO.GetComponent<Camera>();
            if (setupCam == null) setupCam = UICamera.mainCamera ?? Camera.main;
            ModEntry.Log("CNRSkinStoreHook: using camera " + (setupCam != null ? setupCam.name : "null"));
            HideVanillaChildren();
            // Get the SkinPage UIImageButton from its GameObject (same as profile, but field is GameObject)
            UIImageButton storeTabBtnComp = null;
            if (_fi_storeSkinTabBtn != null)
            {
                var tabGO = _fi_storeSkinTabBtn.GetValue(_store) as GameObject;
                if (tabGO != null) storeTabBtnComp = tabGO.GetComponent<UIImageButton>();
            }
            _cachedContentRect = ComputeStoreContentRect(storeTabBtnComp, _vanillaPanelGO, setupCam);
            _storeBackButtonRect = ComputeStoreBackButtonRect(setupCam);
            // Use the panel GO itself as the tab-visibility marker (game calls SetActive on it)
            if (_vanillaPanelGO != null) SkinTabMarker = _vanillaPanelGO;
            ModEntry.Log("CNRSkinStoreHook: ready panel=" + (_vanillaPanelGO != null ? _vanillaPanelGO.name : "null")
                + " rect=" + _cachedContentRect + " marker=" + (SkinTabMarker != null ? SkinTabMarker.name : "null"));
            var gridGo = new GameObject("CNRSkinGridUI");
            gridGo.AddComponent<CNRSkinGridUI>().Hook = this;
        }
        ...
        void BuildExtended()
        {
            ...
            for (int i = 0; i < _dlcSkins.Length; i++)
            {
                ...
                if (tex != null) mat.mainTexture = ContentManager.CropPlayerBase(tex) ?? tex;
        ...
        {
            _mi_SetSkinData.Invoke(_store, null);
            StartCoroutine(DeferredPreviewRefresh());
        }

        IEnumerator DeferredPreviewRefresh()
        {
            for (int i = 0; i < 3; i++)
            {
                yield return null;
                _mi_SetSkinData.Invoke(_store, null);
                UpdatePreviewLabels(CurSkinId);
                ApplyPreviewOverlay();
            }
        }

        void UpdatePreviewLabels(int idx)
        {
            var info = GetExtInfo(idx);
            if (info == null) return;
            string displayName = info.mNameDisplay ?? info.mName ?? "";
            UILabel nameLbl   = _fi_skinDisNameLabel != null ? _fi_skinDisNameLabel.GetValue(_store) as UILabel : null;
            UILabel priceLbl  = _fi_skinPriceLabel != null ? _fi_skinPriceLabel.GetValue(_store) as UILabel : null;
            UILabel unlockLbl = _fi_skinSetUnlockLabel != null ? _fi_skinSetUnlockLabel.GetValue(_store) as UILabel : null;
            if (nameLbl != null)
            {
            ...
            if (priceLbl != null) { priceLbl.gameObject.SetActive(true); priceLbl.enabled = true; priceLbl.text = info.mPrice.ToString(); }
            if (unlockLbl != null)
            {
                unlockLbl.gameObject.SetActive(true);
                unlockLbl.enabled = true;
                unlockLbl.text = info.mIsEnabled ? (SkinUsingId == idx ? "Equipped" : "Set") : "Unlock";
            }
            ModEntry.ForceLabelText(_store != null ? _store.gameObject : null, displayName);
        }

        void ApplyPreviewOverlay()
        {
            GameObject model = ((ICNRSkinHook)this).RoleModel;
            if (model == null || _dlcSkins == null || CurSkinId < _dlcStart)
            {
                ContentManager.RemovePlayerOverlay(model);
                return;
            }

            int di = CurSkinId - _dlcStart;
            if (di < 0 || di >= _dlcSkins.Length)
            {
                ContentManager.RemovePlayerOverlay(model);
                return;
            }

            Texture2D tex = ContentManager.GetSkinTexture(_dlcSkins[di].Id);
            ContentManager.ApplyPlayerOverlayToModel(model, tex);
        }
            ...
            foreach (var evt in allBtnEvents)
            {
                    ...
                    bn == UIStoreBtnEvent.ButtonName.BackToMenu)
                {
                    if (bn == UIStoreBtnEvent.ButtonName.BackToMenu)
                    {
                        ...
                        var guard = evt.gameObject.GetComponent<CNRSkinStoreBackGuard>();
                        if (guard == null) guard = evt.gameObject.AddComponent<CNRSkinStoreBackGuard>();
                        guard.Hook = this;
                        ModEntry.Log("CNRSkinStoreHook: guarded " + bn);
                        continue;
                    }
                    evt.enabled = false;
                    ...
                    if (SkinTabMarker == null) SkinTabMarker = evt.gameObject;
                    if (bn == UIStoreBtnEvent.ButtonName.LeftSkin ||
                        bn == UIStoreBtnEvent.ButtonName.RightSkin)
                        evt.gameObject.SetActive(false);
                }
        ...
        public void OnBack()
        {
            var grid = CNRSkinGridUI.Active;
            if (grid != null && grid.IsDetailView)
            {
                grid.ReturnToGrid();
                return;
            }
    ...
    public class CNRSkinBtnInterceptor : MonoBehaviour
    {
        ...
        void OnClick()
        {
            ...
            switch (BtnTag)
            {
                ...
                case UIStoreBtnEvent.ButtonName.BackToMenu:   Hook.OnBack();      break;
                case UIStoreBtnEvent.ButtonName.SkinSetUnlock: Hook.OnSetUnlock(); break;
            }
        }
    }

    public class CNRSkinStoreBackGuard : MonoBehaviour
    {
        public CNRSkinStoreHook Hook;

        void LateUpdate()
        {
            foreach (var evt in GetComponents<UIStoreBtnEvent>())
                if (evt.buttonName == UIStoreBtnEvent.ButtonName.BackToMenu)
                    evt.buttonName = UIStoreBtnEvent.ButtonName.Nil;
        }

        void OnClick()
        {
            if (Hook != null) Hook.OnBack();
        }
    }
    ...
    public class CNRProfileHook : MonoBehaviour, ICNRSkinHook
    {
        // -- Reflection cache --------------------------------------------------
        static FieldInfo  _fi_gSkinItemInfo, _fi_skinNameList,
                          ...
                          _fi_profileSkinTabBtn, _fi_profileCoinLabel, _fi_profileSkinPagePanel;
        ...
        static void EnsureReflection()
        {
            ...
            _fi_skinUsingId          = t.GetField("skinUsingId",          BindingFlags.NonPublic | BindingFlags.Instance);
            _mi_SetSkinData          = t.GetMethod("SetSkinData",         BindingFlags.NonPublic | BindingFlags.Instance);
            _fi_profileSkinTabBtn    = t.GetField("skinPageBtn",    BindingFlags.Public | BindingFlags.Instance);
            _fi_profileCoinLabel     = t.GetField("coinNumLabel",  BindingFlags.Public | BindingFlags.Instance);
            _fi_profileSkinPagePanel = t.GetField("skinPagePanel", BindingFlags.Public | BindingFlags.Instance);
            _reflReady = true;
        }
        ...
        OfficialSkinEntry[] _dlcSkins;
        GSkinItemInfo[]     _cachedExtInfo; // local copy for ICNRSkinHook
        Material[]          _cachedExtBody; // local copy for thumbnails
        GameObject          _profileSkinMarker; // one of the skin tab buttons (for visibility check)
        ...
        IEnumerator Setup()
        {
            ...
            BuildAndInject();
            FindSkinMarker();  // always cache a visibility marker regardless of DLC count

            // Find vanilla skin panel, hide its children, compute content rect from tab/back anchors
            _vanillaPanelGO = _fi_profileSkinPagePanel != null
                ? _fi_profileSkinPagePanel.GetValue(_profile) as GameObject : null;
            Camera setupCam = UICamera.mainCamera ?? Camera.main;
            HideVanillaChildren();
            UIImageButton tabBtnComp = _fi_profileSkinTabBtn != null
                ? _fi_profileSkinTabBtn.GetValue(_profile) as UIImageButton : null;
            _cachedContentRect = ComputeProfileContentRect(tabBtnComp, setupCam);
            _profileBackButtonRect = ComputeProfileBackButtonRect(setupCam);
            // Use the panel GO itself as the tab-visibility marker (game calls SetActive on it)
            if (_vanillaPanelGO != null) _profileSkinMarker = _vanillaPanelGO;
            ModEntry.Log("CNRProfileHook: panel=" + (_vanillaPanelGO != null ? _vanillaPanelGO.name : "null")
                + " rect=" + _cachedContentRect + " marker=" + (_profileSkinMarker != null ? _profileSkinMarker.name : "null"));
            ...
            string equippedDlcId = PlayerPrefs.GetString("CNR_EquippedDLCSkin", "");
            if (!string.IsNullOrEmpty(equippedDlcId) && _dlcSkins != null)
            {
                for (int i = 0; i < _dlcSkins.Length; i++)
                {
        ...
        void BuildAndInject()
        {
            ...
            if (_dlcSkins.Length == 0)
            {
                // Still cache vanilla arrays so ICNRSkinHook can serve them
                _cachedExtInfo = (GSkinItemInfo[])_fi_gSkinItemInfo.GetValue(_profile);
                _cachedExtBody = (Material[])_fi_bodySkin.GetValue(_profile);
                return;
            }
            ...
            _cachedExtInfo = extInfo;
            _cachedExtBody = extBody;
        }

        // Cache a profile skin button GO for panel-bounds and tab-visibility checks.
        void FindSkinMarker()
        {
            if (_profileSkinMarker != null) return;
            ...
            foreach (var evt in allBtnEvents)
            {
                ...
                {
                    _profileSkinMarker = evt.gameObject;
                    return;
                }
            }
        }

        // -- ICNRSkinHook implementation ---------------------------------------
        int ICNRSkinHook.Total { get { return _vanillaCount + (_dlcSkins != null ? _dlcSkins.Length : 0); } }
        int ICNRSkinHook.EquippedListIdx
        {
            get
            {
                int uid = SkinUsingId;
                if (_dlcSkins != null && uid >= _dlcMatStart)
                {
                    int di = uid - _dlcMatStart;
                    if (di < _dlcSkins.Length) return _vanillaCount + di;
                }
                return uid; // vanilla: matIdx == listIdx
            }
        }
        string ICNRSkinHook.GetDisplayName(int idx)
        {
            if (_cachedExtInfo == null || idx < 0 || idx >= _cachedExtInfo.Length) return "??";
            var info = _cachedExtInfo[idx];
            return info != null && !string.IsNullOrEmpty(info.mNameDisplay) ? info.mNameDisplay
                 : info != null ? info.mName : "??";
        }
        int  ICNRSkinHook.GetPrice(int idx)  { return 0; } // no buying in profile screen
        bool ICNRSkinHook.GetOwned(int idx)  { return true; } // only owned skins appear in list
        Texture2D ICNRSkinHook.GetThumb(int idx)
        {
            if (_cachedExtBody == null) return null;
            int matIdx = idx < _vanillaCount ? idx : _dlcMatStart + (idx - _vanillaCount);
            if (matIdx < 0 || matIdx >= _cachedExtBody.Length || _cachedExtBody[matIdx] == null) return null;
            return _cachedExtBody[matIdx].mainTexture as Texture2D;
        }
        void ICNRSkinHook.NavigateTo(int idx)
        {
            if (_profile == null || _fi_idList == null) return;
            var idList = (int[])_fi_idList.GetValue(_profile);
            if (idList == null || idx < 0 || idx >= idList.Length) return;
            CurSkinIndexInIdList = idx;
            CurSkinId            = idList[idx] - 1;
            _mi_SetSkinData.Invoke(_profile, null);
            StartCoroutine(DeferredPreviewRefresh(idx));
        }

        IEnumerator DeferredPreviewRefresh(int idx)
        {
            for (int i = 0; i < 3; i++)
            {
                yield return null;
                _mi_SetSkinData.Invoke(_profile, null);
                UpdatePreviewLabels(idx);
                ApplyPreviewOverlay(idx);
            }
        }
        ...
        void UpdatePreviewLabels(int idx)
        {
            ...
            if (info == null) return;
            string displayName = info.mNameDisplay ?? info.mName ?? "";
            UILabel nameLbl   = _fi_skinDisNameLabel != null ? _fi_skinDisNameLabel.GetValue(_profile) as UILabel : null;
            UILabel priceLbl  = _fi_skinPriceLabel != null ? _fi_skinPriceLabel.GetValue(_profile) as UILabel : null;
            UILabel unlockLbl = _fi_skinSetUnlockLabel != null ? _fi_skinSetUnlockLabel.GetValue(_profile) as UILabel : null;
            if (nameLbl != null)
            {
                nameLbl.gameObject.SetActive(true);
                nameLbl.enabled = true;
            ...
            if (unlockLbl != null)
            {
                unlockLbl.gameObject.SetActive(true);
                unlockLbl.enabled = true;
                unlockLbl.text = info.mIsEnabled ? (CurSkinIndexInIdList == idx ? "Equipped" : "Set") : "Unlock";
            }
            ModEntry.ForceLabelText(_profile != null ? _profile.gameObject : null, displayName);
        }

        void ApplyPreviewOverlay(int listIdx)
        {
            GameObject model = ((ICNRSkinHook)this).RoleModel;
            if (model == null || _dlcSkins == null || listIdx < _vanillaCount)
            {
                ContentManager.RemovePlayerOverlay(model);
                return;
            }

            int di = listIdx - _vanillaCount;
            if (di < 0 || di >= _dlcSkins.Length)
            {
                ContentManager.RemovePlayerOverlay(model);
                return;
        ...
        void ICNRSkinHook.EquipCurrent()           { OnSetSkin(); }
        GameObject ICNRSkinHook.SkinTabMarker       { get { return _profileSkinMarker; } }
        Rect ICNRSkinHook.SkinContentRect           { get { return _cachedContentRect; } }
        Rect ICNRSkinHook.SkinBackButtonRect        { get { return _profileBackButtonRect; } }
        void ICNRSkinHook.SetVanillaVisible(bool show)
        {
            if (show) ShowVanillaChildren(); else HideVanillaChildren();
        }
        GameObject ICNRSkinHook.RoleModel
        {
            get
            {
                ...
                var fi = typeof(UIProfileDirector).GetField("roleModel", BindingFlags.Public | BindingFlags.Instance);
                return fi != null ? (GameObject)fi.GetValue(_profile) : null;
            }
        }
        ...
        public void HideNativeSkinNavButtons()
        {
            foreach (var evt in (UIProfileBtnEvent[])Resources.FindObjectsOfTypeAll(typeof(UIProfileBtnEvent)))
            {
                if (evt.buttonName == UIProfileBtnEvent.ButtonName.LeftSkin ||
                    evt.buttonName == UIProfileBtnEvent.ButtonName.RightSkin)
                    evt.gameObject.SetActive(false);
            }
        }
        ...
        // Compute the profile skin-page content rect from tab-button height + BackToMenu position.
        // top    = bottom-of-tabs + 0.25 * tabH
        // left   = 0.75 * tabH from each side
        // bottom = top-of-BackToMenu-btn - 0.25 * tabH
        static Rect ComputeProfileContentRect(UIImageButton tabBtnComp, Camera cam)
        {
            float sw = Screen.width, sh = Screen.height;
            float tabH = 0f, tabBotGuiY = 0f;
            if (tabBtnComp != null && cam != null)
            {
                var box = ((Component)tabBtnComp).GetComponent<BoxCollider>();
                if (box != null)
                {
                    ...
                    Vector3 sb = cam.WorldToScreenPoint(new Vector3(wc.x, wc.y + hh, wc.z));
                    tabH       = Mathf.Abs(sb.y - sa.y);
                    float tabTopGuiY = sh - Mathf.Max(sa.y, sb.y);
                    tabBotGuiY = tabTopGuiY + tabH;
                }
            }
            if (tabH < 1f) { tabH = sh * 0.07f; tabBotGuiY = tabH; } // fallback ~7% screen height

            float backTopGuiY = sh - tabH * 0.5f; // fallback near bottom
            var allBtnEvents = (UIProfileBtnEvent[])Resources.FindObjectsOfTypeAll(typeof(UIProfileBtnEvent));
            foreach (var evt in allBtnEvents)
            {
                if (evt.buttonName != UIProfileBtnEvent.ButtonName.BackToMenu || cam == null) continue;
                var box = evt.GetComponent<BoxCollider>();
                if (box != null)
                {
                    Transform tr = box.transform;
                    ...
                    Vector3 sa = cam.WorldToScreenPoint(new Vector3(wc.x, wc.y - hh, wc.z));
                    Vector3 sb = cam.WorldToScreenPoint(new Vector3(wc.x, wc.y + hh, wc.z));
                    backTopGuiY = sh - Mathf.Max(sa.y, sb.y);
                }
                break;
            }

            float rx = tabH * 0.75f;
            float ry = tabBotGuiY + tabH * 0.25f;
            float rw = sw - 2f * tabH * 0.75f;
            float rh = (backTopGuiY - tabH * 0.25f) - ry;
            ModEntry.Log(string.Format("CNRProfileHook: tabH={0:F0} tabBotY={1:F0} backTopY={2:F0} -> rect=({3:F0},{4:F0},{5:F0},{6:F0})",
                tabH, tabBotGuiY, backTopGuiY, rx, ry, rw, rh));
            if (rw < 50f || rh < 50f) return new Rect(0, 0, sw, sh);
            return new Rect(rx, ry, rw, rh);
        }

        static Rect ComputeProfileBackButtonRect(Camera cam)
        {
            if (cam == null) return new Rect(0, 0, Screen.width, Screen.height);
            float xMin = float.MaxValue, xMax = float.MinValue, yMin = float.MaxValue, yMax = float.MinValue;
            foreach (var evt in (UIProfileBtnEvent[])Resources.FindObjectsOfTypeAll(typeof(UIProfileBtnEvent)))
            {
                if (evt.buttonName != UIProfileBtnEvent.ButtonName.BackToMenu) continue;
                var box = evt.GetComponent<BoxCollider>();
                if (box == null) continue;
                Transform tr = box.transform;
                Vector3 wc = tr.TransformPoint(box.center);
                float hh = box.size.y * tr.lossyScale.y * 0.5f;
                float hw = box.size.x * tr.lossyScale.x * 0.5f;
                Vector3 sa = cam.WorldToScreenPoint(new Vector3(wc.x - hw, wc.y - hh, wc.z));
                Vector3 sb = cam.WorldToScreenPoint(new Vector3(wc.x + hw, wc.y + hh, wc.z));
                xMin = Mathf.Min(xMin, Mathf.Min(sa.x, sb.x));
                xMax = Mathf.Max(xMax, Mathf.Max(sa.x, sb.x));
                yMin = Mathf.Min(yMin, Mathf.Min(sa.y, sb.y));
                yMax = Mathf.Max(yMax, Mathf.Max(sa.y, sb.y));
            }
            if (xMax > xMin && yMax > yMin)
                return new Rect(xMin, Screen.height - yMax, xMax - xMin, yMax - yMin);
            return new Rect(0, 0, Screen.width, Screen.height);
            ...
            foreach (var evt in allBtnEvents)
            {
                    ...
                    bn == UIProfileBtnEvent.ButtonName.BackToMenu)
                {
                    if (bn == UIProfileBtnEvent.ButtonName.BackToMenu)
                    {
                        ...
                        var guard = evt.gameObject.GetComponent<CNRProfileBackGuard>();
                        if (guard == null) guard = evt.gameObject.AddComponent<CNRProfileBackGuard>();
                        guard.Hook = this;
                        ModEntry.Log("CNRProfileHook: guarded " + bn);
                        continue;
                    }
                    evt.enabled = false;
    ...
    public class CNRProfileBtnInterceptor : MonoBehaviour
    {
        ...
        void OnClick()
        {
            ...
            switch (BtnTag)
            {
                ...
                case UIProfileBtnEvent.ButtonName.BackToMenu:Hook.OnBack();    break;
            }
        }
    }

    public class CNRProfileBackGuard : MonoBehaviour
    {
        public CNRProfileHook Hook;

        void LateUpdate()
        {
            foreach (var evt in GetComponents<UIProfileBtnEvent>())
                if (evt.buttonName == UIProfileBtnEvent.ButtonName.BackToMenu)
                    evt.buttonName = UIProfileBtnEvent.ButtonName.Nil;
        }
    ...
    //  CNR SKIN GRID UI
    // ─────────────────────────────────────────────────────────────────────────
    // Spawned by CNRSkinStoreHook (StoreScene) and CNRProfileHook (ProfileScene).
    // Draws a custom scrollable skin-picker grid + detail view via OnGUI,
    // replacing the vanilla left/right nav.  Uses ICNRSkinHook as its data source.
    public class CNRSkinGridUI : MonoBehaviour
    {
        public static CNRSkinGridUI Active;
        public ICNRSkinHook Hook;           // assigned by spawner immediately after AddComponent

        enum View { Grid, Detail }
        View    _view            = View.Grid;
        Vector2 _scroll          = Vector2.zero;
        int     _detailListIdx   = -1;

        // Model-rotation drag state (Detail view)
        bool    _dragging;
        Vector2 _lastDrag;
        float   _modelYaw;

        // ── Swipe-to-scroll (same pattern as CNRSettingsMod) ─────────────────
        bool  _swipeActive        = false;
        bool  _swipeDragging      = false;
        float _swipeStartScreenY  = 0f;
        float _swipePrevScreenY   = 0f;
        bool  _swipeGuiFingerDown = false;
        bool  _swipeIsScrolling   = false;
        float _swipeGuiDownY      = 0f;
        const float SWIPE_DEAD_PX  = 15f;  // screen pixels before swipe commits
        const float SWIPE_GUI_DEAD = 10f;  // IMGUI pixels for click suppression

        // ── Per-skin rendered thumbnails ──────────────────────────────────────
        Texture2D[] _thumbTex;              // one per skin index; null until rendered
        bool        _thumbsReady   = false; // true once all thumbs captured
        bool        _thumbsStarted = false; // coroutine guard

        // Style / texture cache – built lazily on first OnGUI
        GUIStyle _sCell, _sCellEquipped, _sCellOwned, _sName, _sPrice,
                 _sBtn, _sTitle, _sHint;
        bool     _stylesBuilt;
        Texture2D _txBg, _txDark, _txBlue, _txGray, _txWhite, _txCellEquipped;

        // Content rect cached each OnGUI frame (used in Update for swipe guard)
        Rect _lastContentRect = new Rect(0, 0, 9999, 9999);
        bool _scrollInitialized = false;  // scroll-to-equipped done once on first draw

        // ── Lifecycle ─────────────────────────────────────────────────────────
        void Start()
        {
            Active = this;
            if (Hook != null && !_thumbsStarted)
            {
                _thumbsStarted = true;
                StartCoroutine(RenderThumbs());
            }
        }
        ...
        void OnDestroy()
        {
            ...
            // Restore vanilla UI if we're destroyed mid-detail (e.g. scene change)
            if (_view == View.Detail && Hook != null)
                Hook.SetVanillaVisible(false);
        }

        public bool IsDetailView { get { return _view == View.Detail; } }
        ...
        static Transform ThumbRoot(GameObject modelGO)
        {
            return modelGO.transform.parent != null ? modelGO.transform.parent : modelGO.transform;
        }

        static void ResetThumbPose(GameObject modelGO)
        {
            if (modelGO == null) return;
            var root = ThumbRoot(modelGO);
            root.rotation = Quaternion.identity;
            modelGO.transform.localRotation = Quaternion.identity;
        }

        static Bounds ComputeModelBounds(GameObject modelGO)
        {
        ...
        static void PositionThumbCamera(GameObject camGO, Camera cam, GameObject modelGO)
        {
            Bounds b = ComputeModelBounds(modelGO);
            Vector3 center = b.center;
            float height = Mathf.Max(0.1f, b.size.y);
            float dist = height * 1.65f;
            Vector3 dir = (Vector3.back + Vector3.right * 0.12f).normalized;
            camGO.transform.position = center + dir * dist + Vector3.up * (height * 0.05f);
            camGO.transform.LookAt(center);
            cam.fieldOfView = 42f;
            cam.nearClipPlane = 0.05f;
            cam.farClipPlane = Mathf.Max(200f, dist * 4f);
        }

        // ── Update: swipe tracking ────────────────────────────────────────────
        void Update()
        {
            // Start thumb rendering if Hook was assigned after Start() (shouldn't happen, but safe)
            if (!_thumbsStarted && Hook != null)
            { _thumbsStarted = true; StartCoroutine(RenderThumbs()); }

            if (Hook != null)
            {
                var storeHook = Hook as CNRSkinStoreHook;
                if (storeHook != null)
                {
                    storeHook.GuardBackButtons();
                    storeHook.HideNativeSkinNavButtons();
                }
                var profileHook = Hook as CNRProfileHook;
                if (profileHook != null)
                {
                    profileHook.GuardBackButtons();
                    profileHook.HideNativeSkinNavButtons();
                }
            ...
            {
                // Only start swipe tracking if the touch is inside the content rect.
                // Convert screen Y (0=bottom) to GUI Y (0=top) for the test.
                float guiY = Screen.height - Input.mousePosition.y;
                var pos2d = new Vector2(Input.mousePosition.x, guiY);
                if (!_lastContentRect.Contains(pos2d)) return;
            ...
            {
                float dy = Input.mousePosition.y - _swipePrevScreenY;
                _swipePrevScreenY = Input.mousePosition.y;
                if (!_swipeDragging &&
                    Mathf.Abs(Input.mousePosition.y - _swipeStartScreenY) > SWIPE_DEAD_PX)
                    _swipeDragging = true;
                if (_swipeDragging)
                {
                    // screen Y increases upward; GUI Y increases downward.
                    // To scroll DOWN (reveal content below), drag finger UP → dy > 0 → scroll.y increases.
                    // GUI pixels == screen pixels in Unity IMGUI, so no scale needed.
                    _scroll.y = Mathf.Max(0f, _scroll.y + dy);
                }
            }
            else if (Input.GetMouseButtonUp(0))
            {
                _swipeActive = false;
                // _swipeDragging cleared in OnGUI after consuming the MouseUp
            }
        }

        // ── Rendered thumbnail coroutine ──────────────────────────────────────
        System.Collections.IEnumerator RenderThumbs()
        {
            // Wait for Hook and model to be ready
            yield return new WaitForSeconds(1.2f);
            if (Hook == null) yield break;

            var modelGO = Hook.RoleModel;
            if (modelGO == null) { ModEntry.Log("CNRSkinGridUI: RoleModel is null, skipping thumbs"); yield break; }

            // The store opens on the weapons tab; roleModel starts inactive.
            // Activate it for rendering and restore afterwards.
            bool wasActive = modelGO.activeSelf;
            if (!wasActive) { modelGO.SetActive(true); yield return null; } // let renderers register
            Transform thumbRoot = ThumbRoot(modelGO);
            Quaternion origRootRot = thumbRoot.rotation;
            Quaternion origModelLocalRot = modelGO.transform.localRotation;
            ResetThumbPose(modelGO);
            yield return null;

            int total = Hook.Total;
            _thumbTex = new Texture2D[total];

            // Compute model world bounds from all renderers in the hierarchy
            var renderers = modelGO.GetComponentsInChildren<Renderer>();
            if (renderers.Length == 0)
            {
                thumbRoot.rotation = origRootRot;
                modelGO.transform.localRotation = origModelLocalRot;
                _thumbsReady = true;
                if (!wasActive) modelGO.SetActive(false);
                yield break;
            }

            // Only render layers that the model actually occupies
            int layerMask = 0;
            foreach (var r in renderers) layerMask |= (1 << r.gameObject.layer);

            // Create offscreen camera
            var camGO = new GameObject("_CNRThumbCam");
            ...
            cam.enabled       = false; // render manually only

            PositionThumbCamera(camGO, cam, modelGO);

            // RenderTexture – portrait aspect to match thumb rect (cellW × cellW*1.3)
            var rt = new RenderTexture(128, 166, 16, RenderTextureFormat.ARGB32);
            rt.antiAliasing = 2;
            cam.targetTexture = rt;
            cam.aspect        = 128f / 166f;

            int origIdx = Hook.EquippedListIdx;

            for (int i = 0; i < total; i++)
            {
                Hook.NavigateTo(i);
                yield return null;  // let SetSkinData apply to renderers
                yield return null;  // extra frame for safety
                Hook.NavigateTo(i); // re-apply in case vanilla nav arrows fired during the yield frames
                ResetThumbPose(modelGO);
                PositionThumbCamera(camGO, cam, modelGO);
                ...
                var tex = new Texture2D(128, 166, TextureFormat.RGB24, false);
                tex.filterMode = FilterMode.Bilinear;
                tex.ReadPixels(new Rect(0, 0, 128, 166), 0, 0);
                tex.Apply();
                RenderTexture.active = null;
                _thumbTex[i] = tex;
            }

            // Restore the originally-shown skin
            int restoreIdx = (origIdx >= 0 && origIdx < total) ? origIdx : 0;
            Hook.NavigateTo(restoreIdx);
            thumbRoot.rotation = origRootRot;
            modelGO.transform.localRotation = origModelLocalRot;

            // Do NOT restore model to inactive here — if the user opened the skin tab while
            // rendering was running, the game already called SetActive(true) on the model.
            // Forcing it back to false would hide the model. Let the game manage tab visibility.
        ...
        void OnGUI()
        {
            ...
            Rect r = Hook.SkinContentRect;
            _lastContentRect = r;  // cache for swipe guard in Update()
            GUI.BeginGroup(r);
            float gw = r.width, gh = r.height;
            switch (_view)
            {
                case View.Grid:   DrawGrid(gw, gh);   break;
                case View.Detail: DrawDetail(gw, gh); break;
            }
            GUI.EndGroup();
        }
        ...
        {
            // Suppress accidental tap at end of a swipe gesture
            if (_swipeDragging && Event.current.type == EventType.MouseDown)
            { _swipeIsScrolling = true; }
            ...
            _swipeIsScrolling = _swipeGuiFingerDown && (
                _swipeDragging ||
                Mathf.Abs(Event.current.mousePosition.y - _swipeGuiDownY) > SWIPE_GUI_DEAD);
            ...
            GUI.DrawTexture(new Rect(0, 0, sw, sh), _txBg);

            // Title bar
            float titleH = 65 * s;
            GUI.DrawTexture(new Rect(0, 0, sw, titleH), _txDark);
            string loadMsg = _thumbsReady ? "" : "  \u2026";
            GUI.Label(new Rect(0, 0, sw, titleH), "Skins" + loadMsg, _sTitle);

            // Grid layout – 5 columns, portrait model thumbnails (1.3:1)
            const int COLS = 5;
            float pad    = 8 * s;
            float cellW  = (sw - pad * (COLS + 1)) / COLS;
            float thumbH = cellW * 1.3f;
            float nameH  = 30 * s;
            float priceH = 24 * s;
            float cellH  = thumbH + nameH + priceH + pad * 1.5f;

            int   total    = Hook.Total;
            int   rows     = Mathf.CeilToInt((float)total / COLS);
            float contentH = rows * (cellH + pad) + pad;

            // On first draw, scroll so the equipped skin is vertically centred.
            if (!_scrollInitialized)
            {
                _scrollInitialized = true;
                int eqIdx = Hook.EquippedListIdx;
                if (eqIdx >= 0)
                {
                    int eqRow  = eqIdx / COLS;
                    float eqCy = pad + eqRow * (cellH + pad);
                    float viewH = sh - titleH;
                    _scroll.y = Mathf.Max(0f, eqCy - (viewH - cellH) * 0.5f);
                }
            }

            // Use invisible scrollbar styles so swipe is the only scroll mechanism
            _scroll = GUI.BeginScrollView(
                new Rect(0, titleH, sw, sh - titleH),
                _scroll, new Rect(0, 0, sw, contentH),
                GUIStyle.none, GUIStyle.none);

            int equipped = Hook.EquippedListIdx;
            for (int i = 0; i < total; i++)
            {
                int   col   = i % COLS;
                int   row   = i / COLS;
                float cx    = pad + col * (cellW + pad);
                float cy    = pad + row * (cellH + pad);
                bool  isEq  = (i == equipped);
                bool  owned = Hook.GetOwned(i);
                var   style = isEq ? _sCellEquipped : (owned ? _sCellOwned : _sCell);

                bool pressed = GUI.Button(new Rect(cx, cy, cellW, cellH), GUIContent.none, style);
                if (pressed && !_swipeIsScrolling)
                { GUI.EndScrollView(); OpenDetail(i); return; }

                // Thumbnail: each cell becomes available independently as RenderThumbs() fills it.
                Texture2D thumb = (_thumbTex != null && i < _thumbTex.Length) ? _thumbTex[i] : null;
                var tRect = new Rect(cx + 2 * s, cy + 2 * s, cellW - 4 * s, thumbH - 2 * s);
                if (thumb != null)
                    GUI.DrawTexture(tRect, thumb, ScaleMode.ScaleToFit);
                else
                    DrawSpinner(tRect);

                string dname = Hook.GetDisplayName(i);
                GUI.Label(new Rect(cx, cy + thumbH + 2 * s, cellW, nameH),
                          dname, _sName);

                string status;
                if      (isEq)  status = "\u2713 Equipped";
                else if (owned) status = "Owned";
                else
                {
                    int p = Hook.GetPrice(i);
                    status = p > 0 ? p + " coins" : "Free";
                }
                GUI.Label(new Rect(cx, cy + thumbH + nameH + 2 * s, cellW, priceH), status, _sPrice);
            }
            GUI.EndScrollView();
        ...
        void OpenDetail(int listIdx)
        {
            _detailListIdx = listIdx;
            _modelYaw      = 0f;
            _dragging      = false;
        ...
        // ── Model rotation ────────────────────────────────────────────────────
        void HandleRotation(float top, float bot, float sw)
        {
            var e = Event.current;
            if (e == null) return;
            var area = new Rect(0, top, sw, bot - top);

            if (e.type == EventType.MouseDown && e.button == 0 && area.Contains(e.mousePosition))
            { _lastDrag = e.mousePosition; _dragging = true; e.Use(); }
            else if (e.type == EventType.MouseDrag && _dragging && _view == View.Detail)
            {
                _modelYaw += (e.mousePosition.x - _lastDrag.x) * 0.5f;
                _lastDrag   = e.mousePosition;
                var go = Hook.RoleModel;
                if (go != null)
                {
                    var root = go.transform.parent != null ? go.transform.parent : go.transform;
                    root.rotation = Quaternion.Euler(0f, _modelYaw, 0f);
                }
                e.Use();
            }
        ...
        void DrawSpinner(Rect r)
        {
            GUI.DrawTexture(r, _txDark ?? Texture2D.blackTexture);
            if (_txWhite == null) return;

            Vector2 center = new Vector2(r.x + r.width * 0.5f, r.y + r.height * 0.5f);
            float radius = Mathf.Min(r.width, r.height) * 0.18f;
            float dot = Mathf.Max(2f, Mathf.Min(r.width, r.height) * 0.035f);
            float phase = Time.realtimeSinceStartup * 8f;
            Color prev = GUI.color;
            for (int j = 0; j < 12; j++)
            {
                float a = ((float)j / 12f) * Mathf.PI * 2f + phase;
                float alpha = 0.20f + 0.80f * ((float)j / 11f);
                GUI.color = new Color(1f, 1f, 1f, alpha);
                float x = center.x + Mathf.Cos(a) * radius - dot * 0.5f;
                float y = center.y + Mathf.Sin(a) * radius - dot * 0.5f;
                GUI.DrawTexture(new Rect(x, y, dot, dot), _txWhite);
            }
            GUI.color = prev;
        }
        ...
        void BuildStyles()
        {
            ...
            _txBg          = MakeTex(new Color(0.05f, 0.05f, 0.12f, 0.98f));
            _txDark        = MakeTex(new Color(0.03f, 0.03f, 0.08f, 0.93f));
            _txBlue        = MakeTex(new Color(0.14f, 0.14f, 0.40f, 0.95f));
            _txGray        = MakeTex(new Color(0.18f, 0.18f, 0.22f, 0.95f));
            _txWhite       = MakeTex(Color.white);
            _txCellEquipped = MakeTex(new Color(0.08f, 0.32f, 0.12f, 0.98f));

            _sCell = new GUIStyle();
            _sCell.normal.background = _txGray;
            _sCell.border = new RectOffset(2, 2, 2, 2);

            _sCellEquipped = new GUIStyle(_sCell);
            _sCellEquipped.normal.background = _txCellEquipped;

            _sCellOwned = new GUIStyle(_sCell);
            _sCellOwned.normal.background = MakeTex(new Color(0.10f, 0.18f, 0.32f, 0.95f));

            _sName = new GUIStyle();
            _sName.alignment = TextAnchor.MiddleCenter;
            _sName.fontSize  = Mathf.Max(16, (int)(18 * s));
            _sName.normal.textColor = Color.white;
            _sName.wordWrap = false;
            _sName.clipping = TextClipping.Clip;

            _sPrice = new GUIStyle(_sName);
            _sPrice.fontSize         = Mathf.Max(14, (int)(16 * s));
            _sPrice.normal.textColor = new Color(1f, 0.85f, 0.3f);

            _sBtn = new GUIStyle();
            _sBtn.normal.background = _txBlue;
            _sBtn.hover.background  = MakeTex(new Color(0.22f, 0.22f, 0.55f, 1f));
            _sBtn.normal.textColor  = Color.white;
            _sBtn.alignment         = TextAnchor.MiddleCenter;
            _sBtn.fontSize          = Mathf.Max(16, (int)(20 * s));
            _sBtn.border            = new RectOffset(4, 4, 4, 4);

            _sTitle = new GUIStyle();
            _sTitle.alignment = TextAnchor.MiddleCenter;
            _sTitle.fontSize  = Mathf.Max(20, (int)(26 * s));
            _sTitle.fontStyle = FontStyle.Bold;
            _sTitle.normal.textColor = Color.white;

            _sHint = new GUIStyle(_sName);
            _sHint.normal.textColor = new Color(1f, 1f, 1f, 0.35f);
            _sHint.fontSize = Mathf.Max(14, (int)(16 * s));
    ...
    public class CNRRemoteSkinRenderer : MonoBehaviour
    {
        ...
        static readonly Dictionary<int, Texture2D> _baseSkinCache = new Dictionary<int, Texture2D>();
        static readonly Dictionary<int, Texture2D> _overlaySkinCache = new Dictionary<int, Texture2D>();
        ...
        bool ApplyToTransform(Transform root, Texture2D tex)
        {
            ...
            bool any = false;
            Texture2D baseTex = tex;
            if (tex != null && tex.width == 64 && tex.height == 64)
            {
                Texture2D cached;
                int key = tex.GetInstanceID();
                if (!_baseSkinCache.TryGetValue(key, out cached) || cached == null)
                {
                    cached = ContentManager.CropPlayerBase(tex);
                    if (cached != null) _baseSkinCache[key] = cached;
                }
                if (cached != null) baseTex = cached;
            }
            ...
            foreach (Component comp in renderers)
            {
                ...
                if (r == null || r.material == null) continue;
                if (IsUnderOverlayRoot(r.transform)) continue;
                string mname = r.material.name;
                int pi = mname.IndexOf(" (Instance)", StringComparison.OrdinalIgnoreCase);
                if (pi >= 0) mname = mname.Substring(0, pi).Trim();
                ...
                {
                    r.material.mainTexture = baseTex;
                    any = true;
                }
            }
            EnsureOverlayClone(charRoot, tex);
            return any;
        }

        static bool IsUnderOverlayRoot(Transform t)
        {
            while (t != null)
            {
                if (t.name == "CNR_SkinOverlay") return true;
                t = t.parent;
            }
            return false;
        }

        void EnsureOverlayClone(Transform charRoot, Texture2D sourceTex)
        {
            try
            {
                if (charRoot == null || sourceTex == null || sourceTex.width != 64 || sourceTex.height != 64) return;

                Texture2D overlayTex;
                int key = sourceTex.GetInstanceID();
                if (!_overlaySkinCache.TryGetValue(key, out overlayTex) || overlayTex == null)
                {
                    overlayTex = ContentManager.CropPlayerOverlay(sourceTex);
                    if (overlayTex != null) _overlaySkinCache[key] = overlayTex;
                }
                if (overlayTex == null) return;

                Transform existing = charRoot.Find("CNR_SkinOverlay");
                GameObject clone = existing != null ? existing.gameObject : null;
                if (clone == null)
                {
                    clone = UnityEngine.Object.Instantiate(charRoot.gameObject) as GameObject;
                    if (clone == null) return;
                    clone.name = "CNR_SkinOverlay";
                    clone.transform.parent = charRoot;
                    clone.transform.localPosition = Vector3.zero;
                    clone.transform.localRotation = Quaternion.identity;
                    clone.transform.localScale = Vector3.one * 1.015f;
            ...
            {
                ModEntry.Log("CNRRemoteSkins: EnsureOverlayClone error: " + ex.Message);
            }
        }
    }
    ...
    public class EconomyHook : MonoBehaviour
    {
        ...
        private static readonly string _DlIconB64 =
            "iVBORw0KGgoAAAANSUhEUgAAAwEAAAK1CAYAAACQM+LCAAAAAXNSR0IArs4c6QAAAARzQklUCAgICHwIZIgAACAASURBVHic7L09" +
            "73U7sidU/p/TLzAJJAzSaBA3OBJSazIishuBOoCInE/SZMCXQCKdEUgEcJEQ0k1Ho0EToJaQzoir0b0gNNEEXOjuc55tgr3W2n6p" +
            "KlfZ5bW89q7fo/+z9162y2W7XK7y2wJwOBwOh8PhcDgcDofD4XA4HA6Hw+FwOBwOh8PhcDgcDofD4XA4HA6Hw+FwOBwOh8PhcDgc" +
            "DofD4XA4HA6Hw+FwOBwOh8PhcDgcDofD4XA4HA6Hw3EWghWhf/m//Jf/0U//8v/4nwAgRgB4/kUIIUCEuOcVI0SAkGcbYwwhhIhx" +
            "FrZ0NbcRIIQ9LASAGGOE8BVSCrGkHZIwADiYS6hCjBEChFe++7MQDk4iQIS4laXgPBY0d8IxPsJWLRhePAHElFEqrEpX1GF4EQgA" +
            "EAPS2iHg9VvkW3Na8gRZzCPs8XjEr68vupwxy6CmibAsCUMi1C1VpDvaB6epDXt2BIpfIixpozwMafMYY/j6erZ5LOs+5OlROcrp" +
            "9td9UpaqM5XpYhW8yUHE5Bctyy5/e+cOYUxW2DBc+o96irkOOMJinTLROTEWOnDvCwp+t68xImGErNaaR5Yu04OHxMjThUx6y3T7" +
            "s6eOynWcOF3BQ0gS4XzGBu0YqfLpeArtdIEvSx4H1w817bz+0r4TnuKpKEug65dNV40PsYwjK0tJG9MrjXSP+GjKCstTrOOEl7FB" +
            "y9iLegyvGHg6qayEon5fssqXRd6n6rJs4RGPsz/T1m9vOjRO8nBcxgBCfDxEPCUdKitLpnswvRkL2oWMJfbD8xki9zHVdXGLE1/5" +
            "xaQP/+Lf/Pvh3/lP/qvckigKNYS//q//029/+Ov/9SvCU9p3XgBiIv3Y7zys5K4cKrdOkD3LogSs/cnoVbSIPKuG3oiExexr3VuO" +
            "wJw4N+LjDOMRw+tL8rFJUXjFbdVn6SiU2dNtt0dohDNxm0kVtLGomBPUD4YZRFNmv2P5oEE7IF+zdi4btgyn05MRm4h1v0i+VL8j" +
            "EY6SjgdPpbyWvFZt2pBfOk95ONqunI7QQpxWUplblKjtbBwtYcRG27Sic/ErvdRgJZI/BInllPXdCOvbXFTheFD+fnX3dGQQAFEt" +
            "JYaqj6LZErKuTDsSkUmIWmkO6PgzSdu2xmY+MTw7rjARV1Mz2lsGnnnMUh9HOedKRYv4OADFb0r3RDxO1g1iXsaIPI+PLeCR0Ntm" +
            "yf61f/vfg7/z7/4H8e//x/9FNiv7PVcuKf7v//F3j//vn/9l+Olf/QvYeTj+Uv8xvBwDEogSq+wVxBjYI+bJ63wk/gfXWJUhl4Yh" +
            "JAEHvX64LVHKQI02aAmC3IQ4yB68wslJ7JIQNpkAECIq6sftGEdnyJAxNH4pNFnGi9N0fhqbOPAz8KF2tRKe1XqHgk6fKsPUdQwI" +
            "2YB5LelSIHJCar88xeJGKEBkj6gA93rX4xwkNlIZtAIhFMx2vT8apO5b6sbpQPpzXNb+dwykDoDq3LjsLTyoKLhdPha+qOyG+Glk" +
            "RAuwhr4O1pqWAJTOG5wINOLc188qAMCdqU1FvKL6P+xJ46lm/wprwk5bSwR0+0GScX5fkTw7CYib2H4or/BRFWTBDtdXE2wdESqu" +
            "fW7BVBbARqxTSAsc9hYQvZ0dysaTiu8spSFmBaquhw5pFY4fRjr1ir/u438LieGvHHzUd5FZHom5qE/Vf8vA0fuiNyv787gget9a" +
            "BF6090xlpLZMI5MnTf0v96PErtnA+23Vdz7orthdclqk+PjC6BOv+5ItTDETP4E//e5B0XlGLCIXG+j3pf1V8ArddZR7+Ap7BZoI" +
            "UB84in0V9UV75BBOo/1WAD5+DfDxR8e/3/jk2/Dxb36zfvqD7/3wv3/+2Z+9+c+f/Pv//vzzN5eTgPqrL3/yj3/9l3//iy+++PNX" +
            "H33tLwBGQHHQiygmhos0HYJUoiTuqO7u7jpv690gVln/DCJLw1WabHNVUPWtkErHS8Fp0RuAtqsKW6+UW/heo4EJBfl3WkkuY3Cl" +
            "FxRLkykOnAqlSW4UeYv50Q3FHUm2ov3PLQfjl1lMQ3JtdGKCuI81QV1fabzfZa9IUICz8uw09FVBi9mhFkWFsTP2S4+8Bj3ldmv5" +
            "2LjH80IPTi/rmahUgFQl3H8OQq5+r4wXN1r8wBErStv2IcCwGSNRGtMK7m/TVeNRlT/HQ99PfcCz8MndHetLlfg05cZyteg88NzP" +
            "v5TSJj+6dxlNRgDk+XFVfsgeqQOak7vd9PUtpbLW44SUIi4jM2GPCPyVab+sNhZIGH7SooUHtb2d2FU9+CL22EFX2MaMWJ6J81Rq" +
            "khOH2M//M6xCs7VKdjWKMP7SigFjl8TOaCOTtM+oVQTlRn3puOFaJ7HysEejq1ZeEPFWVOg7/946nceswhTMcTVuh2uj+I1iLzoZ" +
            "WmKhbAL6+cxBPA+F2OXA1ewjALx+BfD69fH3G5/8Tv3ab33rX+B2+9HX3v7s8//6+edvXn3y7ulyEvDZZ5//27e/852//fo3fvsP" +
            "/vi7f3Ur5erHPD7AB/gAH+ADfIAP8AE+wAf4ADvgn3/wvb8rcPvR0y/+74df/M9/fPHTn/707Td//E/3/wccVZdRZBR5ggAAAABJ" +
            "RU5ErkJggg==";
        private Texture2D   _texSettingsIcon = null;
        private Texture2D   _texMailIcon     = null;
        private Texture2D   _texDlIcon       = null;
        // Set to true/false by CNRModManager via reflection — no reflection needed here.
        public  static bool  ModManagerOpen  = false;
        // Set to true/false by CNRSettingsMod when its settings panel opens/closes.
        // Distinct from ModManagerOpen so EcoHook can tell the two apart.
        public  static bool  SettingsModOpen = false;
        ...
        private void EcoPatchMainMenu()
        {
            ...
            if (_texDlIcon       == null) _texDlIcon       = LoadB64Icon(_DlIconB64);
            // Cache NGUI camera
        ...
        private void OnGUI()
        {
            ...
            // Actual mod manager (external): return early BEFORE ScaleAroundPivot so the
            // GUI matrix is not modified for the mod manager's own IMGUI rendering.
            if (ModManagerIsOpen() && !SettingsModOpen) return;

            float sc = Screen.width / ECO_REF_W;
            GUIUtility.ScaleAroundPivot(new Vector2(sc, sc), Vector2.zero);

            // When an overlay is active (settings or our own overlays), consume all non-rendering
            // events so buttons stay visual-only (no tap-through). CNRSettingsMod draws at
            // GUI.depth=-100 so it visually covers the buttons even though they are still drawn.
            // Note: only consume events for background EcoHook buttons — NOT when an overlay is
            // active, because the overlay's own buttons need to receive those events.
            bool _ecoOverlayActive = SettingsModOpen
                || _showMpDialog
                || (_ecoPatched && (_showEcoMail || _showEcoAccount || _showDownloads));
            bool _ecoBackgroundBlocked = SettingsModOpen
                || (_ecoPatched && (_showEcoMail || _showEcoAccount || _showDownloads));
            if (_ecoBackgroundBlocked
                && Event.current.type != EventType.Repaint
                && Event.current.type != EventType.Layout)
                Event.current.Use();

            if (!_ecoPatched && !_showMpDialog) return;
            ...
            if (_texDlIcon != null)
            {
                GUIStyle iconSt2 = new GUIStyle();
                iconSt2.imagePosition      = ImagePosition.ImageOnly;
                iconSt2.normal.background  = null;
                iconSt2.hover.background   = EcoMkTex(2, 2, new Color(1f, 1f, 1f, 0.2f));
                iconSt2.active.background  = EcoMkTex(2, 2, new Color(1f, 1f, 1f, 0.4f));
                iconSt2.border  = new RectOffset(0, 0, 0, 0);
                iconSt2.padding = new RectOffset(0, 0, 0, 0);
                if (dlMissing > 0) GUI.color = new Color(0.4f, 0.88f, 1f);
                if (GUI.Button(new Rect(dlX, dlY, dlW, dlH),
                               new GUIContent(_texDlIcon), iconSt2)) EcoOpenDownloads();
                GUI.color = Color.white;
                if (dlMissing > 0)
                {
                    GUIStyle badgeSt2 = new GUIStyle(GUI.skin.label);
                    badgeSt2.fontSize  = 11;
                    badgeSt2.fontStyle = FontStyle.Bold;
                    badgeSt2.normal.textColor = new Color(0.4f, 0.88f, 1f);
                    GUI.Label(new Rect(dlX + dlW - 14f, dlY - 4f, 22f, 18f),
                              dlMissing.ToString(), badgeSt2);
                }
            }
            else
            {
            ...
            // Draw overlay on top of buttons (events already consumed above for overlay frames)
            if (_showMpDialog)      { EcoDrawMpRequiredDialog(); }
            else if (_showEcoMail)    { EcoDrawMailOverlay(); }
            else if (_showEcoAccount) { EcoDrawAccountOverlay(); }
            else if (_showDownloads)  { EcoDrawDownloadsOverlay(); }
        }
        ...
        private void EcoDrawMpRequiredDialog()
        {
            float vw = ECO_REF_W;
            float vh = Screen.height / (Screen.width / ECO_REF_W);
            // Fullscreen button swallows any taps on background elements
            GUI.Button(new Rect(0, 0, vw, vh), GUIContent.none, GUIStyle.none);
            // Dim background
            GUI.color = new Color(0f, 0f, 0f, 0.88f);
            GUI.DrawTexture(new Rect(0, 0, vw, vh), Texture2D.whiteTexture);
    ...
    //
    //  Design:
    //  - Room is created with mode=Stronghold (1) so the game loads a
    //    Stronghold-capable map and spawns the Stronghold zone prefabs
    //    (Stronghold_1 = Cop base/flag, Stronghold_2 = Robber base/flag).
    //  - A CTF marker is broadcast to all clients via CNR event 199 {ctf:"1"}.
    //    Custom room properties are avoided as they can trigger SendMonoMessage.
    //  - CtfHook in the game scene disables the vanilla Stronghold MonoBehaviour
    //    on each zone and attaches CtfZone instead, hijacking the trigger collider
    //    and flag visuals already present on the spawned prefab.
    //  - State (score, carrier, drops) is broadcast every 0.5 s by the serverId
    //    player (lowest Photon actor ID, same authority model as vanilla Stronghold).
    //  - A CTF checkbox is injected into the multiplayer map-selection screen,
    //    sitting beneath the existing SH checkbox, visible only when the selected
    //    map supports Stronghold.
    // ============================================================================

    // Shared state -- written by CtfHook/CnrEventBus, read by CtfHook.
    public static class CtfMode
    {
        // Set by CtfCheckBoxBehaviour.OnClick() before room creation.
        // Cleared on left room; also drives immediate IsCtfRoom for the creator.
        public static bool    PendingCtf       = false;
        // Set on all clients via CNR event 199 {ctf:"1"} or {ctf_s:"..."}.
        public static bool    IsCtfRoom        = false;

        // Flag carrier strings:  "" = at base,  "!" = dropped,  else = mId of carrier.
        public const  string  AT_BASE          = "";
        public const  string  DROPPED          = "!";

        public static string  CopFlagStatus    = AT_BASE;
        ...
        static void ParseVec(string s, ref Vector3 v)
        {
            string[] c = s.Split(',');
            if (c.Length == 3)
            {
                float x, y, z;
                if (float.TryParse(c[0], out x) && float.TryParse(c[1], out y) && float.TryParse(c[2], out z))
                    v = new Vector3(x, y, z);
            }
        }
    ...
    // Shared state for the integrated zombie mode.
    public static class ZombieMode
    {
        public static bool PendingZombie = false;
        public static bool IsZombieRoom   = false;
    ...
    // Drives one owned mode button (SH or CTF).
    // Does NOT touch SetActive on itself -- CtfCheckBoxInjector controls visibility
    // from its own persistent Update() so the button can re-appear after being hidden.
    public class CnrModeButton : MonoBehaviour
    {
        private bool              _isCTF;
        private bool              _isZombie;
        private GrowthGameModeTag _modeTag;
        private UISprite          _checkSprite;
        private UILabel           _lbl;
        private string            _labelText;

        public void Init(bool isCTF, bool isZombie, GrowthGameModeTag modeTag, string labelText, UISprite checkSprite, UILabel label)
        {
            _isCTF       = isCTF;
            _isZombie    = isZombie;
            _modeTag     = modeTag;
        ...
        void Update()
        {
            // Re-assert label every frame (belt-and-suspenders vs any surviving localiser).
            if (_lbl != null && _lbl.text != _labelText) _lbl.text = _labelText;

            if (_checkSprite == null) return;
            var msd = MultiplayerSelectDirector.mInstance;
            if (msd == null) return;

            bool isChecked;
            if (_isCTF)
                isChecked = CtfMode.PendingCtf || CtfMode.IsCtfRoom || (msd.curModeSet == _modeTag && !CtfMode.PendingCtf);
            else if (_isZombie)
                isChecked = ZombieMode.PendingZombie || ZombieMode.IsZombieRoom;
            else
                isChecked = (msd.curModeSet == _modeTag);
            _checkSprite.alpha = isChecked ? 1f : 0f;
        }
        ...
        void OnClick()
        {
            var msd = MultiplayerSelectDirector.mInstance;
            if (msd == null) return;
            if (_isCTF) CtfMode.PendingCtf = true;
            if (_isZombie) ZombieMode.PendingZombie = true;
            msd.SwitchToMode(_modeTag);
        }
    }

    public static class ModeSelectUiUtil
    {
        ...
        public static void DestroyAllInChildren<T>(Transform t) where T : Component
        {
            T comp = t.GetComponent<T>();
    ...
    // Spawned once on a persistent GO when the "MultiplayerSelect" scene loads.
    // Hides the original SH checkbox and takes its place with two owned buttons.
    // The original stays permanently hidden so ModeSelectCheckBox.Update() never runs.
    public class CtfCheckBoxInjector : MonoBehaviour
    {
        public static GameObject SharedShOriginal;
        public static GameObject SharedShButton;
        public static GameObject SharedCtfButton;
        private bool       _injected   = false;
        private GameObject _shOriginal;
        private GameObject _shBtn;
        private GameObject _ctfBtn;
        ...
        void Update()
        {
            ...
            // Force original hidden every frame -- WWResetModeCheckBox() re-enables it on map change.
            if (_shOriginal != null && _shOriginal.activeSelf) _shOriginal.SetActive(false);

            // Control our buttons' visibility from here, not from their own Update().
            // A deactivated MonoBehaviour cannot call SetActive(true) on itself.
            bool mapOk = msd.isCurMapSupportThisMode(GrowthGameModeTag.tStronghold);
            if (_shBtn  != null && _shBtn.activeSelf  != mapOk) _shBtn.SetActive(mapOk);
            if (_ctfBtn != null && _ctfBtn.activeSelf != mapOk) _ctfBtn.SetActive(mapOk);
        }

        void LateUpdate()
        {
            if (!_injected) return;
            // Auto-clear PendingCtf when user switches to any non-Stronghold mode (KC, TDM, etc.)
            var msd = MultiplayerSelectDirector.mInstance;
            if (msd != null && CtfMode.PendingCtf && msd.curModeSet != GrowthGameModeTag.tStronghold)
        ...
        private GameObject SpawnButton(GameObject source, bool isCTF, string labelText, Vector3 localPos)
        {
            ...
            GameObject go = (GameObject)UnityEngine.Object.Instantiate(source);
            go.name                    = isCTF ? "CnrBtn_CTF" : "CnrBtn_SH";
            ...
            btn.Init(isCTF, false, GrowthGameModeTag.tStronghold, labelText, checkSprite, lbl);

            // Start hidden; CtfCheckBoxInjector.Update() controls visibility.
            go.SetActive(false);

            ModEntry.Log("CtfCheckBoxInjector: spawned " + go.name
        ...
        private bool TryInject()
        {
            ...
            _shOriginal = msd.mModeCheckBoxSH;
            if (_shOriginal == null) return false;
            SharedShOriginal = _shOriginal;

            GameObject tdmGo = msd.mModeCheckBoxTDM;
            Vector3 shPos = _shOriginal.transform.localPosition;
            float step = (tdmGo != null)
                ? Mathf.Abs(tdmGo.transform.localPosition.y - shPos.y)
                : 0.2f;
            if (step < 0.001f) step = 0.2f;

            _shOriginal.SetActive(false);

            _shBtn  = SpawnButton(_shOriginal, false, "Stronghold",        shPos);
            _ctfBtn = SpawnButton(_shOriginal, true,  "Capture the Flag",  new Vector3(shPos.x, shPos.y - step, shPos.z));
            SharedShButton  = _shBtn;
            SharedCtfButton = _ctfBtn;

            return true;
        }
    }

    // Adds a third multiplayer-select button for Zombies, using Killing Competition
    // as the underlying room mode shell.
    public class ZombieCheckBoxInjector : MonoBehaviour
    {
        private bool       _injected = false;
        private GameObject _source;
        private GameObject _zombieBtn;
        private UISprite   _vanillaKcCheckSprite;
        ...
        void Update()
        {
            ...
            {
                try   { _injected = TryInject(); }
                catch (Exception ex) { ModEntry.Log("ZombieCheckBoxInjector: " + ex.Message); _injected = true; }
                return;
            }
            ...
            bool mapOk = msd.isCurMapSupportThisMode(GrowthGameModeTag.tKillingCompetition);
            if (_zombieBtn != null && _zombieBtn.activeSelf != mapOk) _zombieBtn.SetActive(mapOk);

            bool zombieActive = ZombieMode.PendingZombie || ZombieMode.IsZombieRoom;
            if (_vanillaKcCheckSprite != null)
                _vanillaKcCheckSprite.alpha = zombieActive ? 0f : _vanillaKcCheckSprite.alpha;
        }
        ...
        void LateUpdate()
        {
            if (!_injected) return;
            var msd = MultiplayerSelectDirector.mInstance;
            if (msd != null && ZombieMode.PendingZombie && msd.curModeSet != GrowthGameModeTag.tKillingCompetition)
                ZombieMode.PendingZombie = false;
        }

        private GameObject SpawnZombieButton(GameObject source, string labelText, Vector3 localPos)
        {
            bool wasActive = source.activeSelf;
            if (!wasActive) source.SetActive(true);

            GameObject go = (GameObject)UnityEngine.Object.Instantiate(source);
            go.name                    = "CnrBtn_Zombie";
            go.transform.parent        = source.transform.parent;
            go.transform.localScale    = source.transform.localScale;
            go.transform.localPosition = localPos;
            ...
            MonoBehaviour msb = go.GetComponent("ModeSelectCheckBox") as MonoBehaviour;
            if (msb != null) DestroyImmediate(msb);
            ModeSelectUiUtil.DestroyAllInChildren<UILocalize>(go.transform);

            UICheckbox cb          = go.GetComponent<UICheckbox>();
            UISprite   checkSprite = (cb != null) ? cb.checkSprite : null;
            if (cb != null) DestroyImmediate(cb);

            UILabel lbl = ModeSelectUiUtil.FindInChildren<UILabel>(go.transform);
            if (lbl != null) lbl.text = labelText;

            CnrModeButton btn = go.AddComponent<CnrModeButton>();
            btn.Init(false, true, GrowthGameModeTag.tKillingCompetition, labelText, checkSprite, lbl);

            go.SetActive(false);
            ModEntry.Log("ZombieCheckBoxInjector: spawned " + go.name
                + " label=" + (lbl != null ? lbl.text : "null")
                + " sprite=" + (checkSprite != null ? checkSprite.name : "null"));
            return go;
        }
        ...
        private bool TryInject()
        {
            ...
            _source = CtfCheckBoxInjector.SharedShOriginal != null ? CtfCheckBoxInjector.SharedShOriginal : msd.mModeCheckBoxSH;
            if (_source == null) return false;
            UICheckbox vanillaCb = msd.mModeCheckBoxTDM != null ? msd.mModeCheckBoxTDM.GetComponent<UICheckbox>() : null;
            _vanillaKcCheckSprite = vanillaCb != null ? vanillaCb.checkSprite : null;

            Vector3 shPos = _source.transform.localPosition;
            GameObject ctfBtn = CtfCheckBoxInjector.SharedCtfButton;
            GameObject shBtn = CtfCheckBoxInjector.SharedShButton;
            float step = 0.2f;
            if (shBtn != null && ctfBtn != null)
                step = Mathf.Abs(ctfBtn.transform.localPosition.y - shBtn.transform.localPosition.y);
            else
            {
                GameObject tdmGo = msd.mModeCheckBoxTDM;
                if (tdmGo != null)
                    step = Mathf.Abs(tdmGo.transform.localPosition.y - shPos.y);
            }
            if (step < 0.001f) step = 0.2f;

            float y = (ctfBtn != null && ctfBtn.activeSelf) ? ctfBtn.transform.localPosition.y - step : shPos.y - step;
            Vector3 zombiePos = new Vector3(shPos.x, y, shPos.z);
            _zombieBtn = SpawnZombieButton(_source, "Zombies", zombiePos);
            return true;
        }
    }

    // Spawned in every game scene.  Self-destructs unless:
    //   - CNRMultiplayerManager initialises within 10 s,
    //   - room mode is Stronghold, and
    //   - CTF confirmation arrives within 8 s (ctf event or PendingCtf for creator).
    //
    // When confirmed:
    //   - Disables vanilla Stronghold MB on Stronghold_1/2.
    //   - Attaches CtfZone trigger to each.
    //   - The serverId player (lowest actor ID) auto-returns dropped flags and
    //     periodically broadcasts full state so late joiners sync up.
    //
    // Flag ownership:
    //   Zone 1 (Stronghold_1) = Cop base.
    //     Robbers steal the cop flag here; cops deliver the robber flag here to score.
    //   Zone 2 (Stronghold_2) = Robber base.
    //     Cops steal the robber flag here; robbers deliver the cop flag here to score.
    public class CtfHook : MonoBehaviour
    {
        private GameObject _zone1Go;
        ...
        IEnumerator Init()
        {
            // Wait for CNRMultiplayerManager.
            float timeout = 10f;
            while (CNRMultiplayerManager.mInstance == null && timeout > 0f)
            { timeout -= Time.deltaTime; yield return null; }
            if (CNRMultiplayerManager.mInstance == null)
            { ModEntry.Log("CtfHook: manager timeout"); Destroy(gameObject); yield break; }

            // Only active in Stronghold mode rooms (CTF piggybacks on Stronghold).
            if (CNRMultiplayerManager.mInstance.myModeInfo.mCurMode != GrowthGameModeTag.tStronghold)
            ...
            // One extra frame so StrongholdModeLogic.Start() places zone GOs.
            yield return null;

            _zone1Go = GameObject.Find("Stronghold_1");
            _zone2Go = GameObject.Find("Stronghold_2");
            if (_zone1Go == null || _zone2Go == null)
            { ModEntry.Log("CtfHook: zone GOs not found"); Destroy(gameObject); yield break; }

            _zone1Base = _zone1Go.transform.position;
            _zone2Base = _zone2Go.transform.position;

            // Disable vanilla Stronghold so it stops fighting with our logic / sending RPCs.
            MonoBehaviour sh1 = _zone1Go.GetComponent("Stronghold") as MonoBehaviour;
            MonoBehaviour sh2 = _zone2Go.GetComponent("Stronghold") as MonoBehaviour;
        ...
        void Update()
        {
            ...
            if (myInfo != null)
            {
                PlayerStatus cur = myInfo.mStatus;
            ...
            _bcastTimer += Time.deltaTime;
            if (_bcastTimer >= BroadcastSecs) { _bcastTimer = 0f; BroadcastState(); }
        }

        void OnLocalPlayerDied(PlayerInfo info)
        {
            bool changed = false;
            if (CtfMode.CopFlagStatus == _myId)
        ...
        // because the "Player" tag is only on the local character.
        public void OnZoneEnter(int flagId, Collider other)
        {
            if (!_inited || _gameOver) return;
            if (((Component)other).gameObject.tag != "Player") return;

            PlayerInfo myInfo = CNRMultiplayerManager.mInstance != null
                ? CNRMultiplayerManager.mInstance.myPlayerInfo : null;
    ...
    // Attached to Stronghold_1 and Stronghold_2 by CtfHook.Init();
    // forwards Unity trigger events into CtfHook.
    public class CtfZone : MonoBehaviour
    {
    ...
    public class SpeedHook : MonoBehaviour
    {
        ...
        private const float MeleeSpeedMult  = 1.20f;
