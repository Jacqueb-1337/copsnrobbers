...
namespace CNRMods
{
    ...
    public class ModEntry
    {
        ...
        public const  string Version = "3.2.23";
    ...
    public class ContentManager : MonoBehaviour
    {
        ...
        static Texture2D NormalizePlayerSkin(Texture2D src)
        {
            ...
            var dst = new Texture2D(64, 64, TextureFormat.ARGB32, false);
            dst.filterMode = FilterMode.Point;
            dst.wrapMode   = TextureWrapMode.Clamp;
            Color32[] clear = new Color32[64 * 64];
            for (int i = 0; i < clear.Length; i++) clear[i] = new Color32(0, 0, 0, 0);
            dst.SetPixels32(clear);

            Color32[] srcPx = src.GetPixels32();
            int copyW = Mathf.Min(src.width, 64);
            int copyH = Mathf.Min(src.height, 64);
            for (int y = 0; y < copyH; y++)
            {
                for (int x = 0; x < copyW; x++)
                {
                    ...
                    int dstIdx = y * 64 + x;
                    if (srcIdx >= 0 && srcIdx < srcPx.Length && dstIdx >= 0 && dstIdx < clear.Length)
                        clear[dstIdx] = srcPx[srcIdx];
                }
            }
            dst.SetPixels32(clear);
        ...
        static void LoadSkinFile(string id, string path)
        {
            try
            {
                ...
                if (tex.LoadImage(bytes))
                {
                    tex = NormalizePlayerSkin(tex) ?? tex;
                    tex.name       = id;
    ...
    public class CNRRemoteSkinRenderer : MonoBehaviour
    {
        ...
        void EnsureOverlayClone(Transform charRoot, Texture2D tex)
        {
            try
            {
                if (charRoot == null) return;
                Transform existing = charRoot.Find("CNR_SkinOverlay");
                if (existing != null)
                {
                    foreach (Renderer r in existing.GetComponentsInChildren<Renderer>(true))
                    {
                        if (r != null && r.material != null) r.material.mainTexture = tex;
                    }
                    return;
                }

                GameObject clone = UnityEngine.Object.Instantiate(charRoot.gameObject) as GameObject;
                if (clone == null) return;
                clone.name = "CNR_SkinOverlay";
                clone.transform.parent = charRoot;
                clone.transform.localPosition = Vector3.zero;
                clone.transform.localRotation = Quaternion.identity;
                clone.transform.localScale = Vector3.one * 1.01f;
                ...
                foreach (MonoBehaviour mb in clone.GetComponentsInChildren<MonoBehaviour>(true))
                {
                    ...
                    if (n == "Animation" || n == "Animator") continue;
                ...
                {
                    if (r == null) continue;
                    if (r.material != null) r.material.mainTexture = tex;
                    r.material.renderQueue = 3001;
                }
            }
            catch (Exception ex)
            {
                ModEntry.Log("EnsureOverlayClone error: " + ex.Message);
            }
        }
    }
