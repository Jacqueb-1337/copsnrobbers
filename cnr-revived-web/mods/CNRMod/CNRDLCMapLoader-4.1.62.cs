    ...
    internal class CNRDLCMapLoader : MonoBehaviour
    {
        ...
        internal static bool PrepareFile(string path, out string reason)
        {
            ...
            try
            {
                ...
                if (map.version != LegacyFormatVersion && map.version != FormatVersion)
                {
                    reason = "DLC map format " + map.version + " is unsupported (expected " + LegacyFormatVersion + " or " + FormatVersion + ").";
                    return false;
                }
                ...
                if (map.version == FormatVersion && !HydratePackedMap(map, out reason))
                    return false;
        ...
        private static bool HydratePackedMap(CNRDLCMapFile map, out string reason)
        {
            ...
            for (int c = 0; c < map.chunks.Length; c++)
            {
                ...
                if (!DecodeCollisionBoxes(chunk.collisionBoxesPacked, out decoded, out reason)) return false;
                chunk.collision = decoded;
            }
            return true;
        }
        ...
        private static bool DecodePackedMesh(CNRDLCPackedBlob blob, string kind, out CNRDLCMeshData mesh, out string reason)
        {
            ...
            try
            {
                if (blob == null || string.IsNullOrEmpty(blob.dataBase64) ||
                    (blob.encoding != "cnrmesh-f32-u16-raw-v1" && blob.encoding != "cnrmesh-f32-u16-gzip-v1"))
                {
                ...
                byte[] raw = blob.encoding == "cnrmesh-f32-u16-raw-v1" ? packed : Gunzip(packed);
        ...
        private static bool DecodeCollisionBoxes(CNRDLCPackedBlob blob, out CNRDLCMeshData[] meshes, out string reason)
        {
            meshes = new CNRDLCMeshData[0];
            reason = "";
            try
            {
                if (blob == null || string.IsNullOrEmpty(blob.dataBase64)) return true;
                if (blob.encoding != "cnrboxes-f32-raw-v1" && blob.encoding != "cnrboxes-f32-gzip-v1")
                {
                    reason = "DLC map collision box encoding is invalid.";
                    return false;
                }
                byte[] packed = Convert.FromBase64String(blob.dataBase64);
                if (packed.Length == 0 || packed.Length > MaxPackedCompressedBytes)
                ...
                byte[] raw = blob.encoding == "cnrboxes-f32-raw-v1" ? packed : Gunzip(packed);
                if (raw.Length > MaxPackedInflatedBytes)
                {
                ...
                using (BinaryReader br = new BinaryReader(ms))
                {
                    ...
                    if (count < 0 || count > MaxCollisionBoxesPerChunk || (blob.count > 0 && blob.count != count) || raw.Length != 4 + count * 24)
                    {
                        reason = "DLC map packed collision count is invalid.";
                        return false;
                    }
                    float[][] boxes = new float[count][];
                    for (int i = 0; i < count; i++)
                    {
                        float[] b = new float[6];
                        for (int j = 0; j < 6; j++) b[j] = br.ReadSingle();
                    ...
                    meshes = BuildCollisionMeshesFromBoxes(boxes);
                }
                return true;
            }
            catch (Exception ex)
        ...
        private static byte[] Gunzip(byte[] compressed)
        {
            ...
            using (MemoryStream output = new MemoryStream())
            {
                ...
                while ((read = gz.Read(buffer, 0, buffer.Length)) > 0)
                {
                    ...
                    if (output.Length > MaxPackedInflatedBytes) throw new Exception("Packed DLC map blob expands beyond the safety limit.");
                }
                return output.ToArray();
            }
        }

        private static CNRDLCMeshData[] BuildCollisionMeshesFromBoxes(float[][] boxes)
        {
            if (boxes == null || boxes.Length == 0) return new CNRDLCMeshData[0];
            const int verticesPerBox = 24;
            const int indicesPerBox = 36;
            int boxesPerPart = Math.Max(1, MaxVerticesPerPart / verticesPerBox);
            int partCount = (boxes.Length + boxesPerPart - 1) / boxesPerPart;
            CNRDLCMeshData[] result = new CNRDLCMeshData[partCount];
            for (int p = 0; p < partCount; p++)
            {
                int start = p * boxesPerPart;
                int count = Math.Min(boxesPerPart, boxes.Length - start);
                CNRDLCMeshData data = new CNRDLCMeshData();
                data.vertices = new float[count * verticesPerBox * 3];
                data.uv = new float[0];
                data.triangles = new int[count * indicesPerBox];
                int vo = 0;
                int ti = 0;
                for (int i = 0; i < count; i++) AddCollisionBox(boxes[start + i], data.vertices, ref vo, data.triangles, ref ti);
                result[p] = data;
            }
            return result;
        }

        private static void AddCollisionBox(float[] b, float[] vertices, ref int vo, int[] triangles, ref int ti)
        {
            float x0=b[0], y0=b[1], z0=b[2], x1=b[3], y1=b[4], z1=b[5];
            AddCollisionFace(vertices, ref vo, triangles, ref ti, x0,y0,z0, x1,y0,z0, x1,y1,z0, x0,y1,z0);
            AddCollisionFace(vertices, ref vo, triangles, ref ti, x1,y0,z1, x0,y0,z1, x0,y1,z1, x1,y1,z1);
            AddCollisionFace(vertices, ref vo, triangles, ref ti, x0,y0,z1, x0,y0,z0, x0,y1,z0, x0,y1,z1);
            AddCollisionFace(vertices, ref vo, triangles, ref ti, x1,y0,z0, x1,y0,z1, x1,y1,z1, x1,y1,z0);
            AddCollisionFace(vertices, ref vo, triangles, ref ti, x0,y1,z0, x1,y1,z0, x1,y1,z1, x0,y1,z1);
            AddCollisionFace(vertices, ref vo, triangles, ref ti, x0,y0,z1, x1,y0,z1, x1,y0,z0, x0,y0,z0);
        }

        private static void AddCollisionFace(float[] vertices, ref int vo, int[] triangles, ref int ti,
            float ax,float ay,float az, float bx,float by,float bz, float cx,float cy,float cz, float dx,float dy,float dz)
        {
            int baseVertex = vo / 3;
            vertices[vo++]=ax; vertices[vo++]=ay; vertices[vo++]=az;
            vertices[vo++]=bx; vertices[vo++]=by; vertices[vo++]=bz;
            vertices[vo++]=cx; vertices[vo++]=cy; vertices[vo++]=cz;
            vertices[vo++]=dx; vertices[vo++]=dy; vertices[vo++]=dz;
            triangles[ti++]=baseVertex; triangles[ti++]=baseVertex+1; triangles[ti++]=baseVertex+2;
            triangles[ti++]=baseVertex; triangles[ti++]=baseVertex+2; triangles[ti++]=baseVertex+3;
        }
        ...
        private static void BuildMaterials()
        {
            ...
            if (cutoutShader == null) cutoutShader = Shader.Find("Unlit/Transparent Colored");
