...
namespace CNRMods
{
    ...
    public class EconomyHook : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            ...
            // When an overlay is active (ours or the settings mod), consume all non-rendering
            // events before buttons are drawn so buttons stay visual-only (no tap-through).
            // CNRSettingsMod draws at GUI.depth=-100 (on top) so it visually covers the buttons.
            bool _ecoOverlayActive = ModManagerIsOpen()
                || _showMpDialog
