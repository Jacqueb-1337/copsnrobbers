...
namespace CNRMods
{
    ...
    internal class CNRDLCMapLoader : MonoBehaviour
    {
        ...
        private static bool DecodePackedMesh(CNRDLCPackedBlob blob, string kind, out CNRDLCMeshData mesh, out string reason)
        {
            ...
            try
            {
                if (blob == null || blob.encoding != "cnrmesh-f32-u16-gzip-v1" || string.IsNullOrEmpty(blob.dataBase64))
                {
                    reason = "DLC map " + kind + " packed mesh encoding is invalid.";
                    return false;
                }
                byte[] compressed = Convert.FromBase64String(blob.dataBase64);
                if (compressed.Length == 0 || compressed.Length > MaxPackedCompressedBytes)
                {
        ...
        private static bool DecodeCollisionBoxes(CNRDLCPackedBlob blob, out CNRDLCMeshData[] meshes, out string reason)
        {
            ...
            try
            {
                ...
                if (blob.encoding != "cnrboxes-f32-gzip-v1")
                {
