...
namespace CNRMods
{
    ...
    public class CNRRoomCardMetadata : MonoBehaviour
    {
        ...
        private void SetCustomThumbnail(Texture2D tex, bool showVanillaWhenNull)
        {
            ...
            sprite.outer = new Rect(0f, 0f, tex.width, tex.height);
            sprite.inner = sprite.outer;
            ...
            // Preserve the image's aspect ratio inside the vanilla thumbnail slot.
            Vector3 scale = _originalMapSpriteScale;
            float slotW = Mathf.Abs(scale.x);
            float slotH = Mathf.Abs(scale.y);
            if (tex.width > 0 && tex.height > 0 && slotW > 0.01f && slotH > 0.01f)
            {
                ...
                if (imageAspect > slotAspect)
                    scale.y = Mathf.Sign(scale.y) * (slotW / imageAspect);
                else
                    scale.x = Mathf.Sign(scale.x) * (slotH * imageAspect);
            }
            Original.mapLogoSprite.transform.localScale = scale;
            Original.mapLogoSprite.enabled = true;
            Original.mapLogoSprite.MarkAsChanged();
        }
