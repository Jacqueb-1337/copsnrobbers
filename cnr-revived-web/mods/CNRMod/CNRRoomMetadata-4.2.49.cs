    ...
    internal static class CNRMatchMetadata
    {
        ...
        internal static void RepairHostRoomMetadata()
        {
            try
            {
                ...
                // Room creation freezes the host selections before entering Loading. Never
                // replace that frozen state with whatever the selection UI looks like now.
                MultiplayerSelectDirector msd = MultiplayerSelectDirector.mInstance;
                string canonicalMode = CNRMatchSettings.GetAuthoritativeRoomMode();
                if (string.IsNullOrEmpty(canonicalMode))
                    canonicalMode = CNRCompatibility.GetRoomProp(room, PropGameMode);
                if (string.IsNullOrEmpty(canonicalMode))
                    canonicalMode = GetSelectedGameMode(msd);
                if (string.IsNullOrEmpty(canonicalMode))
                    canonicalMode = CanonicalFromVanillaMode(CNRCompatibility.GetRoomProp(room, "mode"));

                string mapId = CNRCompatibility.GetRoomProp(room, PropMapId) ?? "";
                string mapName = CNRCompatibility.GetRoomProp(room, PropMapName) ?? "";
                string mapThumb = CNRCompatibility.GetRoomProp(room, PropMapThumb) ?? "";
                string advertisedMapType = CNRCompatibility.GetRoomProp(room, PropMapType) ?? "";
                bool hasCustomMap = !string.IsNullOrEmpty(mapId) || advertisedMapType == "dlc" || advertisedMapType == "legacy";
                if (!hasCustomMap)
                    hasCustomMap = TryGetSelectedCustomMap(out mapId, out mapName, out mapThumb);
                OfficialMapEntry official = hasCustomMap ? FindOfficialMap(mapId) : null;
                string activeDlcId = CNRDLCMapLoader.IsActive ? PlayerPrefs.GetString(CNRDLCMapLoader.PrefId, "") : "";
                bool selectedIsActiveDlc = CNRDLCMapLoader.IsActive && !string.IsNullOrEmpty(mapId)
                    && string.Equals(activeDlcId, mapId, StringComparison.OrdinalIgnoreCase);
                bool isDlc = hasCustomMap && (advertisedMapType == "dlc" || (official != null && official.IsDlcMap) || selectedIsActiveDlc);

                string roomManifestUrl = CNRCompatibility.GetRoomProp(room, CNRRoomResources.PropManifestUrl) ?? "";
                string roomManifestHash = CNRCompatibility.GetRoomProp(room, CNRRoomResources.PropManifestHash) ?? "";
                string roomInlineResources = CNRCompatibility.GetRoomProp(room, CNRRoomResources.PropInlineResources) ?? "";
                if (string.IsNullOrEmpty(roomManifestUrl)) roomManifestUrl = CNRRoomResources.HostManifestUrl ?? "";
                if (string.IsNullOrEmpty(roomManifestHash)) roomManifestHash = CNRRoomResources.HostManifestHash ?? "";
                if (string.IsNullOrEmpty(roomInlineResources)) roomInlineResources = CNRRoomResources.BuildInlineHostResources();
                ...
                props[CNRMatchSettings.PropSettings] = CNRMatchSettings.PackAuthoritativeRoom();
