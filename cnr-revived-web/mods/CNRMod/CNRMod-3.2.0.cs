...
namespace CNRMods
{
    ...
    public class CNRProfileHook : MonoBehaviour, ICNRSkinHook
    {
        ...
        GameObject ICNRSkinHook.SkinTabMarker       { get { return null; } } // profile = always show
        GameObject ICNRSkinHook.RoleModel
        {
            get
            {
    ...
    public class CNRSkinGridUI : MonoBehaviour
    {
        ...
        GUIStyle _sCell, _sCellEquipped, _sCellOwned, _sName, _sPrice,
                 _sBtn, _sBtnGreen, _sTitle, _sHint;
        bool     _stylesBuilt;
        Texture2D _txBg, _txDark, _txBlue, _txGreen, _txGray, _txCellEquipped;
        ...
        void DrawGrid()
        {
            ...
            GUI.Label(new Rect(0, 0, sw, titleH), "Skins", _sTitle);

            // Grid layout – 3 columns
            const int COLS = 3;
            float pad    = 10 * s;
            float cellW  = (sw - pad * (COLS + 1)) / COLS;
            float thumbH = cellW * 0.5f;       // 64:32 texture aspect
            float nameH  = 22 * s;
            float priceH = 18 * s;
            ...
            _scroll = GUI.BeginScrollView(new Rect(0, titleH, sw, sh - titleH),
            ...
            for (int i = 0; i < total; i++)
            {
                ...
                if (GUI.Button(new Rect(cx, cy, cellW, cellH), GUIContent.none, style))
                { GUI.EndScrollView(); OpenDetail(i); return; }

                Texture2D thumb = Hook.GetThumb(i);
                var tRect = new Rect(cx + 4 * s, cy + 4 * s, cellW - 8 * s, thumbH - 4 * s);
                if (thumb != null)
        ...
        void DrawDetail()
        {
            float sw = Screen.width, sh = Screen.height;
            float s  = sh / 900f;
            float topH = 70 * s, botH = 150 * s;

            string name  = Hook.GetDisplayName(_detailListIdx);
            bool   owned = Hook.GetOwned(_detailListIdx);
            bool   equip = (_detailListIdx == Hook.EquippedListIdx);
            int    price = Hook.GetPrice(_detailListIdx);

            // Top & bottom bars (transparent centre = 3-D model shows through)
            GUI.DrawTexture(new Rect(0, 0, sw, topH),         _txDark);
            GUI.DrawTexture(new Rect(0, sh - botH, sw, botH), _txDark);

            // Back button
            float bw = 90 * s, bh = 44 * s;
            if (GUI.Button(new Rect(10 * s, (topH - bh) / 2f, bw, bh), "\u2190 Back", _sBtn))
            { _view = View.Grid; return; }

            GUI.Label(new Rect(0, 0, sw, topH), name, _sTitle);
            GUI.Label(new Rect(0, sh * 0.45f, sw, 30 * s), "\u2190 drag to rotate \u2192", _sHint);

            // Action button
            float buyW = 220 * s, buyH = 56 * s;
            float buyX = (sw - buyW) / 2f;
            float buyY = sh - botH + (botH - buyH) / 2f;

            if (equip)
            {
                GUI.enabled = false;
                GUI.Button(new Rect(buyX, buyY, buyW, buyH), "\u2713 Equipped", _sBtnGreen);
                GUI.enabled = true;
            }
            else if (owned || price == 0)
            {
                if (GUI.Button(new Rect(buyX, buyY, buyW, buyH), "Equip", _sBtnGreen))
                    Hook.EquipCurrent();
            }
            else
            {
                if (GUI.Button(new Rect(buyX, buyY, buyW, buyH), "Buy  " + price + " coins", _sBtn))
                    Hook.EquipCurrent();
            }

            HandleRotation(topH, sh - botH, sw);
        }
        ...
        void BuildStyles()
        {
            ...
            _txGreen       = MakeTex(new Color(0.08f, 0.35f, 0.12f, 0.95f));
            ...
            _sBtnGreen = new GUIStyle(_sBtn);
            _sBtnGreen.normal.background = _txGreen;
            _sBtnGreen.hover.background  = MakeTex(new Color(0.12f, 0.50f, 0.18f, 1f));
