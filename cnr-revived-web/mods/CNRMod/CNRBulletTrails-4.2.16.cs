...
namespace CNRMods
{
    ...
    public class CNRBulletTrailSystem : MonoBehaviour
    {
        ...
        private const float TrailVerticalOffset = 0.025f;
        ...
        private static void ConfigureTrail(GameObject go)
        {
            ...
            try
            {
                GameObject anchor = EnsureTrailAnchor(go);
                if (anchor == null) return;

                TrailRenderer trail = anchor.GetComponent<TrailRenderer>();
                if (trail == null) trail = anchor.AddComponent<TrailRenderer>();
        ...
        private static GameObject EnsureTrailAnchor(GameObject go)
        {
            if (go == null) return null;

            Transform existing = go.transform.Find("CNR_BulletTrailAnchor");
            if (existing != null) return existing.gameObject;

            GameObject anchor = new GameObject("CNR_BulletTrailAnchor");
            anchor.transform.parent = go.transform;
            anchor.transform.localPosition = new Vector3(0f, TrailVerticalOffset, 0f);
