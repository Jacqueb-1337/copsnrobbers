...
namespace CNRMods
{
    ...
    internal static class CNRMinecraftWaterRegistry
    {
        ...
        internal static bool TryGetSurface(Vector3 point, out float surfaceY)
        {
            ...
            const float maxSeedDistance = 1.50f;
            ...
            float connectSlop = Mathf.Clamp(seed.size.y * 0.05f, 0.002f, 0.03f);
    ...
    internal class CNRMinecraftWaterController : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            ...
            float rawSubmersion = (surfaceY - body.min.y) / bodyHeight;
            _localPlayerInWater = rawSubmersion > 0.02f;

            // Use hysteresis at the surface. Once swimming has started, keep owning Y
            // movement slightly above the waterline so a held jump can crest the surface
            // and fall back naturally instead of toggling gravity and snapping downward.
            bool deepEnoughToSwim = _wasSwimming
                ? rawSubmersion > SwimExitSubmersion
                : rawSubmersion >= SwimEnterSubmersion;

            // Query live ladder contact, not only the previous frame's climbing state.
            // Unity does not guarantee LateUpdate order between these two components.
            if ((_ladder != null && _ladder.IsClimbing) || !deepEnoughToSwim)
            {
            ...
            float headDepth = surfaceY - body.max.y;
            bool nearSurface = headDepth <= SurfaceHeadBand * scale;
            if (jumpHeld && !nearSurface)
            {
            ...
            {
                // Once the player's head reaches the surface band, stop applying swim thrust and
                // let the remaining upward momentum arc slightly above the waterline before
                // falling back in. Holding Jump then repeats this as the classic surface bob.
                _verticalSpeed -= SurfaceGravity * scale * dt;
                _verticalSpeed = Mathf.Clamp(_verticalSpeed, -MaxSinkSpeed * scale, MaxRiseSpeed * scale);
            }
            else
            {
                // Classic water does not pull an idle player toward the surface. Without
                // upward input the player should settle downward slowly.
                _verticalSpeed = Mathf.MoveTowards(
                    _verticalSpeed,
                    -MaxSinkSpeed * scale,
            ...
            if (rawDelta.sqrMagnitude <= teleportGuard * teleportGuard && _controller.enabled)
            {
                ...
                float maxVerticalCorrection = Mathf.Max(0.20f * scale, MaxRiseSpeed * scale * dt * 2f);
                float correctionY = Mathf.Clamp(wantedY - rawDelta.y, -maxVerticalCorrection, maxVerticalCorrection);
    ...
    internal class CNRDLCMapLoader : MonoBehaviour
    {
        ...
        private const float LegacyWaterDepthBias = 0.0015f;
        ...
        private IEnumerator BuildScene()
        {
            ...
            try
            {
                ...
                ModEntry.Log("DLCMap built: chunks=" + _prepared.chunks.Length + " renderParts=" + renderParts + " collisionParts=" + collisionParts + " bulletPassThrough=" + bulletPassThroughParts + " climbables=" + climbableParts + " waterVolumes=" + waterParts + " legacyWaterBiasQuads=" + _legacyWaterBiasedQuads);
            }
            catch (Exception ex)
            {
        ...
        private static int BuildRenderParts(GameObject parent, CNRDLCMeshData[] parts, Material material, string label)
        {
            ...
            for (int i = 0; i < parts.Length; i++)
            {
                ...
                Mesh mesh = MakeMesh(data, true, string.Equals(label, "Transparent", StringComparison.Ordinal));
        ...
        private static int BuildCollisionParts(GameObject parent, CNRDLCMeshData[] parts)
        {
            ...
            for (int i = 0; i < parts.Length; i++)
            {
                CNRDLCMeshData data = parts[i];
                if (data == null || data.vertices == null || data.vertices.Length == 0) continue;
                Mesh mesh = MakeMesh(data, false, false);
        ...
        private static Mesh MakeMesh(CNRDLCMeshData data, bool withUv, bool applyLegacyWaterDepthBias)
        {
            ...
            if (withUv && applyLegacyWaterDepthBias)
                ApplyLegacyWaterDepthBias(vertices, data.uv, data.triangles);
        ...
        private static void ApplyLegacyWaterDepthBias(Vector3[] vertices, float[] rawUv, int[] triangles)
        {
            ...
            for (int q = 0; q < quadCount; q++)
            {
                ...
                // block faces. Pull only those side faces a hair back into the water volume.
                // The top surface stays at the exact exported height, so shorelines and the
                // gameplay water surface remain unchanged.
                if (Mathf.Abs(normal.y) > 0.25f) continue;

                Vector3 offset = -normal * LegacyWaterDepthBias;
