    ...
    public class CNRMatchSettingsUIHook : MonoBehaviour
    {
        ...
        private void TryInstallGear()
        {
            ...
            try
            {
                ...
                // Same pattern as the virtual mode radios: clone the native control,
                // keep its original NGUI draw behaviour, then replace only its action.
                bool wasActive = donor.activeSelf;
                if (!wasActive) donor.SetActive(true);
                _gear = (GameObject)UnityEngine.Object.Instantiate(donor);
                _gear.name = GearName;
                ...
                UISprite bg = FindSpriteByName(_gear.transform, "Background");
                if (bg != null) bg.transform.localScale = new Vector3(52f, 52f, 1f);

                UILabel label = ModeSelectUiUtil.FindInChildren<UILabel>(_gear.transform);
                if (label != null) label.text = "SET";
                ...
                ModEntry.Log("MatchSettingsUI: settings button cloned from native room control");
            }
            catch (Exception ex)
            {
            ...
            else
            {
                ...
                else if (mode == "kc")
                {
                    ...
                    if (s.KcBotsEnabled != 0)
                        AddValueRow("Bot Count", s.KcBotCount.ToString(), "kcbotcount", 1, ref y);
                }
                else if (mode == "ctf")
                {
                ...
                else
                {
                    ...
                    {
                        AddValueRow("Cop Bots", s.TdmCopBots < 0 ? "AUTO" : s.TdmCopBots.ToString(), "tdmcopbots", 1, ref y);
                        AddValueRow("Robber Bots", s.TdmRobberBots < 0 ? "AUTO" : s.TdmRobberBots.ToString(), "tdmrobberbots", 1, ref y);
                        MakeNativeLabel(_content.transform, "AUTO keeps 5 total players on each team.", new Vector3(0f, y - 0.01f, -2f), _baseDepth + 30, 0.62f);
                    }
                }
            }
        }
        ...
        private void Adjust(string key, int delta)
        {
            ...
            else if (key == "tdmcopbots")
            {
                if (s.TdmCopBots < 0 && delta > 0) s.TdmCopBots = 0;
                else if (s.TdmCopBots == 0 && delta < 0) s.TdmCopBots = -1;
                else s.TdmCopBots += delta;
            }
            else if (key == "tdmrobberbots")
            {
                if (s.TdmRobberBots < 0 && delta > 0) s.TdmRobberBots = 0;
                else if (s.TdmRobberBots == 0 && delta < 0) s.TdmRobberBots = -1;
                else s.TdmRobberBots += delta;
            }
