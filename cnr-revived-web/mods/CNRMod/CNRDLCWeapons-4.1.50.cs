...
namespace CNRMods
{
    ...
    public class CNRDLCWeaponRuntime : MonoBehaviour
    {
        ...
        private void ApplyBussinRecoil()
        {
            ...
            // CNRSettingsMod owns the mobile jump arc when present. Add the vertical
            // recoil directly into that same velocity so jump + blast genuinely raises
            // the apex instead of fighting a second movement loop. Vanilla/no-settings
            // clients fall back to our short-lived additive vertical velocity.
            if (!TryInjectSettingsJumpVelocity(impulse.y))
                _bussinFallbackVerticalVelocity += impulse.y;
        ...
        private bool TryInjectSettingsJumpVelocity(float impulseY)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    // vertical component is the actual net launch velocity requested.
                    current = -Physics.gravity.y + impulseY;
                    active = true;
                }

                _settingsOwnJumpVelY.SetValue(_settingsJumpHook, current);
                _settingsOwnJumpActive.SetValue(_settingsJumpHook, active);
                ModEntry.Log("CNR DLC weapons: injected Bussin vertical recoil into SettingsMod jump velocity=" + current);
                return true;
            }
            catch (Exception ex)
            {
