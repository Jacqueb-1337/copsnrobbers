...
namespace CNRMods
{
    ...
    public class ModEntry
    {
        ...
        public const  string Version = "3.2.36";
    ...
    public class CNRSkinGridUI : MonoBehaviour
    {
        ...
        void DrawDetail(float sw, float sh)
        {
            ...
            // Semi-transparent back button floating in top-left of the content rect
            Color prev = GUI.color;
            GUI.color = new Color(0f, 0f, 0f, 0.6f);
            GUI.DrawTexture(new Rect(6 * s, 6 * s, bw + 4f, bh + 4f),
                _txDark ?? Texture2D.blackTexture);
            GUI.color = prev;
            if (GUI.Button(new Rect(8 * s, 8 * s, bw, bh), "\u2190 Grid", _sBtn))
            {
                Hook.SetVanillaVisible(false);
                _scrollInitialized = false;  // re-centre on newly equipped skin
                _view = View.Grid;
            }
        }
