...
namespace CNRMods
{
    ...
    // -------------------------------------------------------------------------
    //  RemoteSmoothFix — overrides PlayerLocationSync.Update()'s weak asymptotic
    //  lerp (factor=Time.deltaTime*8 ≈ 0.5s to 99% at 30fps) with proper linear
    //  interpolation over the measured packet arrival interval.
    //
    //  Mechanism:
    //    PlayerLocationSync.Update() writes transform.position via slow lerp.
    //    Our LateUpdate() runs AFTER Update() on the same GO, overwriting that
    //    position with a linearly-interpolated value between the two most recent
    //    network snapshots.  The interval is measured from actual packet arrivals
    //    so it adapts to the real Photon sendRateOnSerialize without any hardcoded
    //    constants.
    //
    //  Result: lag drops from ~0.8s to roughly one packet interval (~50-100ms).
    // -------------------------------------------------------------------------
    public class RemoteSmoothFix : MonoBehaviour
    {
        private MonoBehaviour _pls;         // PlayerLocationSync component
        private FieldInfo     _fiPos;       // correctPlayerPos (private)
        private FieldInfo     _fiRot;       // correctPlayerRot (private)
        private FieldInfo     _fiLookPos;   // lookTargetPos    (private)
        private Transform     _lookTarget;  // lookTarget field value (public Transform)

        // Interpolation state
        private Vector3    _fromPos;
        private Quaternion _fromRot;
        private Vector3    _fromLookPos;
        private Vector3    _toPos;
        private Quaternion _toRot;
        private Vector3    _toLookPos;

        private Vector3    _lastTarget;   // last seen correctPlayerPos, to detect new packets
        private float      _interpStart;  // Time.time when the current segment started
        private float      _interval;     // estimated seconds between packets
        ...
        void Start()
        {
            ...
            // Bootstrap from current position so no initial jump
            _toPos = _lastTarget = transform.position;
            _fromPos = transform.position;
            _toRot = _fromRot = transform.rotation;
            if (_lookTarget != null) _fromLookPos = _toLookPos = _lookTarget.position;
            _interpStart = Time.time;
            _interval    = 1f / Mathf.Max(1, PhotonNetwork.sendRateOnSerialize);

            ModEntry.Log("RemoteSmoothFix: attached to " + gameObject.name
                + " interval=" + _interval.ToString("F3") + "s");
        }

        void LateUpdate()
        {
            if (_pls == null || _fiPos == null) return;

            var tgtPos = (Vector3)   _fiPos.GetValue(_pls);
            var tgtRot = (Quaternion)_fiRot.GetValue(_pls);

            // New network snapshot detected?
            if (tgtPos != _lastTarget)
            {
                float now     = Time.time;
                float elapsed = now - _interpStart;
                // Measure actual packet interval; 1.1× buffer prevents snap at segment end
                if (elapsed > 0.02f && elapsed < 2f)
                    _interval = elapsed * 1.1f;

                _fromPos = transform.position;   // start from current display position
                _fromRot = transform.rotation;
                if (_lookTarget != null) _fromLookPos = _lookTarget.position;

                _toPos = tgtPos;
                _toRot = tgtRot;
                if (_fiLookPos != null) _toLookPos = (Vector3)_fiLookPos.GetValue(_pls);

                _lastTarget  = tgtPos;
                _interpStart = now;
            }

            float frac = _interval > 0f
                ? Mathf.Clamp01((Time.time - _interpStart) / _interval)
                : 1f;

            transform.position = Vector3.Lerp(_fromPos, _toPos, frac);
            transform.rotation = Quaternion.Lerp(_fromRot, _toRot, frac);
            if (_lookTarget != null)
                _lookTarget.position = Vector3.Lerp(_fromLookPos, _toLookPos, frac);
        }
    }
}
