...
namespace CNRMods
{
    ...
    public partial class CNRArenaBotManager : MonoBehaviour
    {
        ...
        private static bool RulesEnableBots(CNRMatchSettingsData rules)
        {
            ...
            if (rules.Mode == "kc" || rules.Mode == "ffa") return rules.KcBotsEnabled != 0;
            return false;
        }
        ...
        {
            CNRMatchSettingsData rules = CNRMatchSettings.Active;
            return rules != null && rules.Mode == "ffa";
        }
        ...
        void OnMasterClientSwitched(PhotonPlayer newMaster)
        {
            try
            {
                ...
                {
                    // Master migration can arrive before the normal settings LateUpdate.
                    // Pull the persistent room rules first so takeover never runs on this
                    // client's local/default bot settings.
                    string syncReason;
                    CNRMatchSettings.SyncFromCurrentRoom(out syncReason);
                    ...
                    BroadcastReliableSnapshot();
                }
                else
                {
                    // On initial room entry Photon can report the existing master after its
                    // bot snapshot has already arrived. Preserve a queued packet from that
                    // same master. Only discard state that belongs to the old authority.
                    if (_pendingPackedState != null && _pendingPackedStateSender != newMasterId)
                    {
                        _pendingPackedState = null;
                        _pendingPackedStateSender = 0;
                    }
                    ModEntry.Log("ArenaBots: authority handoff -> remote master " + newMasterId +
                        (_pendingPackedState != null ? " (preserved queued snapshot)" : ""));
                }
            }
        ...
        private void ReconcileRoster(CNRMatchSettingsData rules)
        {
            ...
            else if (rules.Mode == "kc" || rules.Mode == "ffa")
            {
        ...
        private Vector3 FindSpawnPosition(TeamType team, CNRArenaBotState ignore)
        {
            ...
            if (IsFreeForAllMode())
            {
                // FFA ignores the bot's cosmetic Cop/Robber team and draws from the
                // complete multiplayer spawn pool, matching human FFA respawns.
                for (int teamIndex = 1; teamIndex <= 2; teamIndex++)
                {
                    string allPrefix = "Spawn_" + teamIndex + "_";
                    for (int i = 1; i <= 16; i++)
                    {
                        GameObject spawn = GameObject.Find(allPrefix + i);
            ...
            if (_mgr != null)
            {
                bool ffa = IsFreeForAllMode();
                PlayerInfo mine = _mgr.myPlayerInfo;
                if (mine != null && (ffa || mine.mTeam == team) && mine.mConnnectStatus == ConnectStatus.InGame)
                ...
                if (others != null)
                {
                    ...
                    {
                        PlayerInfo p = others[i];
                        if (p == null || IsBotId(p.mId) || (!ffa && p.mTeam != team) || p.mConnnectStatus != ConnectStatus.InGame) continue;
                        return p.mPosition;
                    }
                }
            }
        ...
        // The normal bot stream is intentionally unreliable/high-frequency. Attach one
        // reliable full snapshot to late-join identity broadcasts and authority handoffs
        // so a client can never remain bot-less just because it missed the fast stream.
        public static void AppendReliableSnapshot(System.Collections.Hashtable ht)
        {
            if (ht == null || _instance == null || !_instance.IsAuthority()) return;
            ht["bot_state"] = _instance.PackState();
        }

        public static void BroadcastReliableSnapshot()
        {
            if (_instance == null || !_instance.IsAuthority()) return;
            System.Collections.Hashtable ht = new System.Collections.Hashtable();
            ht["bot_state"] = _instance.PackState();
            ModEntry.RaiseCnrEvent(ht);
            ModEntry.Log("ArenaBots: reliable authority snapshot broadcast bots=" + _instance._bots.Count);
        }
