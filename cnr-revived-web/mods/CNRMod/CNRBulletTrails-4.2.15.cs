...
namespace CNRMods
{
    ...
    public class CNRBulletTrailSystem : MonoBehaviour
    {
        ...
        private const float ProjectileHeadDiameter = 0.055f;
        ...
        private static void EnsureProjectileHead(GameObject go)
        {
            if (go == null) return;

            Transform existing = go.transform.Find("CNR_BulletTrailHead");
            ...
            head.name = "CNR_BulletTrailHead";
            head.transform.parent = go.transform;
            head.transform.localPosition = Vector3.zero;
            head.transform.localRotation = Quaternion.identity;
            head.transform.localScale = new Vector3(ProjectileHeadDiameter, ProjectileHeadDiameter, ProjectileHeadDiameter);

            Collider headCollider = head.GetComponent<Collider>();
            if (headCollider != null)
            {
                headCollider.enabled = false;
                UnityEngine.Object.Destroy(headCollider);
            }

            Renderer headRenderer = head.GetComponent<Renderer>();
            if (headRenderer != null && _trailMaterial != null)
                headRenderer.sharedMaterial = _trailMaterial;
        }
