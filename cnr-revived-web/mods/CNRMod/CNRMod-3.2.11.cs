...
namespace CNRMods
{
    ...
    public class HitRegHook : MonoBehaviour
    {
        ...
        private System.Type    _plsType    = null;  // cached for RemoteSmoothFix attachment
        ...
        void Update()
        {
            ...
            if (Time.frameCount % 60 == 0)
            {
                ...
                // RemoteSmoothFix on each remote PlayerLocationSync GO.
                // Replaces the slow asymptotic Lerp(t=Time.deltaTime*8) with
                // linear interpolation over the actual measured packet interval,
                // cutting perceived remote-player position lag from ~0.8s to ~1 packet interval.
                if (_plsType == null)
                    _plsType = System.Type.GetType("PlayerLocationSync");
                if (_plsType != null)
                {
                    foreach (UnityEngine.Object o in FindObjectsOfType(_plsType))
                    {
                        Component c = (Component)o;
                        PhotonView pv = c.gameObject.GetComponent<PhotonView>();
                        if (pv != null && pv.isMine) continue; // skip local player
                        if (c.gameObject.GetComponent<RemoteSmoothFix>() == null)
                            c.gameObject.AddComponent<RemoteSmoothFix>();
                    }
                }

                // Boost Photon serialization rate for smoother remote-player movement.
                // Default sendRateOnSerialize=10 (100ms between updates) contributes
                // significantly to perceived lag on LAN. 20/sec = 50ms baseline.
                if (PhotonNetwork.sendRate < 30)             PhotonNetwork.sendRate = 30;
                if (PhotonNetwork.sendRateOnSerialize < 20)  PhotonNetwork.sendRateOnSerialize = 20;
            }
        }
    }
    ...
    // -------------------------------------------------------------------------
    //  RemoteSmoothFix v2 — replaces PlayerLocationSync's weak lerp(t=dt*8).
    //
    //  Key insight from v1 failure:
    //    PlayerLocationSync.Update() runs BEFORE our LateUpdate() and modifies
    //    transform.position via its own lerp. Reading transform.position as the
    //    lerp start then applying ANOTHER lerp on top creates double-lerp
    //    interference that produces the ramp-slow-ramp oscillation.
    //
    //    Additionally, snapshot-to-snapshot interpolation with Clamp01 hard-stops
    //    the player at the destination while waiting for the next packet, causing
    //    visible 0.25s pauses.
    //
    //  Fix:
    //    Track _dispPos independently (never read transform.position for start).
    //    Use a single high-factor lerp (k=25) that converges in ~2 frames at 30fps
    //    regardless of packet timing. PlayerLocationSync.Update() still runs and
    //    writes correctPlayerPos via OnPhotonSerializeView; our LateUpdate just
    //    chases that target much faster and overwrites transform.position after.
    //    Snap if distance > 5 m to handle teleports/spawns without long catch-up.
    // -------------------------------------------------------------------------
    public class RemoteSmoothFix : MonoBehaviour
    {
        private MonoBehaviour _pls;
        private FieldInfo     _fiPos;
        private FieldInfo     _fiRot;
        private FieldInfo     _fiLookPos;
        private Transform     _lookTarget;

        private Vector3    _dispPos;
        private Quaternion _dispRot;
        private Vector3    _dispLookPos;
        private bool       _ready;

        private const float LERP_K    = 25f;   // converges ~97% in 2 frames at 30fps
        private const float SNAP_DIST = 5f;    // snap threshold in Unity units

        void Start()
        {
            System.Type t = System.Type.GetType("PlayerLocationSync");
            if (t == null) { Destroy(this); return; }
            _pls = GetComponent(t) as MonoBehaviour;
            if (_pls == null) { Destroy(this); return; }

            PhotonView pv = GetComponent<PhotonView>();
            if (pv != null && pv.isMine) { Destroy(this); return; }

            var flags = BindingFlags.Instance | BindingFlags.NonPublic;
            _fiPos     = t.GetField("correctPlayerPos", flags);
            _fiRot     = t.GetField("correctPlayerRot", flags);
            _fiLookPos = t.GetField("lookTargetPos",    flags);
            FieldInfo fiLT = t.GetField("lookTarget",
                BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            if (fiLT != null) _lookTarget = fiLT.GetValue(_pls) as Transform;

            if (_fiPos == null || _fiRot == null) { Destroy(this); return; }

            _dispPos  = transform.position;
            _dispRot  = transform.rotation;
            if (_lookTarget != null) _dispLookPos = _lookTarget.position;
            _ready = true;
            ModEntry.Log("RemoteSmoothFix v2: attached to " + gameObject.name);
        }
        ...
        void LateUpdate()
        {
            ...
            Vector3    tgtPos = (Vector3)   _fiPos.GetValue(_pls);
            Quaternion tgtRot = (Quaternion)_fiRot.GetValue(_pls);

            // Snap on large jumps (teleport / spawn) to avoid long catch-up drift
            if (Vector3.Distance(_dispPos, tgtPos) > SNAP_DIST)
            {
                _dispPos = tgtPos;
                _dispRot = tgtRot;
            }

            // High-factor lerp from OUR last displayed position (not transform.position
            // which PlayerLocationSync.Update already modified this frame with its slow lerp).
            // Min(1, dt*k) prevents overshoot; at 30fps dt*25=0.83 so converges in ~2 frames.
            float t = Mathf.Min(1f, Time.deltaTime * LERP_K);
            _dispPos = Vector3.Lerp(_dispPos, tgtPos, t);
            _dispRot = Quaternion.Lerp(_dispRot, tgtRot, t);

            transform.position = _dispPos;
            transform.rotation = _dispRot;

            if (_lookTarget != null && _fiLookPos != null)
            {
                Vector3 tgtLook = (Vector3)_fiLookPos.GetValue(_pls);
                if (Vector3.Distance(_dispLookPos, tgtLook) > SNAP_DIST) _dispLookPos = tgtLook;
                _dispLookPos = Vector3.Lerp(_dispLookPos, tgtLook, t);
                _lookTarget.position = _dispLookPos;
            }
        }
    }
}
