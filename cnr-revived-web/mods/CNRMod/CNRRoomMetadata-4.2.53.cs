...
namespace CNRMods
{
    ...
    internal static class CNRMatchMetadata
    {
        ...
        {
            if (raw == "0") return FreeForAllMode.IsActive ? "ffa" : "tdm";
        ...
        internal static string GetSelectedGameMode(MultiplayerSelectDirector msd)
        {
            ...
            if (ZombieMode.PendingZombie || ZombieMode.IsZombieRoom) return "zombies";
            if (FreeForAllMode.IsActive) return "ffa";
            if (msd == null) return "";
        ...
        {
            return id == "ctf" || id == "zombies" || id == "ffa";
        }
    ...
    public class CNRRoomCardMetadata : MonoBehaviour
    {
        ...
        private const string CustomThumbSpriteName = "CNRCustomRoomThumbnail";

        private string _lastKey = "";
        private UILabel _modeLabel;
        private Texture2D _downloadedThumb;
        private string _thumbRequest = "";
        private UIAtlas _customThumbAtlas;
        private Material _customThumbMaterial;
        private UIAtlas _originalMapAtlas;
        private string _originalMapSpriteName = "";
        private UISprite.Type _originalMapSpriteType;
        private Vector3 _originalMapSpriteScale;
        private bool _capturedOriginalMapSprite;
        ...
        void LateUpdate()
        {
            try
            {
                ...
                {
                    // Keep the existing vanilla UISprite as the renderer. Old NGUI is much
                    // more reliable when we swap its atlas than when we inject a UITexture.
                    Texture2D cachedThumb = !string.IsNullOrEmpty(mapId) ? ContentManager.GetMapThumbnail(mapId) : null;
                    if (cachedThumb != null)
                    {
                        if (_customThumbAtlas == null || Original.mapLogoSprite.atlas != _customThumbAtlas || !Original.mapLogoSprite.enabled)
                            SetCustomThumbnail(cachedThumb, false);
                    }
                    else if (_customThumbAtlas == null || Original.mapLogoSprite.atlas != _customThumbAtlas)
                    {
                        // Never flash the donor map while a custom thumbnail is still loading.
        ...
        private void SetCustomThumbnail(Texture2D tex, bool showVanillaWhenNull)
        {
            ...
            if (!_capturedOriginalMapSprite)
            {
                _capturedOriginalMapSprite = true;
                _originalMapAtlas = Original.mapLogoSprite.atlas;
                _originalMapSpriteName = Original.mapLogoSprite.spriteName ?? "";
                _originalMapSpriteType = Original.mapLogoSprite.type;
                _originalMapSpriteScale = Original.mapLogoSprite.transform.localScale;
            }

            if (tex == null)
            {
                if (showVanillaWhenNull)
                {
                    Original.mapLogoSprite.atlas = _originalMapAtlas;
                    Original.mapLogoSprite.spriteName = _originalMapSpriteName;
                    Original.mapLogoSprite.type = _originalMapSpriteType;
                    Original.mapLogoSprite.transform.localScale = _originalMapSpriteScale;
                    Original.mapLogoSprite.enabled = true;
                    Original.mapLogoSprite.MarkAsChanged();
                }
                else
                {
                    Original.mapLogoSprite.enabled = false;
            ...
            if (_customThumbAtlas == null)
            {
                Material donorMaterial = null;
                if (_originalMapAtlas != null) donorMaterial = _originalMapAtlas.spriteMaterial;
                if (donorMaterial == null) donorMaterial = Original.mapLogoSprite.material;

                if (donorMaterial != null)
                    _customThumbMaterial = new Material(donorMaterial);
                else
                {
                    Shader shader = Shader.Find("Unlit/Transparent Colored");
                    if (shader == null) shader = Shader.Find("Unlit/Texture");
                    _customThumbMaterial = new Material(shader);
                }
                _customThumbMaterial.hideFlags = HideFlags.DontSave;
                // This old NGUI groups widgets by material into separate MeshRenderers and
                // does not preserve widget depth between those draw calls. Keep the custom
                // thumbnail draw call above the room-card atlas/background draw call.
                _customThumbMaterial.renderQueue = 3100;

                GameObject atlasGo = new GameObject("CNRRoomThumbnailAtlas");
                atlasGo.hideFlags = HideFlags.DontSave;
                atlasGo.transform.parent = Original.transform;
                atlasGo.transform.localPosition = Vector3.zero;
                atlasGo.transform.localRotation = Quaternion.identity;
                atlasGo.transform.localScale = Vector3.one;
                _customThumbAtlas = atlasGo.AddComponent<UIAtlas>();
                _customThumbAtlas.spriteMaterial = _customThumbMaterial;
            }

            _customThumbMaterial.mainTexture = tex;
            UIAtlas.Sprite sprite = new UIAtlas.Sprite();
            sprite.name = CustomThumbSpriteName;

            // Cover the vanilla thumbnail slot with the one source image. Crop the
            // source rect around its centre instead of shrinking the widget, so wide
            // screenshots lose their sides automatically and never need a second PNG.
            Rect sourceRect = new Rect(0f, 0f, tex.width, tex.height);
            float slotW = Mathf.Abs(_originalMapSpriteScale.x);
            float slotH = Mathf.Abs(_originalMapSpriteScale.y);
            if (tex.width > 0 && tex.height > 0 && slotW > 0.01f && slotH > 0.01f)
            {
                float imageAspect = (float)tex.width / (float)tex.height;
                float slotAspect = slotW / slotH;
                if (imageAspect > slotAspect)
                {
                    float cropW = tex.height * slotAspect;
                    sourceRect.x = (tex.width - cropW) * 0.5f;
                    sourceRect.width = cropW;
                }
                else if (imageAspect < slotAspect)
                {
                    float cropH = tex.width / slotAspect;
                    sourceRect.y = (tex.height - cropH) * 0.5f;
                    sourceRect.height = cropH;
                }
            }
            sprite.outer = sourceRect;
            sprite.inner = sourceRect;
            List<UIAtlas.Sprite> sprites = new List<UIAtlas.Sprite>();
            sprites.Add(sprite);
            _customThumbAtlas.spriteList = sprites;

            Original.mapLogoSprite.atlas = _customThumbAtlas;
            Original.mapLogoSprite.spriteName = CustomThumbSpriteName;
            Original.mapLogoSprite.type = UISprite.Type.Simple;
            Original.mapLogoSprite.transform.localScale = _originalMapSpriteScale;
            Original.mapLogoSprite.enabled = true;
            Original.mapLogoSprite.MarkAsChanged();
        }
        ...
        void OnDestroy()
        {
            ...
            if (_customThumbAtlas != null)
            {
                GameObject atlasGo = _customThumbAtlas.gameObject;
                _customThumbAtlas = null;
                if (atlasGo != null) Destroy(atlasGo);
            }
            if (_customThumbMaterial != null) Destroy(_customThumbMaterial);
            _customThumbMaterial = null;
        }
    }
}
