    ...
    public class CNRPreJoinGate : MonoBehaviour
    {
        ...
        internal void CreateRoomWithCompatibility()
        {
            ...
            if (CNRMatchMetadata.TryGetSelectedCustomMap(out selectedMapId, out selectedMapName, out selectedMapThumb))
            {
                ...
                {
                    ShowBlocked("DLC map '" + selectedEntry.Name + "' is not ready. Re-select the map after it finishes downloading, or check the content download error.", true);
                    ModEntry.Log("PreJoin: blocked room creation because selected DLC map is not active: " + selectedEntry.Id);
                    return;
                }
            }
            try
