    ...
    public class CNRArenaBotManager : MonoBehaviour
    {
        ...
        private void ThinkBot(CNRArenaBotState bot)
        {
            ...
            if (!string.IsNullOrEmpty(targetId) && TryResolveEnemyTarget(bot, targetId, out targetInfo, out targetBot))
            {
                ...
                visible = HasLineOfSight(bot.Position + new Vector3(0f, 1.15f, 0f), actual + new Vector3(0f, 0.9f, 0f), targetId, actualDist + 1f);
                if (visible)
                {
                    targetPos = actual;
            ...
            // NetPlayerController.SetBodyRotation applies a -90 degree correction when
            // mapping PlayerInfo rotation onto the remote model. Real players naturally
            // send the CharacterBody rotation expected by that code; virtual bots must
            // supply the equivalent +90 degree compensated yaw.
            ...
            {
                bot.Status = PlayerStatus.fire;
                bot.NextShotAt = now + UnityEngine.Random.Range(0.32f, 0.55f);
        ...
        private void MoveBot(CNRArenaBotState bot, Vector3 desired, float amount)
        {
            ...
            if (TryProjectToWorldGround(next, out grounded))
                next.y = grounded.y + GetBodyGroundOffset();
            else
        ...
        private void FindNearestVisibleEnemy(CNRArenaBotState bot, out PlayerInfo bestInfo, out CNRArenaBotState bestBot)
        {
            ...
            for (int i = 0; i < _bots.Count; i++)
            {
                ...
                if (!HasLineOfSight(bot.Position + new Vector3(0f, 1.15f, 0f), b.Position + new Vector3(0f, 0.9f, 0f), b.Id, dist + 1f)) continue;
                best = d;
                bestInfo = null;
                bestBot = b;
            }
        ...
        private void ConsiderVisibleHumanTarget(CNRArenaBotState bot, PlayerInfo p, ref PlayerInfo best, ref float bestDist)
        {
            ...
            float dist = Mathf.Sqrt(d);
            if (!HasLineOfSight(bot.Position + new Vector3(0f, 1.15f, 0f), p.mPosition + new Vector3(0f, 0.9f, 0f), p.mId, dist + 1f)) return;
            bestDist = d;
            best = p;
        }
        ...
        private Vector3 SnapBodyPositionToGround(Vector3 position)
        {
            ...
            if (TryProjectToWorldGround(position, out grounded))
                position.y = grounded.y + GetBodyGroundOffset();
            return position;
        }
