...
namespace CNRMods
{
    ...
    internal static class CNRCompatibility
    {
        ...
        internal static bool ValidateRoom(RoomInfo room, out string reason, out bool modManagerHelpful)
        {
            ...
            string advertisedMapType = (GetRoomProp(room, CNRMatchMetadata.PropMapType) ?? "").Trim().ToLowerInvariant();
            string advertisedMapId = GetRoomProp(room, CNRMatchMetadata.PropMapId) ?? "";
            string advertisedMapName = GetRoomProp(room, CNRMatchMetadata.PropMapName) ?? "";
            bool advertisesCustomMap = advertisedMapType == "dlc" || advertisedMapType == "legacy";
            if (advertisesCustomMap && string.IsNullOrEmpty(advertisedMapId) && string.IsNullOrEmpty(advertisedMapName))
            {
                reason = "Bad match state. Host advertises a custom map without an authoritative map identity.";
                return false;
            }
            if (advertisedMapType == "dlc" && string.IsNullOrEmpty(advertisedMapId))
            {
                reason = "Bad match state. DLC map ID is missing; refusing donor-map fallback.";
                return false;
            }
            if (advertisedMapType == "vanilla" && (!string.IsNullOrEmpty(advertisedMapId) || !string.IsNullOrEmpty(advertisedMapName)))
            {
                reason = "Bad match state. Vanilla/custom map metadata is contradictory.";
                return false;
            }
    ...
    public class CNRPreJoinGate : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            EnsureRoomBrowserLimits();
            HookRoomButtons();
            HookMapSelectButtons();
        }

        private void EnsureRoomBrowserLimits()
        {
            CNRConnectMenu[] menus = (CNRConnectMenu[])FindObjectsOfType(typeof(CNRConnectMenu));
            for (int i = 0; i < menus.Length; i++)
            {
                CNRConnectMenu menu = menus[i];
                if (menu == null || menu.playerLimitArray == null || menu.playerLimitArray.Length < 2) continue;
                if (menu.playerLimitArray[1] < 12)
                {
                    menu.playerLimitArray[1] = 12;
                    ModEntry.Log("PreJoin: expanded room-browser max-player filter to 12");
                }
            }
        }
        ...
        private void SetPreflightStatus(string status, float progress)
        {
            _preflightStatus = status ?? "";
            EconomyHook.UpdateGlobalTaskProgress(_preflightStatus, progress);
        }

        private void StopPreflightProgress(bool completed)
        {
            EconomyHook.EndGlobalTaskProgress(completed);
        }
        ...
        private IEnumerator JoinRoomPreflightRoutine(RoomInfo room)
        {
            ...
            _blockReason = "";
            EconomyHook.BeginGlobalTaskProgress("Validating room...", 0.04f);
            SetPreflightStatus("Validating advertised room state...", 0.08f);
            ...
            if (!CNRCompatibility.ValidateRoom(room, out reason, out managerHelpful))
            {
                ...
                StopPreflightProgress(false);
                ShowBlocked(reason, managerHelpful);
                yield break;
            }

            SetPreflightStatus("Reading advertised match settings...", 0.18f);
            ...
            SetPreflightStatus("Checking required match resources...", 0.28f);
            bool resourcesOk = false;
            string resourceReason = "";
            yield return StartCoroutine(CNRRoomResources.PrepareRoom(room,
                delegate(string status) { SetPreflightStatus(status, 0.48f); },
            ...
            if (!resourcesOk)
            {
                ...
                _preflightStatus = "";
                CNRMatchSettings.ClearPending();
                CNRDLCMapLoader.CancelPrebuiltJoin();
                StopPreflightProgress(false);
                string missingDlcMapId = CNRRoomResources.TakeMissingRequiredDlcMapId();
                if (!string.IsNullOrEmpty(missingDlcMapId) && EconomyHook.OpenDownloadsForMap(missingDlcMapId))
                {
                    ModEntry.Log("PreJoin: required DLC map missing; opened Downloads for " + missingDlcMapId);
                    yield break;
                }
            ...
            // Do not enter Photon until the advertised custom map is actually active and,
            // for DLC maps, fully constructed under the hidden persistent pre-join root.
            string preparedMapType = (CNRCompatibility.GetRoomProp(room, CNRMatchMetadata.PropMapType) ?? "").Trim().ToLowerInvariant();
            string preparedMapId = CNRCompatibility.GetRoomProp(room, CNRMatchMetadata.PropMapId) ?? "";
            if (preparedMapType == "dlc")
            {
                SetPreflightStatus("Verifying advertised DLC map...", 0.66f);
                string activeId = CNRDLCMapLoader.IsActive ? PlayerPrefs.GetString(CNRDLCMapLoader.PrefId, "") : "";
                if (!CNRDLCMapLoader.IsActive || string.IsNullOrEmpty(preparedMapId) ||
                    !string.Equals(activeId, preparedMapId, StringComparison.OrdinalIgnoreCase))
                {
                    _preflightRunning = false;
                    _preflightStatus = "";
                    CNRMatchSettings.ClearPending();
                    CNRDLCMapLoader.CancelPrebuiltJoin();
                    StopPreflightProgress(false);
                    ShowBlocked("DLC map preparation did not reach a stable active state; room join was cancelled.", false);
                    yield break;
                }

                bool mapBuilt = false;
                string mapBuildReason = "";
                yield return StartCoroutine(CNRDLCMapLoader.PrebuildActiveForJoin(
                    delegate(string status, float progress)
                    {
                        SetPreflightStatus(status, 0.68f + 0.27f * Mathf.Clamp01(progress));
                    },
                    delegate(bool ok, string why)
                    {
                        mapBuilt = ok;
                        mapBuildReason = why;
                    }));
                if (!mapBuilt)
                {
                    ...
                    CNRDLCMapLoader.CancelPrebuiltJoin();
                    StopPreflightProgress(false);
                    ShowBlocked(string.IsNullOrEmpty(mapBuildReason) ? "DLC map could not be built before joining." : mapBuildReason, false);
                    yield break;
                }
            }
            else if (preparedMapType == "legacy")
            {
                SetPreflightStatus("Preparing custom map...", 0.82f);
                ...
                if (string.IsNullOrEmpty(activeUrl))
                {
                    ...
                    CNRMatchSettings.ClearPending();
                    StopPreflightProgress(false);
                    ShowBlocked("Custom map preparation did not complete; room join was cancelled.", false);
                    yield break;
                }
            }

            SetPreflightStatus("Joining room...", 0.98f);
            BeginJoin(room);
            StopPreflightProgress(true);
            _preflightRunning = false;
        }
        ...
        {
            if (_preflightRunning) return;
            StartCoroutine(CreateRoomWithCompatibilityRoutine());
        }

        private IEnumerator CreateRoomWithCompatibilityRoutine()
        {
            _preflightRunning = true;
            _blockReason = "";
            EconomyHook.BeginGlobalTaskProgress("Validating host match...", 0.05f);

            string localReason;
            if (!CNRCompatibility.ValidateLocalEnvironment(out localReason))
            {
                _preflightRunning = false;
                StopPreflightProgress(false);
                ShowBlocked(localReason, true);
                yield break;
            }
            ...
            if (msd == null)
            {
                _preflightRunning = false;
                StopPreflightProgress(false);
                ShowBlocked("Multiplayer room setup is unavailable.", false);
                yield break;
            }

            string selectedMapId, selectedMapName, selectedMapThumb;
            if (CNRMatchMetadata.TryGetSelectedCustomMap(out selectedMapId, out selectedMapName, out selectedMapThumb))
            {
                OfficialMapEntry selectedEntry = CNRMatchMetadata.FindOfficialMap(selectedMapId);
                if (selectedEntry != null && selectedEntry.IsDlcMap)
                {
                    SetPreflightStatus("Verifying selected DLC map...", 0.22f);
                    string activeId = CNRDLCMapLoader.IsActive ? PlayerPrefs.GetString(CNRDLCMapLoader.PrefId, "") : "";
                    bool selectedActive = CNRDLCMapLoader.IsActive
                        && string.Equals(activeId, selectedEntry.Id, StringComparison.OrdinalIgnoreCase);

                    if (!selectedActive)
                    {
                        string cachePath = ContentManager.MapCacheDir + selectedEntry.Id + ".json";
                        bool cached = File.Exists(cachePath);
                        if (cached && !string.IsNullOrEmpty(selectedEntry.Hash)
                            && !CNRRoomResources.VerifyFileHash(cachePath, selectedEntry.Hash))
                        {
                            ModEntry.Log("PreJoin: host selected DLC cache hash mismatch, deleting: " + selectedEntry.Id);
                        ...
                        if (cached)
                        {
                            ...
                            if (CNRDLCMapLoader.ActivateFile(cachePath, selectedEntry.Url, selectedEntry.Id, out activateReason))
                            {
                                selectedActive = true;
                                ModEntry.Log("PreJoin: host activated selected DLC map from cache: " + selectedEntry.Id);
                            }
                            else
                            {
                                ModEntry.Log("PreJoin: host selected DLC activation failed: " + activateReason);
                                try { File.Delete(cachePath); } catch { }
                                cached = false;
                            }
                        }
                        ...
                        if (!selectedActive)
                        {
                            _preflightRunning = false;
                            StopPreflightProgress(false);
                            if (!EconomyHook.OpenDownloadsForMap(selectedEntry.Id))
                                ShowBlocked("DLC map '" + selectedEntry.Name + "' must be downloaded before creating the room.", true);
                            ModEntry.Log("PreJoin: host room creation blocked because DLC is not ready: " + selectedEntry.Id);
                            yield break;
                        }
                    }
                    ...
                    yield return StartCoroutine(CNRDLCMapLoader.PrebuildActiveForJoin(
                        delegate(string status, float progress)
                        {
                            SetPreflightStatus(status, 0.30f + 0.58f * Mathf.Clamp01(progress));
                        },
                        delegate(bool ok, string why)
                        {
                            mapBuilt = ok;
                    ...
                    if (!mapBuilt)
                    {
                        ...
                        CNRDLCMapLoader.CancelPrebuiltJoin();
                        StopPreflightProgress(false);
                        ShowBlocked(string.IsNullOrEmpty(mapBuildReason) ? "DLC map could not be built before creating the room." : mapBuildReason, false);
                        yield break;
                    }
                }
            }

            SetPreflightStatus("Advertising room...", 0.96f);
            CreateRoomWithCompatibilityAfterPrebuild();
            StopPreflightProgress(string.IsNullOrEmpty(_blockReason));
            _preflightRunning = false;
        }

        private void CreateRoomWithCompatibilityAfterPrebuild()
        {
            string localReason;
            if (!CNRCompatibility.ValidateLocalEnvironment(out localReason)) { ShowBlocked(localReason, true); return; }
            MultiplayerSelectDirector msd = MultiplayerSelectDirector.mInstance;
            ...
            // A portal DLC map always uses a vanilla scene as a bootstrap. Never let a
            // failed/not-yet-finished DLC preparation silently turn that bootstrap into
            // the room's real map.
            string selectedMapId, selectedMapName, selectedMapThumb;
            if (CNRMatchMetadata.TryGetSelectedCustomMap(out selectedMapId, out selectedMapName, out selectedMapThumb))
            {
                ...
                if (selectedEntry != null && selectedEntry.IsDlcMap)
                {
                    string activeId = CNRDLCMapLoader.IsActive
                        ? PlayerPrefs.GetString(CNRDLCMapLoader.PrefId, "") : "";
                    bool selectedActive = CNRDLCMapLoader.IsActive
                        && string.Equals(activeId, selectedEntry.Id, StringComparison.OrdinalIgnoreCase);
                    ...
                    if (!selectedActive)
                    {
                        ...
                        if (cached && !string.IsNullOrEmpty(selectedEntry.Hash)
                            && !CNRRoomResources.VerifyFileHash(cachePath, selectedEntry.Hash))
                        {
                            ModEntry.Log("PreJoin: selected DLC map cache hash mismatch, deleting: " + selectedEntry.Id);
                            try { File.Delete(cachePath); } catch { }
                            cached = false;
                        }
                        ...
                        if (cached)
                        {
                            ...
                            {
                                selectedActive = true;
                                ModEntry.Log("PreJoin: activated selected DLC map from cache before room creation: " + selectedEntry.Id);
                            }
                            else
                            {
                                ModEntry.Log("PreJoin: cached DLC map could not be activated: " + activateReason);
                                try { File.Delete(cachePath); } catch { }
                                cached = false;
                            }
                        }
                        ...
                        if (!selectedActive)
                        {
                            if (!EconomyHook.OpenDownloadsForMap(selectedEntry.Id))
                                ShowBlocked("DLC map '" + selectedEntry.Name + "' must be downloaded before creating the room.", true);
                            ModEntry.Log("PreJoin: blocked room creation because selected DLC map is not downloaded/active: " + selectedEntry.Id);
                            return;
                        }
                    }
                }
            ...
            try
            {
                // Freeze every host-selected value before entering Loading. ToSubScene can
                // mutate the vanilla map/mode selection and used to make us advertise defaults.
                string roomMap = msd.mCurWWMapSelect;
                string roomVanillaMode = ((int)msd.curModeSet).ToString();
                string packedSettings = CNRMatchSettings.PackHost(msd);
                string canonicalMode = CNRMatchMetadata.GetSelectedGameMode(msd);
                string mapId, mapDisplayName, mapThumbUrl;
                bool hasCustomMap = CNRMatchMetadata.TryGetSelectedCustomMap(out mapId, out mapDisplayName, out mapThumbUrl);
                OfficialMapEntry roomMapEntry = hasCustomMap ? CNRMatchMetadata.FindOfficialMap(mapId) : null;
                bool roomMapIsDlc = hasCustomMap && (roomMapEntry != null
                    ? roomMapEntry.IsDlcMap
                    : (CNRDLCMapLoader.IsActive && string.Equals(PlayerPrefs.GetString(CNRDLCMapLoader.PrefId, ""), mapId, StringComparison.OrdinalIgnoreCase)));
                string roomManifestUrl = CNRRoomResources.HostManifestUrl ?? "";
                string roomManifestHash = CNRRoomResources.HostManifestHash ?? "";
                string roomInlineResources = CNRRoomResources.BuildInlineHostResources();
                int maxPlayers = 8;
                ...
                props[CNRMatchSettings.PropSettings] = packedSettings;
                props[CNRRoomResources.PropManifestUrl] = roomManifestUrl;
                props[CNRRoomResources.PropManifestHash] = roomManifestHash;
                props[CNRRoomResources.PropInlineResources] = roomInlineResources;
                ...
                props[CNRMatchMetadata.PropMapType] = !hasCustomMap ? "vanilla" : (roomMapIsDlc ? "dlc" : "legacy");
                if (hasCustomMap)
                {
                ...
                string[] lobbyProps = new string[] {
                    ...
                    CNRMatchMetadata.PropMapName, CNRMatchMetadata.PropMapThumb, CNRMatchMetadata.PropMapType,
                ...
                string advertisementReason;
                if (!CNRMatchMetadata.ValidateInitialAdvertisement(props, out advertisementReason))
                {
                    CNRMatchSettings.ClearPending();
                    CNRDLCMapLoader.CancelPrebuiltJoin();
                    ShowBlocked("Room was not created because its advertised state is incomplete: " + advertisementReason, false);
                    ModEntry.Log("PreJoin create advertisement BLOCKED: " + advertisementReason);
                    return;
                }
                ModEntry.Log("PreJoin advertisement ready room=" + roomName + " mode=" + canonicalMode
                    + " mapType=" + props[CNRMatchMetadata.PropMapType] + " mapId=" + (mapId ?? "")
                    + " settings=" + packedSettings + " resources=" + roomInlineResources);

                // Only leave the selection UI after the complete authoritative lobby payload
                // has been captured. Photon receives map, mode, settings and DLC resources
                // in the initial CreateRoom operation, never as a post-join correction.
        ...
        void OnPhotonCreateRoomFailed(object[] cause)
        {
            _preflightRunning = false;
            _preflightStatus = "";
            CNRMatchSettings.ClearPending();
            CNRDLCMapLoader.CancelPrebuiltJoin();
            StopPreflightProgress(false);
            ShowBlocked("Room creation failed before the match could start.", false);
        }
