...
namespace CNRMods
{
    ...
    internal class CNRDLCMapLoader : MonoBehaviour
    {
        ...
        {
            // Old donor scenes can leave the fallback lit shaders with very dark ambient
            // values, which makes doorways/holes blend into corner shadows. Keep a small
            // neutral ambient floor and add one soft fill light with shadows disabled.
            Color ambient = RenderSettings.ambientLight;
            const float floor = 0.34f;
            RenderSettings.ambientLight = new Color(
                Mathf.Max(ambient.r, floor),
                Mathf.Max(ambient.g, floor),
                Mathf.Max(ambient.b, floor),
                1f);

            if (parent != null)
            {
                GameObject fillGo = new GameObject("CNRDLCMapFillLight");
                fillGo.transform.parent = parent.transform;
                fillGo.transform.localPosition = Vector3.zero;
                fillGo.transform.localRotation = Quaternion.Euler(48f, -32f, 0f);
