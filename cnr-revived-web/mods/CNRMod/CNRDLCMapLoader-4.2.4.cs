...
namespace CNRMods
{
    ...
    {
        private const int PassThroughLayer = 2;
        private bool _oldIgnoreLayer2;
        ...
        void Awake()
        {
            try
            {
                _oldIgnoreLayer2 = Physics.GetIgnoreLayerCollision(PassThroughLayer, PassThroughLayer);
                Physics.IgnoreLayerCollision(PassThroughLayer, PassThroughLayer, true);
            }
            catch (Exception ex) { ModEntry.Log("DLCMap projectile pass-through setup warning: " + ex.Message); }
        }

        void Start() { ConfigureProjectiles(); }
        ...
        void OnDestroy()
        {
            try
            {
                foreach (KeyValuePair<GameObject, int> kv in _originalLayers)
                    if (kv.Key != null) kv.Key.layer = kv.Value;
                _originalLayers.Clear();
                Physics.IgnoreLayerCollision(PassThroughLayer, PassThroughLayer, _oldIgnoreLayer2);
            }
            catch { }
        }
            ...
            try
            {
                ...
                if (_weaponScriptType != null)
                {
                    ...
                    for (int i = 0; i < scripts.Length; i++)
                    {
                        ...
                        if (c == null) continue;
                        var glField = c.GetType().GetField("grenadeLauncher");
                        if (glField == null) continue;
                        object gl = glField.GetValue(c);
                        if (gl == null) continue;
                        var projectileField = gl.GetType().GetField("projectile");
                        if (projectileField == null) continue;
                        Rigidbody projectile = projectileField.GetValue(gl) as Rigidbody;
                        if (projectile != null) SetLayerRecursive(projectile.gameObject, PassThroughLayer);
                    }
                }
                ...
                if (_weaponSyncType != null)
                {
                    ...
                    for (int i = 0; i < syncs.Length; i++)
                    {
                        ...
                        if (c == null) continue;
                        var projectileField = c.GetType().GetField("projectile");
                        if (projectileField == null) continue;
                ...
                // Fallback for projectiles already instantiated before their weapon prefab was discovered.
                if (_projectileType != null)
                {
                    ...
                    for (int i = 0; i < live.Length; i++)
                    {
                        ...
                        if (c != null) SetLayerRecursive(c.gameObject, PassThroughLayer);
                    }
                }
            }
            catch (Exception ex) { ModEntry.Log("DLCMap projectile pass-through scan warning: " + ex.Message); }
        }

        private void SetLayerRecursive(GameObject go, int layer)
        {
            if (go == null) return;
            if (!_originalLayers.ContainsKey(go)) _originalLayers.Add(go, go.layer);
            go.layer = layer;
    ...
    internal class CNRDLCMapLoader : MonoBehaviour
    {
        ...
        private static int BuildBulletPassThroughBoxes(GameObject parent, float[][] boxes)
        {
            ...
            go.transform.localRotation = Quaternion.identity;
            go.transform.localScale = Vector3.one;
            go.layer = 2; // Unity built-in Ignore Raycast layer: solid physics, omitted by default gun raycasts.
            go.isStatic = true;
        ...
        private static int BuildClimbableBoxes(GameObject parent, float[][] boxes)
        {
            ...
            for (int i = 0; i < boxes.Length; i++)
            {
                ...
                go.transform.parent = parent.transform;
                go.transform.localPosition = new Vector3((b[0] + b[3]) * 0.5f, (b[1] + b[4]) * 0.5f, (b[2] + b[5]) * 0.5f);
                go.transform.localRotation = Quaternion.identity;
                go.transform.localScale = Vector3.one;
                go.tag = "Ladder";
