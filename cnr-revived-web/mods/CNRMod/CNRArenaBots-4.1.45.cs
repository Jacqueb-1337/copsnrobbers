...
namespace CNRMods
{
    ...
    public class CNRArenaBotManager : MonoBehaviour
    {
        ...
        private const float NAV_HEIGHT_TOLERANCE = 1.60f;
        ...
        private float _nextVisualRotateAt;
        ...
        void Update()
        {
            ...
            InjectPlayerInfos();
            if (Time.realtimeSinceStartup >= _nextVisualRotateAt)
            {
                _nextVisualRotateAt = Time.realtimeSinceStartup + 0.10f;
                ApplyBotVisualRotations();
            }
        ...
        private void ThinkBot(CNRArenaBotState bot)
        {
            ...
            Vector3 navDir = dir;
            if ((!visible || dist > 14f) && !TryGetNavigationDirection(bot, targetPos, out navDir)) navDir = dir;

            // While an enemy is visible, face the enemy itself. While searching a last
            // known position, face the next navigation waypoint so corners/stairs look
            // like actual movement rather than a body sliding sideways through the path.
            Vector3 facingDir = visible ? dir : navDir;
            ...
            if (!visible)
            {
                // Search the last known location through the shared zombie navigation
                // grid instead of moving in a straight line through map geometry.
                move = navDir;
        ...
        private bool TryGetNavigationDirection(CNRArenaBotState bot, Vector3 target, out Vector3 direction)
        {
            ...
            if (repath)
            {
                bot.NavPath = CNRZombieMod.ZombieNavGrid.Query(bot.Position, target);
                bot.NavPathIndex = 0;
            ...
            Vector3 waypoint;
            if (bot.NavPath == null ||
                !CNRZombieMod.ZombieNavGrid.GetWaypoint(bot.NavPath, bot.Position, ref bot.NavPathIndex, out waypoint))
                return false;
        ...
        private void MoveBot(CNRArenaBotState bot, Vector3 desired, float amount)
        {
            ...
            bool resolvedByNav = false;

            // When the shared zombie nav grid is available, trust its body-clearance and
            // stair/ledge walkability instead of treating every low riser as a wall. This
            // is what lets arena bots physically traverse the same stairs and small ledges
            // that the zombie controller already handles.
            ...
            {
                // Invalidate the path first. The next think will ask A* for a fresh route;
                // only use the old small side nudge as a fallback if no nav route is live.
                bot.NavPath = null;
                bot.NavPathIndex = 0;
                bot.NextNavRepathAt = 0f;
                if (!CNRZombieMod.ZombieNavGrid.Ready)
        ...
        private bool TryResolveEnemyTarget(CNRArenaBotState bot, string targetId, out PlayerInfo info, out CNRArenaBotState targetBot)
        {
            ...
            if (b != null)
            {
                if (b == bot || b.Team == bot.Team || b.Status == PlayerStatus.dead || b.Hp <= 0) return false;
                targetBot = b;
                return true;
            }
        ...
        {
            if (p == null || p.mId == "null" || p.mTeam == TeamType.Nil || p.mTeam == bot.Team) return false;
        ...
        private void WanderBot(CNRArenaBotState bot)
        {
            ...
            if (now >= bot.NextWanderAt || bot.WanderDirection.sqrMagnitude < 0.01f)
            {
                ...
                Vector3 snapped;
                if (_arenaNavScene == _scene && CNRZombieMod.ZombieNavGrid.Ready &&
                    CNRZombieMod.ZombieNavGrid.TrySnapToWalkable(candidate, 16, out snapped))
        ...
        private Vector3 SnapBodyPositionToGround(Vector3 position)
        {
            Vector3 grounded;
            if (_arenaNavScene == _scene && CNRZombieMod.ZombieNavGrid.Ready &&
                CNRZombieMod.ZombieNavGrid.TrySnapToWalkableNearHeight(position, 6, position.y, 2.5f, out grounded))
            {
                position.x = grounded.x;
                position.z = grounded.z;
                position.y = grounded.y + GetBodyGroundOffset() + BOT_GROUND_EXTRA_LIFT;
                return position;
            }
        ...
        private void ApplyBotVisualRotations()
        {
            ...
            for (int i = 0; i < players.Length; i++)
            {
                ...
                CNRArenaBotState bot = FindBot(npc.pInfo.mId);
                if (bot == null || bot.Status == PlayerStatus.dead) continue;
                try
                {
                    npc.SetBodyRotation(bot.BodyRotation);
                    npc.SetGunRotation(bot.GunRotation);
                }
                catch { }
            }
        }
