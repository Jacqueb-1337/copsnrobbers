...
namespace CNRMods
{
    ...
    public class EconomyHook : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            ...
            // Stop all EcoHook drawing/event-processing while Mod Manager overlay is open
            if (ModManagerIsOpen()) return;

            // When one of our own overlays is active, consume all non-rendering events NOW,
            // before the buttons are drawn. This keeps the buttons visual-only (matching the
            // recordings viewer, which also leaves EcoHook buttons visible under its dim overlay)
            // without the risk of a button tap firing through the dim.
            bool _ecoOverlayActive = _showMpDialog
