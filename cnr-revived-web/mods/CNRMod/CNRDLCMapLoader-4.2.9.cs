...
namespace CNRMods
{
    ...
    internal class CNRMinecraftLadderController : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            ...
            bool onClimbable = HasLiveContact();
            if (!onClimbable)
            {
                if (_wasOnClimbable)
                {
            ...
            if (_fps != null) jumpPressed = _fps.inputJump;
            if (!jumpPressed)
            {
                try { jumpPressed = Input.GetButton("Jump"); }
                catch { }
            }

            if (horizontalCollision || jumpPressed)
            {
            ...
            if (rawDelta.sqrMagnitude <= teleportGuard * teleportGuard && _controller != null && _controller.enabled)
            {
                ...
                Vector3 correction = desiredDelta - rawDelta;
