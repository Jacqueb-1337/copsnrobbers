...
namespace CNRRecordingMod
{
    ...
    public static class RecordingModEntry
    {
        ...
        public  const string Version = "1.39.7";
            ...
            try
            {
                ...
                for (int f = 0; f < _capturedFrames; f++)
                {
                    ...
                    if (inIdx >= 0)
                    {
                        ...
                        // java.nio.Buffer.address is a package-private long field holding the
                        // native memory pointer of a DirectByteBuffer. JNI bypasses Java access
                        // control so Get<long> reads it without reflection. Writing via Marshal.Copy
                        // sidesteps all ByteBuffer.put() JNI signature issues in Unity 4.6.
                        long nativeAddr = rawBuf.Get<long>("address");
                        if (f < 3) RecordingModEntry.Log("  frame " + f + " bufCap=" + bufCap + " addr=" + nativeAddr);
                        if (nativeAddr == 0)
                            throw new Exception("Buffer.address=0 (bufCap=" + bufCap + ")");
                        Marshal.Copy(feedBuf, 0, new IntPtr(nativeAddr), putLen);
