...
namespace CNRRecordingMod
{
    ...
    public static class RecordingModEntry
    {
        ...
        public  const string Version = "1.39.8";
    ...
    public class RecordingHook : MonoBehaviour
    {
        ...
        private PostRenderCapture _postRenderCapture;
        ...
        public void StartCapture()
        {
            ...
            // captureRT removed — OnPostRender approach doesn't need it.

            IsCapturing = true;
            // Attach PostRenderCapture to the highest-depth non-kamcord camera.
            // OnPostRender fires BEFORE eglSwapBuffers, so ReadPixels there captures the actual
            // rendered frame rather than the empty new back-buffer seen at WaitForEndOfFrame.
            if (_postRenderCapture != null) { Destroy(_postRenderCapture); _postRenderCapture = null; }
            Camera postCam = null;
            foreach (Camera c in Camera.allCameras)
            {
                if (c.name.ToLower().StartsWith("kamcord")) continue;
                if (postCam == null || c.depth > postCam.depth) postCam = c;
            }
            if (postCam != null)
            {
                _postRenderCapture = postCam.gameObject.AddComponent<PostRenderCapture>();
                _postRenderCapture.ReadTex = _readTex;
                RecordingModEntry.Log("  PostRenderCapture -> " + postCam.name + " d=" + postCam.depth);
            }
            else RecordingModEntry.Log("  PostRenderCapture: no camera found (will use null-RT fallback)");
        ...
        private IEnumerator CaptureFrameCoroutine()
        {
            ...
            try
            {
                // OnPostRender (attached to the highest-depth game camera) fires BEFORE
                // eglSwapBuffers and fills _readTex with the actual rendered frame.
                // By the time WaitForEndOfFrame fires, FrameReady is already true.
                if (_postRenderCapture != null && _postRenderCapture.FrameReady)
                {
                    _postRenderCapture.FrameReady = false;
                    if (verbose) RecordingModEntry.Log("  method=postRender");
                }
                else
                {
                    // Fallback: null-RT ReadPixels (reads after buffer swap — may be grey)
                    RenderTexture.active = null;
                    _readTex.ReadPixels(new Rect(0, 0, _scrW, _scrH), 0, 0, false);
                    RenderTexture.active = null;
                    _readTex.Apply(false);
                    if (verbose) RecordingModEntry.Log("  method=nullRT_fallback");
                }

                // First-frame diagnostics: log all cameras and take a reference screenshot.
                if (_capturedFrames == 0)
                {
                    ...
                    // Ask Unity to capture the actual screen to a PNG so we can compare.
                    Application.CaptureScreenshot("cnr_diag.png");
                    RecordingModEntry.Log("  [diag] CaptureScreenshot queued");
                }
                // Frame 30: read back the diagnostic screenshot and log its center pixel.
                if (_capturedFrames == 30)
                {
                    string diagPath = Application.persistentDataPath + "/cnr_diag.png";
                    if (File.Exists(diagPath))
                    {
                        try
                        {
                            byte[] pngBytes = File.ReadAllBytes(diagPath);
                            var diagTex = new Texture2D(2, 2, TextureFormat.RGBA32, false);
                            diagTex.LoadImage(pngBytes);
                            Color32 dC = diagTex.GetPixels32()[(diagTex.height / 2) * diagTex.width + diagTex.width / 2];
                            RecordingModEntry.Log("  [diag] screenshot (" + diagTex.width + "x" + diagTex.height
                                + ") center=(" + dC.r + "," + dC.g + "," + dC.b + ")");
                            Destroy(diagTex);
                        }
                        catch (Exception dex) { RecordingModEntry.Log("  [diag] screenshot read err: " + dex.Message); }
                    }
                    else RecordingModEntry.Log("  [diag] screenshot not found: " + diagPath);
                }

                Color32[] px = _readTex.GetPixels32();
                int texW = _readTex.width;
                Color32 pC = px[(_scrH / 2) * texW + _scrW / 2];
                if (verbose) RecordingModEntry.Log("  probe=(" + pC.r + "," + pC.g + "," + pC.b + ")");
            ...
            try
            {
                ...
                for (int f = 0; f < _capturedFrames; f++)
                {
                    ...
                    if (inIdx >= 0)
                    {
                        ...
                        // Write frame data into the input buffer via ByteBuffer.put(byte[]).
                        // Marshal.Copy via Buffer.address was unreliable on API 33 (x86_64 WSA)
                        // because the address field may contain a JVM heap pointer for software
                        // codec buffers, silently corrupting memory and crashing the codec.
                        // sbyte[] maps to Java byte[] in Unity's JNI bridge -- Call<T>("put")
                        // will invoke ByteBuffer.put(byte[]) with the exact JNI signature.
                        bool isDirect = rawBuf.Call<bool>("isDirect");
                        if (f < 3) RecordingModEntry.Log("  frame " + f + " bufCap=" + bufCap + " isDirect=" + isDirect);
                        sbyte[] javaBytes = new sbyte[putLen];
                        System.Buffer.BlockCopy(feedBuf, 0, javaBytes, 0, putLen);
                        rawBuf.Call<AndroidJavaObject>("put", javaBytes);
    ...
    // CaptureDisplay removed — OnPostRender approach is the active capture path.

    // Reads the screen in OnPostRender (before eglSwapBuffers) into a shared Texture2D.
    // Attached to the highest-depth game camera by RecordingHook.StartCapture().
    public class PostRenderCapture : MonoBehaviour
    {
        internal Texture2D ReadTex;
        internal bool      FrameReady;
        private void OnPostRender()
        {
            if (ReadTex == null) return;
            RenderTexture.active = null;
            ReadTex.ReadPixels(new Rect(0, 0, Screen.width, Screen.height), 0, 0, false);
            RenderTexture.active = null;
            ReadTex.Apply(false);
            FrameReady = true;
        }
