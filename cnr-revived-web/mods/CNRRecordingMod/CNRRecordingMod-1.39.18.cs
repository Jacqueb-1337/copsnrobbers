...
namespace CNRRecordingMod
{
    ...
    public static class RecordingModEntry
    {
        ...
        public  const string Version = "1.39.18";
    ...
    public class RecordingHook : MonoBehaviour
    {
        ...
        public void StartCapture()
        {
            ...
            // Disable kamcordPreCamera and kamcordPostCamera before scanning cameras.
            // Their OnPreRender/OnPostRender callbacks redirect all scene rendering into
            // Kamcord's EGL PBuffer (regardless of IsRecording() state), leaving the EGL
            // backbuffer grey. Disabling stops the callbacks; scene cameras then render
            // directly to the EGL backbuffer, making null-RT ReadPixels work.
            ...
            IsCapturing = true;
            // Identify the primary scene camera for diagnostic logging.
            _captureCamera = null;
            foreach (Camera c in Camera.allCameras)  // Kamcord cams now excluded (disabled)
            {
            ...
            // Attach PostRenderCapture to the primary scene camera.
            // OnRenderImage fires BEFORE the buffer swap; src is the camera's rendered
            // temp RT with actual scene content. (With Kamcord cameras now disabled,
            // the redirect to PBuffer is gone so src is no longer empty.)
            if (_postRenderCapture != null) { Destroy(_postRenderCapture); _postRenderCapture = null; }
            if (_captureCamera != null)
            {
                _captureRT = new RenderTexture(scrW, scrH, 0, RenderTextureFormat.ARGB32);
                _captureRT.Create();
                _postRenderCapture = _captureCamera.gameObject.AddComponent<PostRenderCapture>();
                _postRenderCapture.CapRT = _captureRT;
                RecordingModEntry.Log("  PostRenderCapture -> " + _captureCamera.name);
            }
        ...
        private IEnumerator CaptureFrameCoroutine()
        {
            ...
            try
            {
                // PostRenderCapture.OnRenderImage blits src into _captureRT before the swap.
                // With Kamcord cameras disabled src is the actual rendered scene (not grey).
                if (_postRenderCapture != null && _postRenderCapture.FrameReady)
                {
                    _postRenderCapture.FrameReady = false;
                    if (_postRenderCapture.SrcProbeSubmitted)
                    {
                        _postRenderCapture.SrcProbeSubmitted = false;
                        _postRenderCapture.SrcProbeTex.Apply(false);
                        Color32 sp = _postRenderCapture.SrcProbeTex.GetPixels32()[0];
                        if (verbose) RecordingModEntry.Log("  srcProbe=(" + sp.r + "," + sp.g + "," + sp.b + ")");
                    }
                    RenderTexture.active = _captureRT;
                    _readTex.ReadPixels(new Rect(0, 0, _scrW, _scrH), 0, 0, false);
                    RenderTexture.active = null;
                    _readTex.Apply(false);
                    if (verbose) RecordingModEntry.Log("  method=onRenderImage");
                }
                else
                {
                    // Fallback: null-RT (reads cleared backbuffer — grey, but at least won't crash)
                    RenderTexture.active = null;
                    _readTex.ReadPixels(new Rect(0, 0, _scrW, _scrH), 0, 0, false);
                    _readTex.Apply(false);
            ...
            try
            {
                ...
                for (int f = 0; f < _capturedFrames; f++)
                {
                    ...
                    for (int d = 0; d < 10; d++)
                    {
                        ...
                        // On C2 encoder (Android 13), releasing a CODEC_CONFIG buffer
                        // briefly puts the codec in an unstable state. Give it time to settle.
                        if ((flags & 2) != 0) System.Threading.Thread.Sleep(200);
                        if ((flags & 4) != 0) break;
                    }
                }
