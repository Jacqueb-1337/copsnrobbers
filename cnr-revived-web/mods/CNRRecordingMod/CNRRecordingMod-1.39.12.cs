...
namespace CNRRecordingMod
{
    ...
    public static class RecordingModEntry
    {
        ...
        public  const string Version = "1.39.12";
    ...
    public class RecordingHook : MonoBehaviour
    {
        ...
        private RenderTexture  _captureRT;     // filled by PostRenderCapture.OnRenderImage each frame
        ...
        public void StartCapture()
        {
            ...
            // captureRT: filled each frame by PostRenderCapture.OnRenderImage (actual rendered RT).
            ...
            IsCapturing = true;
            // Prefer the Skybox/SolidColor (primary) camera for OnRenderImage.
            // Unity 4.6: depth-clear cameras with OnRenderImage get a FRESH temp RT —
            // previous cameras' accumulated color is NOT composited into it, so src is empty.
            // A Skybox camera renders its own scene from scratch into a temp RT, so src IS valid.
            if (_postRenderCapture != null) { Destroy(_postRenderCapture); _postRenderCapture = null; }
            Camera postCam = null;
            ...
            if (postCam == null)  // fallback: any non-kamcord camera
                foreach (Camera c in Camera.allCameras)
                {
                    if (c.name.ToLower().StartsWith("kamcord")) continue;
                    if (postCam == null || c.depth > postCam.depth) postCam = c;
        ...
        private IEnumerator CaptureFrameCoroutine()
        {
            ...
            try
            {
                // OnRenderImage on the primary (Skybox) camera:
                // src probe was submitted in OnRenderImage — read result now (render thread synced).
                if (_postRenderCapture != null && _postRenderCapture.SrcProbeSubmitted)
                {
