...
// using System.Runtime.InteropServices; // was used for Marshal.Copy (now replaced with ByteBuffer.put)
...
namespace CNRRecordingMod
{
    ...
    public static class RecordingModEntry
    {
        ...
        public  const string Version = "1.39.2";
    ...
    public class RecordingHook : MonoBehaviour
    {
        ...
        internal RenderTexture _captureRT;    // CaptureDisplay.OnRenderImage blits src here each frame

        private CaptureDisplay _captureDisplayComp; // component added to the highest-depth game camera
        ...
        private void OnLevelWasLoaded(int level)
        {
            ...
            // Scene load destroys cameras — re-attach CaptureDisplay to the new last camera.
            if (IsCapturing)
                StartCoroutine(RehookCaptureDisplay());
        }
        ...
        private void OnDestroy()
        {
            ...
            if (_captureDisplayComp != null) { Destroy(_captureDisplayComp); _captureDisplayComp = null; }
        ...
        public void StartCapture()
        {
            ...
            // captureRT receives the game frame via CaptureDisplay.OnRenderImage on the
            // highest-depth actual game camera. On both OpenGL ES (physical Android) and
            // DirectX (WSA), OnRenderImage on a real rendering camera passes the camera's
            // fully-composited output as src, which we blit into captureRT.
            ...
            // Attach CaptureDisplay to the highest-depth real game camera.
            AttachCaptureDisplay();
        ...
        // OnRenderImage on a real rendering camera passes the camera's composited output
        // (previous cameras + this camera) as src, giving us the full game frame.
        private void AttachCaptureDisplay()
        {
            if (_captureDisplayComp != null) { Destroy(_captureDisplayComp); _captureDisplayComp = null; }
            ...
            foreach (Camera c in cams)
            {
                ...
                if (lastCam == null || c.depth > lastCam.depth)
                    lastCam = c;
            }
            if (lastCam != null)
            {
                _captureDisplayComp      = lastCam.gameObject.AddComponent<CaptureDisplay>();
                _captureDisplayComp.hook = this;
                RecordingModEntry.Log("AttachCaptureDisplay: " + lastCam.name + " d=" + lastCam.depth);
            }
            else
            {
                RecordingModEntry.Log("AttachCaptureDisplay: no suitable camera found");
            }
        }

        private IEnumerator RehookCaptureDisplay()
        {
        ...
        private IEnumerator CaptureFrameCoroutine()
        {
            ...
            try
            {
                ...
                // CaptureDisplay.OnRenderImage blitted the game frame into _captureRT this frame.
        ...
        private void EncodeThread()
        {
            ...
            try
            {
                ...
                // Pre-allocate re-usable sbyte[] for ByteBuffer.put() — avoids per-frame allocation
                sbyte[] jByteScratch = new sbyte[encBufSize];
                ...
                for (int f = 0; f < _capturedFrames; f++)
                {
                    ...
                    if (inIdx >= 0)
                    {
                        ...
                        // Use ByteBuffer.put(byte[]) — correct API on all Android versions.
                        // The old java.nio.Buffer.address field hack fails on API 33 (returns 0).
                        if (jByteScratch.Length < putLen) jByteScratch = new sbyte[putLen];
                        Buffer.BlockCopy(feedBuf, 0, jByteScratch, 0, putLen);
                        rawBuf.Call("clear");
                        rawBuf.Call<AndroidJavaObject>("put", (object)jByteScratch);
    ...
    internal class CaptureDisplay : MonoBehaviour
    {
        ...
        {
            // src = this camera's rendered output (includes all lower-depth cameras because
            // those cameras render to the framebuffer before this one).
            // Always pass src through so the game display is unaffected.
