...
namespace CNRRecordingMod
{
    ...
    public static class RecordingModEntry
    {
        ...
        public  const string Version = "1.39.13";
    ...
    public class RecordingHook : MonoBehaviour
    {
        ...
        private RenderTexture  _captureRT;     // camera renders directly into this each frame
        private Camera         _captureCamera; // primary (Skybox) camera used for Camera.Render()
        private PostRenderCapture _postRenderCapture; // dead — kept for fallback if no Skybox cam
        ...
        public void StartCapture()
        {
            ...
            IsCapturing = true;
            // Capture via Camera.Render() + targetTexture — bypasses Kamcord's EGL/PBuffer redirect.
            // Kamcord hooks the EGL surface and pumps all cameras into an off-screen PBuffer;
            // OnRenderImage and null-RT ReadPixels both see the empty main EGL surface (grey).
            // Rendering directly into a Unity-managed RT (targetTexture) bypasses that redirect.
            _captureCamera = null;
            ...
            if (_captureCamera == null)  // fallback: any non-kamcord camera
                foreach (Camera c in Camera.allCameras)
                {
                    if (c.name.ToLower().StartsWith("kamcord")) continue;
                    if (_captureCamera == null || c.depth > _captureCamera.depth) _captureCamera = c;
        ...
        private IEnumerator CaptureFrameCoroutine()
        {
            ...
            try
            {
                ...
                {
                    // Camera.Render() into targetTexture bypasses Kamcord's EGL/PBuffer redirect.
                    // We get Main Camera's scene rendered directly into _captureRT.
                    ...
                    if (verbose) RecordingModEntry.Log("  method=camRender");
                }
                else
                {
                    // Fallback: null-RT ReadPixels
                    RenderTexture.active = null;
                    _readTex.ReadPixels(new Rect(0, 0, _scrW, _scrH), 0, 0, false);
                    RenderTexture.active = null;
                    _readTex.Apply(false);
