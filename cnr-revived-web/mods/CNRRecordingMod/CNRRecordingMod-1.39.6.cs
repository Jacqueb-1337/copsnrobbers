...
namespace CNRRecordingMod
{
    ...
    public static class RecordingModEntry
    {
        ...
        public  const string Version = "1.39.6";
    ...
    public class RecordingHook : MonoBehaviour
    {
        ...
        private RenderTexture  _captureRT;    // fallback RT used if null-RT ReadPixels returns grey
        ...
        public void StartCapture()
        {
            ...
            // captureRT is used as a fallback if null-RT ReadPixels returns grey.
            // Camera.Render() to this RT is synchronous and doesn’t race the render thread.
        ...
        private IEnumerator CaptureFrameCoroutine()
        {
            ...
            try
            {
                // Method 1: null-RT — reads the screen backbuffer after all rendering is done.
                ...
                bool nullGrey = pC.r > 180 && pC.r < 220 && pC.r == pC.g && pC.g == pC.b;

                if (nullGrey && _captureRT != null)
                {
                    // Method 2: explicit Camera.Render() to _captureRT.
                    // Camera.Render() is synchronous with respect to the render thread, so by
                    // the time it returns, _captureRT has actual rendered content.
                    // Uses the lowest-depth game camera (main scene, skybox/solid background).
                    Camera lowestCam = null;
                    foreach (Camera c in Camera.allCameras)
                    {
                        if (c.name.ToLower().StartsWith("kamcord")) continue;
                        if (lowestCam == null || c.depth < lowestCam.depth)
                            lowestCam = c;
                    }
                    if (lowestCam != null)
                    {
                        lowestCam.targetTexture = _captureRT;
                        lowestCam.Render();
                        lowestCam.targetTexture = null;
                        RenderTexture.active = _captureRT;
                        _readTex.ReadPixels(new Rect(0, 0, _scrW, _scrH), 0, 0, false);
                        RenderTexture.active = null;
                        _readTex.Apply(false);
                        px   = _readTex.GetPixels32();
                        pC   = px[(_scrH / 2) * texW + _scrW / 2];
                        if (verbose) RecordingModEntry.Log(
                            "  method=camRender/" + lowestCam.name
                            + " probe=(" + pC.r + "," + pC.g + "," + pC.b + ")");
                    }
                }
                else if (verbose)
                {
                    RecordingModEntry.Log("  method=nullRT probe=(" + pC.r + "," + pC.g + "," + pC.b + ")");
                }
    ...
    // CaptureDisplay removed — null-RT ReadPixels is the active capture path.
