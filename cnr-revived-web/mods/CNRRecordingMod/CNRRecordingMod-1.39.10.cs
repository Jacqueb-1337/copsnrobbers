...
namespace CNRRecordingMod
{
    ...
    public static class RecordingModEntry
    {
        ...
        public  const string Version = "1.39.10";
    ...
    public class RecordingHook : MonoBehaviour
    {
        ...
        public void StartCapture()
        {
            ...
            // OnRenderImage(src,dest) provides 'src' = the actual rendered frame RT
            // (Unity 4.6 composites into an internal RT, then blits to the GL backbuffer on swap;
            //  ReadPixels from OnPostRender hits the empty backbuffer — OnRenderImage avoids this).
            if (_postRenderCapture != null) { Destroy(_postRenderCapture); _postRenderCapture = null; }
            Camera postCam = null;
        ...
        private IEnumerator CaptureFrameCoroutine()
        {
            ...
            try
            {
                // OnRenderImage (attached to the highest-depth game camera) provides 'src' =
                // the actual rendered frame RT, filled BEFORE Unity blits it to the GL backbuffer.
                // When FrameReady, _captureRT already has correct content; read into _readTex.
    ...
    // Blits 'src' into CapRT each frame so CaptureFrameCoroutine can ReadPixels from it.
