...
namespace CNRRecordingMod
{
    ...
    public static class RecordingModEntry
    {
        ...
        public  const string Version = "1.39.16";
    ...
    public class RecordingHook : MonoBehaviour
    {
        ...
        private IEnumerator CaptureFrameCoroutine()
        {
            ...
            try
            {
                // Null-RT ReadPixels: reads from the EGL backbuffer.
                // Kamcord cameras are disabled in StartCapture so scene cameras render
                // directly to the EGL backbuffer instead of Kamcord's off-screen PBuffer.
                RenderTexture.active = null;
                _readTex.ReadPixels(new Rect(0, 0, _scrW, _scrH), 0, 0, false);
                _readTex.Apply(false);
                if (verbose) RecordingModEntry.Log("  method=nullRT");
