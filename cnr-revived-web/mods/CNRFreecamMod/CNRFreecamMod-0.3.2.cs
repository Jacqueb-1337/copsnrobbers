...
namespace CNRFreecamMod
{
    ...
    public class FreecamHook : MonoBehaviour
    {
        ...
        private void DumpSceneRenderers()
        {
            ...
            // All Renderers (active and inactive via Resources.FindObjectsOfTypeAll)
            UnityEngine.Object[] all = Resources.FindObjectsOfTypeAll(typeof(Renderer));
            FreecamModEntry.Log("Total Renderer instances (incl. prefabs): " + all.Length);
            int count = 0;
            foreach (UnityEngine.Object o in all)
            {
                Renderer r = o as Renderer;
                if (r == null) continue;
                // Skip prefabs / assets (HideAndDontSave means not in an active scene)
                if ((r.gameObject.hideFlags & HideFlags.HideAndDontSave) != 0) continue;
                string goActive  = r.gameObject.activeInHierarchy ? "ACTIVE" : "inactive";
                string renEnabled = r.enabled ? "enabled" : "DISABLED";
                string layer = LayerMask.LayerToName(r.gameObject.layer);
                FreecamModEntry.Log(string.Format("  [{0}] {1} | go={2} | ren={3} | layer={4}",
                    r.GetType().Name, GetPath(r.transform), goActive, renEnabled, layer));
                count++;
                if (count >= 200) { FreecamModEntry.Log("  ... (truncated at 200)"); break; }
            }
            FreecamModEntry.Log("=== End dump (" + count + " scene renderers) ===");
        }
