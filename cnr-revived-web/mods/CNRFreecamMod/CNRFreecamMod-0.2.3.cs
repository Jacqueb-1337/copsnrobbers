...
namespace CNRFreecamMod
{
    ...
    public class FreecamHook : MonoBehaviour
    {
        ...
        // The game hides the local player body two ways:
        //   1. renderer.enabled = false  on individual SMRs
        //   2. gameObject.SetActive(false) on whole body-part GOs
        // We need to undo both; track originals so we can restore cleanly.
        private SkinnedMeshRenderer[] _playerSMRs;
        private bool[]                _smrWasEnabled;
        private Transform[]           _activatedGOs;   // GOs we called SetActive(true) on
        ...
        // also activating any inactive ancestor GOs up to (not including) the
        // player root so the whole body becomes visible.
        private void ShowPlayerBody(bool show)
        {
            if (show)
            {
                ...
                // Step 1: activate any inactive child GOs between root and each SMR.
                // GetComponentsInChildren(true) finds SMRs even on inactive GOs.
                ...
                foreach (Transform t in _activatedGOs)
                    t.gameObject.SetActive(true);

                // Step 2: now all SMR GOs are active — grab them and enable renderers.
                ...
                FreecamModEntry.Log("Freecam: activated " + _activatedGOs.Length +
                    " GOs, enabled " + _playerSMRs.Length + " SMRs");
            }
            else
            {
                // Restore renderer enabled states
                if (_playerSMRs != null)
                {
                    for (int i = 0; i < _playerSMRs.Length; i++)
                    {
                ...
                // Restore GO active states (reverse order so parents deactivated last)
                if (_activatedGOs != null)
                {
                    for (int i = _activatedGOs.Length - 1; i >= 0; i--)
                    {
                        if (_activatedGOs[i] != null)
                            _activatedGOs[i].gameObject.SetActive(false);
                    }
                }
                _playerSMRs    = null;
                _smrWasEnabled = null;
