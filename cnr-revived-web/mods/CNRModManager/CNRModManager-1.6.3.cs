...
namespace CNRModManager
{
    ...
    public class ModManagerHook : MonoBehaviour
    {
        ...
        // Optional CNRMod 4.2.11+ queued-download bridge. When unavailable we retain
        // the legacy WWW downloader so this manager remains compatible with older cores.
        private static bool       _coreDlApiChecked = false;
        private static MethodInfo _coreDlEnqueueMI = null;
        private static MethodInfo _coreDlStateMI = null;
        private static MethodInfo _coreDlErrorMI = null;
        private static MethodInfo _coreDlCurrentMI = null;
        private static MethodInfo _coreDlTotalMI = null;
        private static MethodInfo _coreDlQueuePosMI = null;
        private static MethodInfo _coreDrawProgressMI = null;
        ...
        private void OnGUI()
        {
            ...
            if (_showWindow)
            {
                DrawWindow(vw, vh);
                DrawToast(vw, vh);
                DrawCoreDownloadProgressTopmost();
        ...
        private static void ResolveCoreDownloadApi()
        {
            if (_coreDlApiChecked) return;
            _coreDlApiChecked = true;
            try
            {
                Assembly myAsm = typeof(ModManagerEntry).Assembly;
                foreach (Assembly asm in AppDomain.CurrentDomain.GetAssemblies())
                {
                    if (asm == myAsm) continue;
                    Type q = asm.GetType("CNRMods.ManagedDownloadQueue");
                    if (q != null && _coreDlEnqueueMI == null)
                    {
                        BindingFlags sf = BindingFlags.Public | BindingFlags.Static;
                        _coreDlEnqueueMI = q.GetMethod("Enqueue", sf);
                        _coreDlStateMI = q.GetMethod("GetState", sf);
                        _coreDlErrorMI = q.GetMethod("GetError", sf);
                        _coreDlCurrentMI = q.GetMethod("GetCurrentBytes", sf);
                        _coreDlTotalMI = q.GetMethod("GetTotalBytes", sf);
                        _coreDlQueuePosMI = q.GetMethod("GetQueuePosition", sf);
                    }
                    Type eco = asm.GetType("CNRMods.EconomyHook");
                    if (eco != null && _coreDrawProgressMI == null)
                        _coreDrawProgressMI = eco.GetMethod("DrawGlobalDownloadProgressTopmost", BindingFlags.Public | BindingFlags.Static);
                }
            }
            catch (Exception ex) { ModManagerEntry.Log("Core download API probe failed: " + ex.Message); }
        }

        private static bool TryEnqueueCoreDownload(string url, string dest, string label, out string jobId)
        {
            jobId = "";
            ResolveCoreDownloadApi();
            if (_coreDlEnqueueMI == null || _coreDlStateMI == null) return false;
            try
            {
                object value = _coreDlEnqueueMI.Invoke(null, new object[] { url, dest, label });
                jobId = value as string ?? "";
                return !string.IsNullOrEmpty(jobId);
            }
            catch (Exception ex)
            {
                ModManagerEntry.Log("Core download enqueue failed; using WWW fallback: " + ex.Message);
                return false;
            }
        }

        private static string CoreDownloadString(MethodInfo mi, string jobId, string fallback)
        {
            if (mi == null) return fallback;
            try
            {
                object value = mi.Invoke(null, new object[] { jobId });
                return value != null ? value.ToString() : fallback;
            }
            catch { return fallback; }
        }

        private static int CoreDownloadInt(MethodInfo mi, string jobId)
        {
            if (mi == null) return 0;
            try
            {
        ...
        private static string DownloadDisplayName(string label)
        {
            if (string.IsNullOrEmpty(label)) return "file";
            int colon = label.IndexOf(':');
            return colon >= 0 && colon < label.Length - 1 ? label.Substring(colon + 1) : label;
        }

        private static void DrawCoreDownloadProgressTopmost()
        {
            ResolveCoreDownloadApi();
            if (_coreDrawProgressMI == null) return;
            try { _coreDrawProgressMI.Invoke(null, null); }
            catch { }
        }

        private IEnumerator DownloadToPath(string progressLabel, string url, string dest, DownloadResult result)
        {
            result.success = false;
            result.error = "";
            result.bytes = 0;
            string display = DownloadDisplayName(progressLabel);
            ...
            {
                result.error = "Could not create download directory: " + ex.Message;
                yield break;
            }

            string jobId;
            if (TryEnqueueCoreDownload(url, dest, progressLabel, out jobId))
            {
                ModManagerEntry.Log("Queued core download " + jobId + ": " + url + " -> " + dest);
                while (true)
                {
                    string state = CoreDownloadString(_coreDlStateMI, jobId, "missing");
                    int current = CoreDownloadInt(_coreDlCurrentMI, jobId);
                    int total = CoreDownloadInt(_coreDlTotalMI, jobId);
                    int queuePos = CoreDownloadInt(_coreDlQueuePosMI, jobId);

                    if (state == "succeeded")
                    {
                        result.success = true;
                        result.bytes = current > 0 ? current : total;
                        yield break;
                    }
                    if (state == "failed" || state == "missing")
                    {
                        result.error = CoreDownloadString(_coreDlErrorMI, jobId,
                            state == "missing" ? "download queue lost the job" : "download failed");
                        yield break;
                    }

                    if (state == "queued")
                        _statusMsg = "Queued " + display + (queuePos > 0 ? " (#" + queuePos + ")" : "") + "...";
                    else if (total > 0)
                        _statusMsg = "Downloading " + display + "... "
                            + Mathf.RoundToInt(Mathf.Clamp01(current / (float)total) * 100f) + "%";
                    else
                        _statusMsg = "Downloading " + display + "...";
            ...
            try
            {
                ...
                result.bytes = bytes != null ? bytes.Length : 0;
                result.success = true;
            }
            catch (Exception ex)
            {
        ...
        private IEnumerator DownloadMod(string displayName, string filename, string url, string version)
        {
            ...
            yield return StartCoroutine(DownloadToPath("mod:" + displayName, url, dest, result));
            if (!result.success)
            {
                _statusMsg = "Download error: " + result.error;
                ModManagerEntry.Log("Download err: " + result.error);
                yield break;
            }

            try
            {
                long savedBytes = File.Exists(dest) ? new FileInfo(dest).Length : result.bytes;
                ModManagerEntry.Log("Download OK: " + dest + " (" + savedBytes + " bytes)");
        ...
        private IEnumerator DownloadDependencyFile(RepoMod owner, ModDependency dep)
        {
            ...
            DownloadResult result = new DownloadResult();
            yield return StartCoroutine(DownloadToPath("dependency:" + destName, dep.url, dest, result));
            if (!result.success)
            {
                _statusMsg = "Dependency download error: " + result.error;
                ModManagerEntry.Log("Dependency file err: " + result.error);
                yield break;
            }
            ...
            {
                long savedBytes = File.Exists(dest) ? new FileInfo(dest).Length : result.bytes;
                ModManagerEntry.Log("Dependency file OK: " + dest + " (" + savedBytes + " bytes)");
            }
            catch (Exception ex)
            {
