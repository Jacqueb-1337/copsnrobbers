...
namespace CNRSettingsMod
{
    ...
    public static class SettingsExternalTabs
    {
        ...
        public static void Register(string id, Action<float> draw)
        {
            ...
            _drawers[id.Trim()] = draw;
        }

        public static bool Has(string id)
        {
            return !string.IsNullOrEmpty(id) && _drawers.ContainsKey(id);
        }
    ...
    public static class SettingsModEntry
    {
        ...
        public  const string Version = "3.1.108";
    ...
    public class SettingsModHook : MonoBehaviour
    {
        ...
        private Vector2 _scrollDev = Vector2.zero;
        ...
        private void DrawMatchStyleSettingsShell(float ox, float oy)
        {
            ...
            // External mods may append a fifth DEV tab through SettingsExternalTabs.
            GUI.Label(new Rect(ox + 300f, oy + 6f, 1000f, 70f), "CNR SETTINGS", _gsMatchTitle);

            bool hasDevTab = SettingsExternalTabs.Has("DEV");
            if (!hasDevTab && _activeTab == 4) _activeTab = 0;
            if (hasDevTab)
            {
                if (DrawMatchStyleButton(new Rect(ox + 120f, oy + 92.5f, 220f, 112f), "GENERAL", _activeTab == 0, 0.72f))
                { if (_activeTab != 0) { _activeTab = 0; _scroll = Vector2.zero; _hudEditNotInGameMsg = false; } }
                if (DrawMatchStyleButton(new Rect(ox + 405f, oy + 92.5f, 220f, 112f), "KB+MOUSE", _activeTab == 1, 0.72f))
                { if (_activeTab != 1) { _activeTab = 1; _scrollKbm = Vector2.zero; } }
                if (DrawMatchStyleButton(new Rect(ox + 690f, oy + 92.5f, 220f, 112f), "CONTROLLERS", _activeTab == 3, 0.72f))
                { if (_activeTab != 3) { _activeTab = 3; _scrollCtrl = Vector2.zero; } }
                if (DrawMatchStyleButton(new Rect(ox + 975f, oy + 92.5f, 220f, 112f), "ACCOUNT", _activeTab == 2, 0.72f))
                { if (_activeTab != 2) { _activeTab = 2; _scrollAccount = Vector2.zero; } }
                if (DrawMatchStyleButton(new Rect(ox + 1260f, oy + 92.5f, 220f, 112f), "DEV", _activeTab == 4, 0.72f))
                { if (_activeTab != 4) { _activeTab = 4; _scrollDev = Vector2.zero; } }
            }
            else
            {
                if (DrawMatchStyleButton(new Rect(ox + 162f, oy + 92.5f, 220f, 112f), "GENERAL", _activeTab == 0, 0.72f))
                { if (_activeTab != 0) { _activeTab = 0; _scroll = Vector2.zero; _hudEditNotInGameMsg = false; } }
                if (DrawMatchStyleButton(new Rect(ox + 514f, oy + 92.5f, 220f, 112f), "KB+MOUSE", _activeTab == 1, 0.72f))
                { if (_activeTab != 1) { _activeTab = 1; _scrollKbm = Vector2.zero; } }
                if (DrawMatchStyleButton(new Rect(ox + 866f, oy + 92.5f, 220f, 112f), "CONTROLLERS", _activeTab == 3, 0.72f))
                { if (_activeTab != 3) { _activeTab = 3; _scrollCtrl = Vector2.zero; } }
                if (DrawMatchStyleButton(new Rect(ox + 1218f, oy + 92.5f, 220f, 112f), "ACCOUNT", _activeTab == 2, 0.72f))
                { if (_activeTab != 2) { _activeTab = 2; _scrollAccount = Vector2.zero; } }
            }
        ...
        private void DrawSettingsWindow(int id)
        {
            ...
            else if (_activeTab == 4) _scrollDev = sv;
            ...
            {
                GUILayout.Space(2f);
                bool clicked = GUILayout.Button(GUIContent.none, GhostBtnStyle(), GUILayout.Height(38f));
                Rect rd = GUILayoutUtility.GetLastRect();
                Texture2D chkTex = _damageNumbers
                    ? (_spSelectKuang ?? MakeTex(2, 2, Color.white))
                    : (_spPropKuang   ?? MakeTex(2, 2, new Color(0.35f, 0.35f, 0.35f)));
                GUI.DrawTexture(new Rect(rd.x + 5f, rd.y + 4f, 30f, 30f), chkTex, ScaleMode.ScaleToFit);
                GUI.Label(new Rect(rd.x + 43f, rd.y, rd.width - 46f, rd.height), "Damage numbers", LabelStyle());
                if (clicked)
                {
                    _damageNumbers = !_damageNumbers;
            ...
            else if (_activeTab == 4) { SettingsExternalTabs.Draw("DEV", pw); }
        ...
        private void ApplySwipeDelta(float dy)
        {
            ...
            else if (_activeTab == 4) _scrollDev.y     = Mathf.Max(0f, _scrollDev.y     - dy);
            else                      _scrollAccount.y = Mathf.Max(0f, _scrollAccount.y - dy);
        }
