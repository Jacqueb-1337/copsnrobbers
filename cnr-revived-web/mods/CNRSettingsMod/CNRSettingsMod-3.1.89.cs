...
namespace CNRSettingsMod
{
    ...
    public class SettingsModHook : MonoBehaviour
    {
        ...
        private void ApplySensitivity()
        {
            ...
            if (_prevFpsOnFire != -1 && curFpsOnFire != _prevFpsOnFire)
            {
                ...
                if (_isAiming && !_unscopeOnFire)
                    _scopeLockUntil = Time.time + 0.6f;
            }
            ...
            if (_scopeLockUntil > Time.time)
                PreventFireUnscope();
            if (_isAiming && !_unscopeOnFire)
            {
                if (_scopeLockUntil <= 0f)
                    BeginScopeLock();
                else
                    PreventFireUnscope();
            }
        ...
        {
            if (_isAiming || _scopeLockUntil > 0f) return true;
            if (_mainCam == null) CacheMainCam();
        ...
        private void BeginScopeLock()
        {
            ...
            _scopeLockUntil = float.PositiveInfinity;
            _keepScopeTimer = -1f;
