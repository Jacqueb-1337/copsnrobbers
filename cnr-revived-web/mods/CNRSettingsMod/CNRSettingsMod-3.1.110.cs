...
namespace CNRSettingsMod
{
    ...
    // -------------------------------------------------------------------------
    // External settings tabs. Separate mods can register a tab without taking a
    // compile-time dependency on CNRSettingsMod by invoking Register via reflection.
    // -------------------------------------------------------------------------
    public static class SettingsExternalTabs
    {
        private static readonly Dictionary<string, Action<float>> _drawers = new Dictionary<string, Action<float>>(StringComparer.OrdinalIgnoreCase);
        private static readonly Dictionary<string, string> _titles = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
        private static readonly List<string> _order = new List<string>();

        // Backward-compatible registration: the tab ID is also its visible title.
        public static void Register(string id, Action<float> draw)
        {
            RegisterTab(id, id, draw);
        }

        // Preferred API for mods. Every registered tab is automatically added to
        // the Settings tab bar and receives its own independent scroll position.
        public static void RegisterTab(string id, string title, Action<float> draw)
        {
            if (string.IsNullOrEmpty(id) || draw == null) return;
            id = id.Trim();
            if (id.Length == 0) return;
            if (!_drawers.ContainsKey(id)) _order.Add(id);
            _drawers[id] = draw;
            _titles[id] = string.IsNullOrEmpty(title) ? id : title.Trim();
        }

        public static bool Has(string id)
        {
            return !string.IsNullOrEmpty(id) && _drawers.ContainsKey(id.Trim());
        }

        public static int Count { get { return _order.Count; } }

        public static string GetId(int index)
        {
            return index >= 0 && index < _order.Count ? _order[index] : null;
        }
        ...
        public static string GetTitle(int index)
        {
            ...
            string title;
            return _titles.TryGetValue(id, out title) && !string.IsNullOrEmpty(title) ? title : id;
        }

        public static void Draw(string id, float contentWidth)
        {
            Action<float> draw;
            if (!_drawers.TryGetValue(id, out draw) || draw == null) return;
            try { draw(contentWidth); }
            catch (Exception ex) { SettingsModEntry.Log("External tab '" + id + "' failed: " + ex); }
        }

        public static void DrawAt(int index, float contentWidth)
        {
            string id = GetId(index);
            if (!string.IsNullOrEmpty(id)) Draw(id, contentWidth);
        }
    }

    // Canonical Settings UI surface. Native tabs and external tabs both render
    // through these helpers so spacing, fonts, checkbox hit targets and section
    // styling stay identical as the Settings UI evolves.
    public static class SettingsUI
    {
        public static void Section(string title) { SettingsModHook.UISection(title); }
        public static void Label(string text) { SettingsModHook.UILabel(text); }
        public static void Hint(string text) { SettingsModHook.UIHint(text); }
        public static void Space(float pixels) { GUILayout.Space(Mathf.Max(0f, pixels)); }
        public static bool Toggle(string label, bool value) { return SettingsModHook.UIToggle(label, value); }
        public static bool Disclosure(string label, bool expanded) { return SettingsModHook.UIDisclosure(label, expanded); }
    }
    ...
    public static class SettingsModEntry
    {
        ...
        public  const string Version = "3.1.110";
    ...
    public class SettingsModHook : MonoBehaviour
    {
        ...
        private bool         _unscopeOnFire   = false;  // mod default: keep scope stable after fire
        private bool         _damageNumbers   = false;  // client-only; intentionally OFF by default
        ...
        private bool  _modAimed      = false;
        private bool  _prevIsAiming  = false; // track transitions to avoid writing every frame
        private float _keepScopeTimer = -1f;  // legacy fallback; no longer used for fire recovery
        private float _scopeLockUntil = -1f;  // while active, keep scoped weapon aimed through vanilla bowReload()
        private float _scopeHoldFov   = -1f;  // scoped FOV captured before vanilla tries to un-scope
        private bool  _scopeLockCoroutineActive = false;
        private CRWeaponScript _scopeLockWeapon = null;
        private object _scopeLockAnyWeapon = null;
        private float _postFireScopeUntil = -1f;
        private float _firePulseUntil = -1f;
        private int _postFireScopeLogBudget = 8;
        private const float POST_FIRE_SCOPE_RESTORE_SECS = 0.55f;
        private Dictionary<Type, FieldInfo> _aimedFieldCache = new Dictionary<Type, FieldInfo>();
        private Dictionary<Type, FieldInfo> _canAimFieldCache = new Dictionary<Type, FieldInfo>();
        private Dictionary<Type, FieldInfo> _weaponNameFieldCache = new Dictionary<Type, FieldInfo>();
        private Component _scopeWidgetComp = null;
        private GameObject _scopeWidgetGo = null;
        ...
        private int   _scopeDumpBudget = 3;
        ...
        private readonly Dictionary<string, Vector2> _externalTabScroll = new Dictionary<string, Vector2>(StringComparer.OrdinalIgnoreCase);
        ...
        private string  _createLoginUser = "";
        private string  _createLoginPassword = "";
        private string  _loginUser = "";
        private string  _loginPassword = "";
        private string  _pendingLoginUser = "";
        ...
        private int                     _rmbAimPressSerialSeen = 0;
        private int                     _rmbEscapePressSerialSeen = 0;
        private bool                    _escapeUnlockPending = false;
        private int                     _escapeUnlockQueuedSerial = 0;
        ...
        private static GUIStyle  _gsMatchTabText   = null;
        private static GUIStyle  _gsMatchFooterText = null;
        private static GUIStyle  _gsMatchTitle     = null;
        ...
        private float     _weapCooldownPct  = 0f;
        private float     _weapCooldownTotal = 0f;
        private float     _weapCooldownStart = -1f;
        private float     _weapCooldownEnd   = -1f;
        private float     _weapCooldownPrevNft = -1f;
        private bool      _weapWasReloading = false;
        private object    _weapCooldownWeapon = null;
        ...
        // -- Multiplayer ping HUD --------------------------------------------
        private float _pingHudNextSample = 0f;
        private int   _pingHudValue      = 0;
        private float _pingHudNextLog    = 0f;
        private float _pingHudRectLogAt  = 0f;

        // -- Audio ----------------------------------------------------------
        private float _audioMaster = 1f;
        private float _audioMusic = 1f;
        private float _audioAnnounce = 1f;
        private float _audioSfx = 1f;
        private float _audioNextApply = 0f;
        private float _audioNextScan = 0f;
        private AudioSource[] _audioCache = new AudioSource[0];
        private float _audioLastMaster = -1f;
        private float _audioLastMusic = -1f;
        private float _audioLastAnnounce = -1f;
        private float _audioLastSfx = -1f;
        private static readonly string[] _audioMusicHints = new string[]
        {
            "backgroundaudio", "background audio", "bgaudio", "bgm", "music", "menuaudio", "menu audio", "mainmenu", "menu", "singlemodebackaudiocontrol", "background"
        };
        private static readonly string[] _audioAnnounceHints = new string[]
        {
            "headshotkill", "enemydieaudio", "bullethitaudio", "killannounce", "announcer", "announce"
        };
        ...
        private void UpdateScene(string scene)
        {
            ...
            _scopeLockUntil   = -1f;
            _scopeHoldFov     = -1f;
            _scopeLockWeapon  = null;
            _scopeLockAnyWeapon = null;
            _postFireScopeUntil = -1f;
            _firePulseUntil   = -1f;
            _scopeWidgetComp  = null;
            _scopeWidgetGo    = null;
            _lastFireTime     = -999f;
            _prevFpsOnFire    = -1;
            _scopeDumpBudget  = 3;
            _postFireScopeLogBudget = 8;
            _weapCooldownPct   = 0f;
            _weapCooldownTotal = 0f;
            _weapCooldownStart = -1f;
            _weapCooldownEnd   = -1f;
            _weapCooldownPrevNft = -1f;
            _weapCooldownWeapon = null;
            _audioMaster = PlayerPrefs.GetFloat("CNRMod_AudioMaster", 1f);
            _audioMusic = PlayerPrefs.GetFloat("CNRMod_AudioMusic", 1f);
            _audioAnnounce = PlayerPrefs.GetFloat("CNRMod_AudioAnnounce", 1f);
            _audioSfx = PlayerPrefs.GetFloat("CNRMod_AudioSfx", 1f);
            _audioNextApply = 0f;
            _audioNextScan = 0f;
            _audioCache = new AudioSource[0];
            _audioLastMaster = -1f;
            _audioLastMusic = -1f;
            _audioLastAnnounce = -1f;
            _audioLastSfx = -1f;
            _sliderotate      = null;
            ...
            try { Type t = EcoType(); if (t != null) { var fi = t.GetField("ModManagerOpen", BindingFlags.Public | BindingFlags.Static); if (fi != null) fi.SetValue(null, false); var fi2 = t.GetField("SettingsModOpen", BindingFlags.Public | BindingFlags.Static); if (fi2 != null) fi2.SetValue(null, false); } } catch { }
            _nguiCam          = null;
            _pausePanelRef    = null;
            _pauseUIPanel     = null;
        ...
        private void Update()
        {
            ...
            if (_showSettings)
            {
                ...
                else if (Input.GetMouseButton(0) && _swipeActive)
                {
                    ...
                    {
                        _swipeDragging = true;
                        // Defocus any text field that captured MouseDown at the start of this gesture.
                        // Clears keyboard focus and dismisses the software keyboard.
            ...
            // Some Android builds report RMB as both Mouse1 and Back/Escape.  The
            // captured-pointer callback can arrive a frame later than Unity's Escape edge,
            // so defer a suspicious Escape by one frame and cancel it if an RMB press lands.
            bool aimUsesRmb = _kbKeys[8] == KeyCode.Mouse1;
            int rmbSerial = _capListener != null ? _capListener.rmbPressSerial : 0;
            bool rmbPressedSinceSeen = _capListener != null && rmbSerial != _rmbEscapePressSerialSeen;
            bool aimKeyHeld = aimUsesRmb && (Input.GetMouseButton(1) || (_capListener != null && _capListener.rmbHeld));

            if (_escapeUnlockPending)
            {
                bool delayedRmb = aimUsesRmb && _capListener != null &&
                    (_capListener.rmbPressSerial != _escapeUnlockQueuedSerial || _capListener.rmbHeld);
                _escapeUnlockPending = false;
                if (!delayedRmb && _cursorLocked)
                {
                    _rmbEscapePressSerialSeen = rmbSerial;
                    KbmSetCursorLocked(false);
                    return;
                }
            }
            ...
            {
                bool rmbEvidence = aimKeyHeld || (aimUsesRmb && (Input.GetMouseButtonDown(1) || rmbPressedSinceSeen));
                if (!rmbEvidence && aimUsesRmb && _capListener != null)
                {
                    _escapeUnlockPending = true;
                    _escapeUnlockQueuedSerial = rmbSerial;
                    _rmbEscapePressSerialSeen = rmbSerial;
                    return;
                }
                if (!rmbEvidence) { KbmSetCursorLocked(false); return; }
            }
            _rmbEscapePressSerialSeen = rmbSerial;
            ...
            // Aim toggle (index 8).  The serial catches quick RMB taps that begin and end
            // between Unity frames, while rmbHeld preserves the normal captured-button path.
            bool aimKeyDown;
            if (_capListener != null && _kbKeys[8] == KeyCode.Mouse1)
            {
                bool rmbNow = _capListener.rmbHeld;
                int pressSerial = _capListener.rmbPressSerial;
                aimKeyDown = pressSerial != _rmbAimPressSerialSeen || (rmbNow && !_rmbWasHeld);
                _rmbAimPressSerialSeen = pressSerial;
                _rmbWasHeld = rmbNow;
            }
            else
            {
                ...
                _rmbWasHeld = false;
                _rmbAimPressSerialSeen = _capListener != null ? _capListener.rmbPressSerial : 0;
            }
        ...
        public void OnFireButtonProxy(bool pressed)
        {
            if (!pressed) return;

            MarkFirePulse();

            if (_joyStickCtrl != null && _fiFireHoldCount != null)
                _fiFireHoldCount.SetValue(_joyStickCtrl, 0.235f);

            if (CRInput.mInstance != null)
        ...
        public void OpenSettings()
        {
            ...
            if (_showSettings)
            {
                ...
                try { Type t = EcoType(); if (t != null) { var fi = t.GetField("ModManagerOpen", BindingFlags.Public | BindingFlags.Static); if (fi != null) fi.SetValue(null, true); var fi2 = t.GetField("SettingsModOpen", BindingFlags.Public | BindingFlags.Static); if (fi2 != null) fi2.SetValue(null, true); } } catch { }
        ...
        private void UpdateWeaponCooldown()
        {
            ...
            if (_wmComp == null) { _weapOnCooldown = false; _weapCooldownPct = 0f; return; }
            ...
            if (selectedWeapon == null) { _weapOnCooldown = false; _weapCooldownPct = 0f; return; }

            if (selectedWeapon != _weapCooldownWeapon)
            {
                _weapCooldownWeapon = selectedWeapon;
                _weapCooldownStart = -1f;
                _weapCooldownEnd = -1f;
                _weapCooldownTotal = 0f;
                _weapCooldownPct = 0f;
                _weapCooldownPrevNft = -1f;
                _weapWasReloading = false;
                _fiWmNextFireTime = null;
            }

            float reloadTime;
            bool isReloading = TryGetReloadState(selectedWeapon, out reloadTime);
            if (isReloading)
            {
                if (!_weapWasReloading)
                {
                    _weapWasReloading = true;
                    _weapCooldownStart = Time.time;
                    _weapCooldownTotal = Mathf.Max(0.1f, reloadTime);
                    _weapCooldownEnd = _weapCooldownStart + _weapCooldownTotal;
                    _weapCooldownPct = 0f;
                }
                _weapOnCooldown = true;
                _weapCooldownPct = Mathf.Clamp01((Time.time - _weapCooldownStart) / Mathf.Max(0.1f, _weapCooldownTotal));
                return;
            }
            _weapWasReloading = false;
            ...
            if (_fiWmNextFireTime == null) { _weapOnCooldown = false; _weapCooldownPct = 0f; return; }

            float nft = (float)_fiWmNextFireTime.GetValue(selectedWeapon);
            if (nft <= Time.time + 0.001f)
            {
                _weapOnCooldown = false;
                _weapCooldownPct = 0f;
                _weapCooldownStart = -1f;
                _weapCooldownEnd = -1f;
                _weapCooldownTotal = 0f;
                _weapCooldownPrevNft = nft;
                return;
            }

            bool firedAgain = _weapCooldownPrevNft < 0f || nft > _weapCooldownPrevNft + 0.001f;
            if (_weapCooldownStart < 0f || firedAgain)
            {
                _weapCooldownStart = Time.time;
                _weapCooldownEnd = nft;
                _weapCooldownTotal = Mathf.Max(0f, _weapCooldownEnd - _weapCooldownStart);
                _weapCooldownPrevNft = nft;
            }

            if (_weapCooldownTotal < 0.25f)
            {
                _weapOnCooldown = false;
                _weapCooldownPct = 0f;
                return;
            ...
            _weapOnCooldown = true;
            _weapCooldownPct = Mathf.Clamp01((Time.time - _weapCooldownStart) / _weapCooldownTotal);
        }

        private static bool TryGetReloadState(object weapon, out float reloadTime)
        {
            reloadTime = 0f;
            if (weapon == null) return false;
            try
            {
                Type wt = weapon.GetType();
                FieldInfo isReloadField = wt.GetField("isReload", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                if (isReloadField == null || !(bool)isReloadField.GetValue(weapon)) return false;

                string[] groups = new string[] { "ShotGun", "machineGun" };
                for (int i = 0; i < groups.Length; i++)
                {
                    FieldInfo groupField = wt.GetField(groups[i], BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                    object group = groupField != null ? groupField.GetValue(weapon) : null;
                    if (group == null) continue;
                    FieldInfo reloadField = group.GetType().GetField("reloadTime", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                    if (reloadField == null) continue;
                    reloadTime = Convert.ToSingle(reloadField.GetValue(group));
                    if (reloadTime > 0f) return true;
                }

                Component weaponComponent = weapon as Component;
                if (weaponComponent != null && weaponComponent.gameObject != null &&
                    weaponComponent.gameObject.name.IndexOf("Bussin", StringComparison.OrdinalIgnoreCase) >= 0)
                {
                    reloadTime = PlayerPrefs.GetFloat("CNR_BussinReloadTime", 0f);
                    if (reloadTime > 0f) return true;
                }
                return false;
            }
        ...
        {
            if (Time.realtimeSinceStartup < _audioNextApply)
                return;
            _audioNextApply = Time.realtimeSinceStartup + 0.5f;

            if (Mathf.Abs(_audioLastMaster - _audioMaster) > 0.0005f ||
                Mathf.Abs(_audioLastMusic - _audioMusic) > 0.0005f ||
                Mathf.Abs(_audioLastAnnounce - _audioAnnounce) > 0.0005f ||
                Mathf.Abs(_audioLastSfx - _audioSfx) > 0.0005f ||
                Time.realtimeSinceStartup >= _audioNextScan ||
                _audioCache == null || _audioCache.Length == 0)
            {
                _audioCache = FindObjectsOfType(typeof(AudioSource)) as AudioSource[];
                if (_audioCache == null) _audioCache = new AudioSource[0];
                _audioNextScan = Time.realtimeSinceStartup + 2f;
            }

            float master = Mathf.Clamp01(_audioMaster);
            float music = Mathf.Clamp01(_audioMusic);
            float announce = Mathf.Clamp01(_audioAnnounce);
            float sfx = Mathf.Clamp01(_audioSfx);
            float musicVol = master * music;
            float announceVol = master * announce;
            float sfxVol = master * sfx;

            try { NGUITools.soundVolume = sfxVol; } catch { }

            for (int i = 0; i < _audioCache.Length; i++)
            {
                AudioSource src = _audioCache[i];
                if (src == null) continue;

                float target = sfxVol;
                string key = GetAudioKey(src);
                if (MatchesHint(key, _audioMusicHints)) target = musicVol;
                else if (MatchesHint(key, _audioAnnounceHints)) target = announceVol;

                src.volume = target;
            }

            _audioLastMaster = _audioMaster;
            _audioLastMusic = _audioMusic;
            _audioLastAnnounce = _audioAnnounce;
            _audioLastSfx = _audioSfx;
        }

        private static string GetAudioKey(AudioSource src)
        {
            if (src == null) return "";
            string name = "";
            try { if (src.gameObject != null) name = src.gameObject.name ?? ""; } catch { }
            if (string.IsNullOrEmpty(name) || string.Equals(name, "audio", StringComparison.OrdinalIgnoreCase))
            {
                try { name = src.name ?? ""; } catch { }
            }
            if (string.IsNullOrEmpty(name) || string.Equals(name, "audio", StringComparison.OrdinalIgnoreCase))
            {
                try { if (src.clip != null) name = src.clip.name ?? ""; } catch { }
            }
            if (string.IsNullOrEmpty(name) || string.Equals(name, "audio", StringComparison.OrdinalIgnoreCase))
            {
                try
                {
                    ...
                    {
                        string tn = t.name ?? "";
                        if (!string.IsNullOrEmpty(tn) && !string.Equals(tn, "audio", StringComparison.OrdinalIgnoreCase))
                        {
                            if (string.IsNullOrEmpty(name)) name = tn;
                            else name = tn + "/" + name;
                        }
                    }
                }
        ...
        private static bool MatchesHint(string key, string[] hints)
        {
            if (string.IsNullOrEmpty(key) || hints == null) return false;
            for (int i = 0; i < hints.Length; i++)
            {
                if (!string.IsNullOrEmpty(hints[i]) && key.IndexOf(hints[i], StringComparison.OrdinalIgnoreCase) >= 0)
                    return true;
            }
            return false;
        }

        private void DrawGunIconReloadOverlay()
        {
            if (_dragGOs[9] == null) return;
            if (!_weapOnCooldown && _weapCooldownPct <= 0f) return;

            Rect rect = GetGunIconOverlayRect();
            if (rect.width <= 1f || rect.height <= 1f) return;
            float padX = Mathf.Max(4f, rect.width * 0.08f);
            float padY = Mathf.Max(2f, rect.height * 0.04f);
            rect.x -= padX;
            rect.y -= padY;
            rect.width += padX * 2f;
            rect.height += padY * 2f;
            ...
            float pct = Mathf.Clamp01(_weapCooldownPct);
            float barH = Mathf.Max(3f, rect.height * 0.12f);
            Rect barBg = new Rect(rect.x + 1f, rect.yMax - barH - 1f, rect.width - 2f, barH);
            Rect barFg = new Rect(barBg.x, barBg.y, Mathf.Max(0f, barBg.width * pct), barBg.height);
            GUI.color = new Color(0.15f, 0.95f, 0.55f, 0.55f);
            GUI.DrawTexture(barBg, Texture2D.whiteTexture);
            GUI.color = new Color(0.95f, 1f, 1f, 0.85f);
            GUI.DrawTexture(barFg, Texture2D.whiteTexture);

            float sweep = Mathf.Clamp01(pct);
            float sweepW = Mathf.Max(rect.width * 0.20f, 10f);
            float sweepX = rect.x - sweepW + (rect.width + sweepW) * sweep;
            GUI.color = new Color(1f, 1f, 1f, 0.12f);
            GUI.DrawTexture(new Rect(sweepX, rect.y, sweepW, rect.height), Texture2D.whiteTexture);

            if (pct > 0.66f)
            {
                GUI.color = new Color(1f, 0.95f, 0.55f, 0.12f);
                GUI.DrawTexture(new Rect(rect.x + 2f, rect.y + 2f, rect.width - 4f, 2f), Texture2D.whiteTexture);
            }
            GUI.color = old;
        }

        private void DrawPingHudText()
        {
            if (_dragGOs[11] == null) return;

            if (Time.time >= _pingHudNextSample)
            {
                _pingHudNextSample = Time.time + 0.5f;
                if (PhotonNetwork.connected)
                {
                    _pingHudValue = Mathf.Max(0, PhotonNetwork.GetPing());
                    if (Time.time >= _pingHudNextLog)
                    {
                        _pingHudNextLog = Time.time + 10f;
                        SettingsModEntry.Log("[PingHUD] ping=" + _pingHudValue + "ms scene=" + _sceneName);
                    }
                }
                else
                {
            ...
            if (Time.time >= _pingHudRectLogAt)
            {
                _pingHudRectLogAt = Time.time + 10f;
                SettingsModEntry.Log("[PingHUD] rect=" + rect.x.ToString("F1") + "," + rect.y.ToString("F1") + " " + rect.width.ToString("F1") + "x" + rect.height.ToString("F1"));
            }
            ...
            Color txt;
            if (_pingHudValue < 100) txt = new Color(0.2f, 1f, 0.35f, 1f);
            else if (_pingHudValue < 200) txt = new Color(1f, 0.88f, 0.2f, 1f);
            else txt = new Color(1f, 0.3f, 0.3f, 1f);

            GUIStyle st = new GUIStyle(GUI.skin.label);
            st.alignment = TextAnchor.MiddleCenter;
            st.fontStyle = FontStyle.Bold;
            st.fontSize = 16;
            st.normal.textColor = txt;
            GUI.contentColor = txt;
            GUI.color = Color.white;
            GUI.Label(rect, _pingHudValue + "ms", st);

            GUI.color = old;
        }

        private Rect GetGunIconOverlayRect()
        {
            GameObject go = _dragGOs[9];
            if (go == null) return new Rect(-9999f, -9999f, 1f, 1f);
            ...
            for (int i = 0; i < cols.Length; i++)
            {
                ...
                Rect r;
                if (!TryProjectBounds(cols[i].bounds, out r)) continue;
                rect = found ? UnionRect(rect, r) : r;
                found = true;
            }
            ...
            for (int i = 0; i < widgets.Length; i++)
            {
                ...
                Rect r;
                if (!TryProjectWidget(widgets[i], out r)) continue;
                rect = found ? UnionRect(rect, r) : r;
                found = true;
            }

            return found ? rect : GetHandleRect(9);
        }
        ...
        private bool TryProjectBounds(Bounds bounds, out Rect rect)
        {
            ...
            return TryProjectWorldPoints(pts, out rect);
        }

        private bool TryProjectWidget(UIWidget widget, out Rect rect)
        {
            rect = new Rect(-9999f, -9999f, 1f, 1f);
            if (widget == null) return false;

            Vector2 size = widget.relativeSize;
            if (size.x <= 0.001f || size.y <= 0.001f) return false;

            Vector2 pivot = widget.pivotOffset;
            float cx = (pivot.x + 0.5f) * size.x;
            float cy = (pivot.y - 0.5f) * size.y;
            Vector2 half = size * 0.5f;
            Transform t = widget.transform;
            Vector3[] pts = new Vector3[]
            {
                t.TransformPoint(new Vector3(cx - half.x, cy - half.y, 0f)),
                t.TransformPoint(new Vector3(cx - half.x, cy + half.y, 0f)),
                t.TransformPoint(new Vector3(cx + half.x, cy - half.y, 0f)),
                t.TransformPoint(new Vector3(cx + half.x, cy + half.y, 0f))
            };
        ...
        private bool TryProjectWorldPoints(Vector3[] pts, out Rect rect)
        {
            rect = new Rect(-9999f, -9999f, 1f, 1f);
            if (pts == null || pts.Length == 0) return false;
            if (_nguiCam == null) CacheNguiCam();
            if (_nguiCam == null) return false;
            ...
            {
                Vector3 sp = _nguiCam.WorldToScreenPoint(pts[i]);
                if (sp.z < 0f) continue;
                float x = sp.x;
            ...
            if (!any) return false;
            rect = new Rect(minX, minY, Mathf.Max(1f, maxX - minX), Mathf.Max(1f, maxY - minY));
            return true;
        }
        ...
        private void ReleaseKbmFireState()
        {
            if ((object)_joyStickCtrl != null && _fiFireHoldCount != null)
        ...
        private void KbmFireDetect()
        {
            if (!_kbmEnabled || !_cursorLocked)
            {
                if (_kbmFireWasHeld) ReleaseKbmFireState();
                return;
            }
        ...
        private void TouchCameraDetect()
        {
            ...
            float touchSens = _modAimed ? (_sensNormal * _adsMultiplier) : _sensNormal;
        ...
        private void ApplySensitivity()
        {
            ...
            if (doLog) SettingsModEntry.Log("DIAG: modAimed=" + _modAimed
            ...
            bool hasWeaponAim = TryReadActiveWeaponAimed(out weaponAimed);
            bool fireRestoreActive = _postFireScopeUntil > Time.time || _scopeLockUntil > Time.time;
            bool scopedOrLocked = hasWeaponAim ? (weaponAimed || fireRestoreActive) : (isNowScoped || fireRestoreActive);
            bool wasAiming = _modAimed;
            _modAimed = scopedOrLocked;
            UpdateScopeWidget(_modAimed);
            ArmPostFireScopeRestoreIfNeeded(isNowScoped, hasWeaponAim, weaponAimed);

            // If vanilla bowReload() forces an un-scope after a shot, just record it.
            // Do not auto re-scope; that creates the visible flicker/loop.
            if (wasAiming && !_modAimed && !_unscopeOnFire && _postFireScopeUntil <= Time.time)
            {
                float timeSinceFire = Time.time - _lastFireTime;
                SettingsModEntry.Log("KeepScope: vanilla fire unscoped (dt=" + timeSinceFire.ToString("F3") + "s)");
                DumpScopeCandidates();
            }
            if (_modAimed) _scopeLockAnyWeapon = GetActiveWeaponObject();
            ...
            if (_modAimed != wasAiming)
            {
                float newSens = _modAimed ? aimedSens : _sensNormal;
                WriteAllSens(newSens);
                SettingsModEntry.Log("SensChange: scoped=" + _modAimed + " fov=" + currentFov.ToString("F1") + " -> sensitivityX=" + newSens.ToString("F2") + " (" + _allSliderotates.Count + " instances)");
            }
            _prevIsAiming = _modAimed;
        }

        private void MarkFirePulse()
        {
            _lastFireTime = Time.time;
            _firePulseUntil = Time.time + 0.18f;
        }

        private bool IsScopeFirePulseActive()
        {
            if (_firePulseUntil > Time.time) return true;
            if (_kbmFireWasHeld) return true;
            if ((object)CRInput.mInstance != null && CRInput.mInstance.m_bFire) return true;
            if ((object)CRJoyStickController.mInstance != null && CRJoyStickController.mInstance.fireFlag) return true;
            return false;
        }

        private void ArmPostFireScopeRestoreIfNeeded(bool isNowScoped, bool hasWeaponAim, bool weaponAimed)
        {
            if (_unscopeOnFire)
            {
                _postFireScopeUntil = -1f;
            ...
            if (!IsScopeFirePulseActive()) return;
            if (!(_modAimed || isNowScoped || (hasWeaponAim && weaponAimed))) return;

            CaptureScopeHoldFov();
            object weapon = GetActiveWeaponObject();
            if (weapon != null) _scopeLockAnyWeapon = weapon;
            _scopeLockWeapon = GetSelectedWeapon();

            float until = Time.time + POST_FIRE_SCOPE_RESTORE_SECS;
            _scopeLockUntil = until;
            _postFireScopeUntil = until;

            if (!_scopeLockCoroutineActive)
                StartCoroutine(PreventFireUnscopeEndOfFrame());
        }

        private void CaptureScopeHoldFov()
        {
            if (_mainCam == null) CacheMainCam();
            if (_mainCam == null) return;

            float fov = _mainCam.fieldOfView;
            if (fov > 1f && fov < _defaultFov - 2f)
            {
                _scopeHoldFov = fov;
                if (_scopedFov < 0f || fov < _scopedFov) _scopedFov = fov;
            }
            else if (_scopedFov > 1f)
            {
                _scopeHoldFov = _scopedFov;
            }
        }

        private bool IsLikelyScopedNow()
        {
            if (_modAimed) return true;
            if (_mainCam == null) CacheMainCam();
            if (_mainCam == null) return false;
            return _mainCam.fieldOfView < _defaultFov - 3f;
        }

        private FieldInfo GetCachedWeaponField(Dictionary<Type, FieldInfo> cache, Type type, string name)
        {
            if (type == null) return null;
            FieldInfo fi;
            if (cache.TryGetValue(type, out fi)) return fi;
            fi = type.GetField(name, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            cache[type] = fi;
            return fi;
        }

        private object GetSelectedUnityWeapon()
        {
            try
            {
                if (_wmComp == null)
                {
                    GameObject wmGo = GameObject.FindWithTag("WeaponManager");
                    if (wmGo != null) _wmComp = wmGo.GetComponent("WeaponManager");
                }
                if (_wmComp == null) return null;
                if (_fiWmSelWeapon == null)
                    _fiWmSelWeapon = _wmComp.GetType().GetField("SelectedWeapon");
                return _fiWmSelWeapon != null ? _fiWmSelWeapon.GetValue(_wmComp) : null;
            }
            catch
            {
                return null;
        ...
        private object GetActiveWeaponObject()
        {
            object weapon = GetSelectedUnityWeapon();
            if (weapon != null) return weapon;

            CRWeaponScript crWeapon = GetSelectedWeapon();
            if (crWeapon != null) return crWeapon;
            return GetActiveAimWeapon();
        }

        private bool TryReadActiveWeaponAimed(out bool aimed)
        {
            aimed = false;
            object weapon = GetActiveWeaponObject();
            if (weapon == null) return false;
            return TryReadWeaponAimed(weapon, out aimed);
        }

        private bool TryReadWeaponAimed(object weapon, out bool aimed)
        {
            aimed = false;
            try
            {
                FieldInfo fi = GetCachedWeaponField(_aimedFieldCache, weapon.GetType(), "aimed");
                if (fi == null) return false;
                object value = fi.GetValue(weapon);
        ...
        private bool TrySetWeaponAimed(object weapon, bool aimed)
        {
            try
            {
                if (weapon == null) return false;
        ...
        private bool WeaponCanAim(object weapon)
        {
            try
            {
                if (weapon == null) return false;
                FieldInfo fi = GetCachedWeaponField(_canAimFieldCache, weapon.GetType(), "canAim");
        ...
        private string WeaponDebugName(object weapon)
        {
            try
            {
                if (weapon == null) return "null";
                FieldInfo fi = GetCachedWeaponField(_weaponNameFieldCache, weapon.GetType(), "weaponName");
                object value = fi != null ? fi.GetValue(weapon) : null;
                return value != null ? value.ToString() : weapon.GetType().Name;
            }
            catch
            {
                return "unknown";
        ...
        private void DumpScopeCandidates()
        {
            if (_scopeDumpBudget <= 0) return;
            _scopeDumpBudget--;
            try
            {
                UnityEngine.Object[] all = Resources.FindObjectsOfTypeAll(typeof(GameObject));
                int logged = 0;
                SettingsModEntry.Log("ScopeDump: begin objects=" + all.Length);
                for (int i = 0; i < all.Length && logged < 80; i++)
                {
                    GameObject go = all[i] as GameObject;
                    if (go == null) continue;
                    string n = go.name != null ? go.name.ToLower() : "";
                    if (!(n.Contains("scope") || n.Contains("sni") || n.Contains("aim") ||
                          n.Contains("awp") || n.Contains("bow") || n.Contains("cross") ||
                          n.Contains("zoom") || n.Contains("qiang")))
                        continue;

                    string comps = "";
                    Component[] cs = go.GetComponents(typeof(Component));
                    for (int c = 0; c < cs.Length && c < 6; c++)
                    {
                        if (cs[c] == null) continue;
                        if (comps.Length > 0) comps += ",";
                        comps += cs[c].GetType().Name;
                    }
                    SettingsModEntry.Log("ScopeDump: " + ScopePath(go)
                        + " activeSelf=" + go.activeSelf
                        + " activeHier=" + go.activeInHierarchy
                        + " comps=" + comps);
                    logged++;
                }
                SettingsModEntry.Log("ScopeDump: end logged=" + logged);
            }
            catch (Exception ex)
            {
                SettingsModEntry.Log("ScopeDump error: " + ex.Message);
            }
        }

        private void UpdateScopeWidget(bool shouldShow)
        {
            try
            {
                if (_scopeWidgetGo != null)
                {
                    if (_scopeWidgetGo.activeSelf != shouldShow)
                        _scopeWidgetGo.SetActive(shouldShow);
                    if (_scopeWidgetComp is Behaviour)
                        ((Behaviour)_scopeWidgetComp).enabled = shouldShow;
                    if (shouldShow) return;
                }

                if (!shouldShow && _scopeWidgetGo != null) return;

                CRWeaponScript weapon = GetSelectedWeapon();
                if (weapon == null) return;

                GameObject root = ((Component)weapon).gameObject;
                if (root == null) return;

                Component[] comps = root.GetComponents(typeof(Component));
                for (int i = 0; i < comps.Length; i++)
                {
                    Component c = comps[i];
                    if (c == null) continue;
                    string tn = c.GetType().Name.ToLower();
                    if (tn.Contains("sniperscope") || tn.Contains("scope") || tn.Contains("aim"))
                    {
                        _scopeWidgetComp = c;
                        _scopeWidgetGo = c.gameObject;
                        if (_scopeWidgetGo != null)
                            _scopeWidgetGo.SetActive(shouldShow);
                        if (c is Behaviour)
                            ((Behaviour)c).enabled = shouldShow;
                        if (_scopePreventLogBudget > 0)
                        {
                            _scopePreventLogBudget--;
                            SettingsModEntry.Log("ScopeWidget: " + ScopePath(_scopeWidgetGo) + " comp=" + c.GetType().Name + " show=" + shouldShow);
                        }
                        return;
                    }
                }
                ...
                for (int i = 0; i < trs.Length; i++)
                {
                    ...
                    if (!(n.Contains("scope") || n.Contains("aim") || n.Contains("cross") || n.Contains("sniper")))
                        continue;
                    _scopeWidgetGo = t.gameObject;
                    _scopeWidgetComp = t.GetComponent(typeof(Behaviour));
                    _scopeWidgetGo.SetActive(shouldShow);
                    if (_scopeWidgetComp is Behaviour)
                        ((Behaviour)_scopeWidgetComp).enabled = shouldShow;
                    if (_scopePreventLogBudget > 0)
                    {
                        _scopePreventLogBudget--;
                        SettingsModEntry.Log("ScopeWidget: " + ScopePath(_scopeWidgetGo) + " show=" + shouldShow);
                    }
                    return;
                }
            }
            ...
            {
                SettingsModEntry.Log("ScopeWidget error: " + ex.Message);
            }
        }

        private string ScopePath(GameObject go)
        {
            if (go == null) return "null";
            string path = go.name;
            Transform t = go.transform;
            int guard = 0;
            while (t != null && t.parent != null && guard++ < 20)
            {
                t = t.parent;
        ...
        private void BeginScopeLock()
        {
            CaptureScopeHoldFov();
            if (_scopeHoldFov <= 1f) return;

            _scopeLockUntil = Time.time + 0.12f;
            _keepScopeTimer = -1f;
            _scopeLockWeapon = GetSelectedWeapon();
            _scopeLockAnyWeapon = GetActiveWeaponObject();
            PreventFireUnscope();
        }

        private void CancelScopeLock(bool restoreFov)
        {
            _scopeLockUntil = -1f;
            _postFireScopeUntil = -1f;
            _scopeHoldFov = -1f;
            ...
            _scopeLockAnyWeapon = null;
            _keepScopeTimer = -1f;
            if (restoreFov && _mainCam != null && _defaultFov > 1f)
                _mainCam.fieldOfView = _defaultFov;
        }

        private CRWeaponScript GetActiveAimWeapon()
        {
            CRWeaponScript weapon = null;
            if (_weaponManager == null)
                _weaponManager = (CRWeaponManager)UnityEngine.Object.FindObjectOfType(typeof(CRWeaponManager));
            if (_weaponManager != null)
                weapon = _weaponManager.SelectedWeapon;
            if (weapon != null && weapon.canAim) return weapon;

            CRWeaponScript[] weapons = (CRWeaponScript[])UnityEngine.Object.FindObjectsOfType(typeof(CRWeaponScript));
            for (int i = 0; i < weapons.Length; i++)
            {
                if (weapons[i] != null && weapons[i].canAim && ((Component)weapons[i]).gameObject.activeInHierarchy)
                    return weapons[i];
            }
            return null;
        }

        private CRWeaponScript GetSelectedWeapon()
        {
            if (_weaponManager == null)
                _weaponManager = (CRWeaponManager)UnityEngine.Object.FindObjectOfType(typeof(CRWeaponManager));
            return _weaponManager != null ? _weaponManager.SelectedWeapon : null;
        }

        private void PreventFireUnscope()
        {
            if (_unscopeOnFire) return;

            try
            {
                _modAimed = true;
                if (_mainCam == null) CacheMainCam();
                if (_mainCam != null && _scopeHoldFov > 1f)
                {
                    _mainCam.fieldOfView = _scopeHoldFov;
                }

                object weapon = _scopeLockAnyWeapon;
                if (weapon == null) weapon = GetActiveWeaponObject();
                if (weapon == null || !WeaponCanAim(weapon)) return;

                TrySetWeaponAimed(weapon, true);
                UpdateScopeWidget(true);
                if (_scopePreventLogBudget > 0)
                {
                    _scopePreventLogBudget--;
                    SettingsModEntry.Log("PreventFireUnscope: held " + WeaponDebugName(weapon)
                        + " type=" + weapon.GetType().Name
                        + " fov=" + (_mainCam != null ? _mainCam.fieldOfView.ToString("F1") : "null"));
                }
            }
            catch (Exception ex)
            {
                SettingsModEntry.Log("PreventFireUnscope error: " + ex.Message);
                CancelScopeLock(true);
            }
        }

        private void SyncWeaponAimState()
        {
            try
            {
                if (_unscopeOnFire) return;
                if (_postFireScopeUntil <= Time.time && _scopeLockUntil <= Time.time) return;
                PreventFireUnscope();
            }
            catch (Exception ex)
            {
                SettingsModEntry.Log("SyncWeaponAimState error: " + ex.Message);
            }
        }

        private IEnumerator PreventFireUnscopeEndOfFrame()
        {
            _scopeLockCoroutineActive = true;
            while (!_unscopeOnFire && (_postFireScopeUntil > Time.time || _scopeLockUntil > Time.time))
            {
            ...
            _postFireScopeUntil = -1f;
            _scopeLockCoroutineActive = false;
        }

        private int _scopePreventLogBudget = 8;
        ...
        public void ToggleAiming() { _modAimed = !_modAimed; }
        ...
        private void PatchAimButton()
        {
            if (_aimBtn == null) return;
            AimBtnClick proxy = _aimBtn.GetComponent<AimBtnClick>() ?? _aimBtn.AddComponent<AimBtnClick>();
            proxy.hook = this;

            MonoBehaviour[] comps = _aimBtn.GetComponents<MonoBehaviour>();
            for (int i = 0; i < comps.Length; i++)
            {
                MonoBehaviour mb = comps[i];
                if (mb == null) continue;
                string tn = mb.GetType().Name.ToLower();
                if (tn == "aimbtnclick") continue;
                if (tn.Contains("button") || tn.Contains("press") || tn.Contains("click") || tn.Contains("aim"))
        ...
        private void OnGUI()
        {
            ...
            if (_inGameScene && !_hudEditMode && !_showSettings)
            {
                DrawGunIconReloadOverlay();
                DrawPingHudText();
            }
            ...
            else
            {
                ...
                // Match Settings is authored against a 1600x900 backdrop. Use that exact
                // coordinate system here so title, tabs and footer share its geometry.
                scale = Mathf.Min(Screen.width / 1600f, Screen.height / 900f);
                if (scale <= 0f) scale = 1f;
                GUIUtility.ScaleAroundPivot(new Vector2(scale, scale), Vector2.zero);
                vw = Screen.width / scale;
                vh = Screen.height / scale;
                float ox = (vw - 1600f) * 0.5f;
                float oy = (vh - 900f) * 0.5f;

                GUI.color = new Color(0.16f, 0.16f, 0.16f, 1f);
                GUI.DrawTexture(new Rect(0f, 0f, vw, vh), Texture2D.whiteTexture);
                GUI.color = Color.white;

                DrawMatchStyleSettingsShell(ox, oy);

                // Same center content band used by Match Settings, between tabs and footer.
                _winRect = new Rect(ox + 280f, oy + 220f, 1040f, 510f);
                ...
                GUILayout.BeginArea(_winRect);
                DrawSettingsWindow(0);
                GUILayout.EndArea();
        ...
        private void DrawMatchStyleSettingsShell(float ox, float oy)
        {
            EnsureMatchStyleSettingsText();

            // Exact CNRMatchSettingsUI geometry converted from its normalized positions.
            // The tab strip is data-driven: built-in tabs plus any tabs requested by mods.
            GUI.Label(new Rect(ox + 300f, oy + 6f, 1000f, 70f), "CNR SETTINGS", _gsMatchTitle);

            int externalCount = SettingsExternalTabs.Count;
            if (_activeTab >= 4 && (_activeTab - 4) >= externalCount) _activeTab = 0;

            string[] builtInLabels = new string[] { "GENERAL", "KB+MOUSE", "CONTROLLERS", "ACCOUNT" };
            int[] builtInTabs = new int[] { 0, 1, 3, 2 };
            int totalTabs = builtInLabels.Length + externalCount;
            float stripX = 120f;
            float stripWidth = 1360f;
            float laneWidth = totalTabs > 0 ? stripWidth / totalTabs : stripWidth;
            float tabWidth = Mathf.Max(72f, Mathf.Min(220f, laneWidth - 16f));

            for (int i = 0; i < builtInLabels.Length; i++)
            {
                float x = stripX + laneWidth * i + (laneWidth - tabWidth) * 0.5f;
                int tabId = builtInTabs[i];
                if (DrawMatchStyleButton(new Rect(ox + x, oy + 92.5f, tabWidth, 112f), builtInLabels[i], _activeTab == tabId, 0.72f))
                    ActivateSettingsTab(tabId);
            }

            for (int i = 0; i < externalCount; i++)
            {
                float x = stripX + laneWidth * (builtInLabels.Length + i) + (laneWidth - tabWidth) * 0.5f;
                int tabId = 4 + i;
                string title = SettingsExternalTabs.GetTitle(i);
                if (string.IsNullOrEmpty(title)) title = SettingsExternalTabs.GetId(i) ?? "MOD";
                if (DrawMatchStyleButton(new Rect(ox + x, oy + 92.5f, tabWidth, 112f), title.ToUpperInvariant(), _activeTab == tabId, 0.72f))
                    ActivateSettingsTab(tabId);
            }

            if (DrawMatchStyleButton(new Rect(ox + 877f, oy + 758.5f, 230f, 112f), "BACK", true, 0.78f))
                CloseMatchStyleSettings(false);
            if (DrawMatchStyleButton(new Rect(ox + 1237f, oy + 758.5f, 230f, 112f), "SAVE", true, 0.78f))
                CloseMatchStyleSettings(true);
        }

        private void ActivateSettingsTab(int tabId)
        {
            ...
            if (tabId == 0)
            {
                ...
                _hudEditNotInGameMsg = false;
            }
            else if (tabId == 1) _scrollKbm = Vector2.zero;
            else if (tabId == 2) _scrollAccount = Vector2.zero;
            else if (tabId == 3) _scrollCtrl = Vector2.zero;
            else
            {
                string id = SettingsExternalTabs.GetId(tabId - 4);
                if (!string.IsNullOrEmpty(id)) _externalTabScroll[id] = Vector2.zero;
            }
        }

        private string ActiveExternalTabId()
        {
            return _activeTab >= 4 ? SettingsExternalTabs.GetId(_activeTab - 4) : null;
        }

        private bool DrawMatchStyleButton(Rect rect, string text, bool active, float labelScale)
        {
            if (_spButtonNull != null && Event.current.type == EventType.Repaint)
            {
                Color old = GUI.color;
                GUI.color = new Color(1f, 1f, 1f, active ? 1f : 0.60f);
                GUI.DrawTexture(rect, _spButtonNull, ScaleMode.StretchToFill);
                GUI.color = old;
            }

            GUIStyle style = labelScale >= 0.77f ? _gsMatchFooterText : _gsMatchTabText;
            return GUI.Button(rect, text, style);
        }

        private void EnsureMatchStyleSettingsText()
        {
            if (_gsMatchTabText == null)
            {
                _gsMatchTabText = new GUIStyle(GUI.skin.label);
                _gsMatchTabText.alignment = TextAnchor.MiddleCenter;
                _gsMatchTabText.fontStyle = FontStyle.Bold;
                _gsMatchTabText.fontSize = 19;
                _gsMatchTabText.normal.textColor = Color.white;
                _gsMatchTabText.hover.textColor = Color.white;
                _gsMatchTabText.active.textColor = Color.white;
                if (_gameFont != null) _gsMatchTabText.font = _gameFont;
            }
            if (_gsMatchFooterText == null)
            {
                _gsMatchFooterText = new GUIStyle(_gsMatchTabText);
                _gsMatchFooterText.fontSize = 20;
            }
            if (_gsMatchTitle == null)
            {
                _gsMatchTitle = new GUIStyle(_gsMatchTabText);
                _gsMatchTitle.fontSize = 31;
            }
        }

        private void CloseMatchStyleSettings(bool save)
        {
            _showSettings = false;
            if (_nguiUICam != null) _nguiUICam.enabled = true;
            try
            {
                ...
                if (t != null)
                {
                    var fi = t.GetField("ModManagerOpen", BindingFlags.Public | BindingFlags.Static);
                    if (fi != null) fi.SetValue(null, false);
                    var fi2 = t.GetField("SettingsModOpen", BindingFlags.Public | BindingFlags.Static);
                    if (fi2 != null) fi2.SetValue(null, false);
                }
            }
            catch { }
            if (save)
        ...
        {
            // If the finger just lifted after a swipe-scroll, consume the MouseUp so no button
            // fires accidentally. Don't use GUI.enabled=false — that tints all controls grey and
            // makes the panel visually darker for the entire duration of the scroll gesture.
            ...
            if (_gsVScroll == null) { _gsVScroll = new GUIStyle(GUI.skin.verticalScrollbar); _gsVScroll.fixedWidth = 12f; }
            string activeExternalId = ActiveExternalTabId();
            Vector2 externalScroll = Vector2.zero;
            if (!string.IsNullOrEmpty(activeExternalId)) _externalTabScroll.TryGetValue(activeExternalId, out externalScroll);
            Vector2 sv = (_activeTab == 0) ? _scroll
                       : (_activeTab == 1) ? _scrollKbm
                       : (_activeTab == 3) ? _scrollCtrl
                       : (_activeTab == 2) ? _scrollAccount
                       : externalScroll;
            // Swipe scrolling remains the primary mobile input path, but use a narrow native
            // vertical scrollbar whenever content actually overflows so every Settings tab
            // has a visible position indicator. Keep the background invisible to avoid the
            // old mobile-skin dark overlay flash during touch.
            sv = GUILayout.BeginScrollView(sv, false, false, GUIStyle.none, _gsVScroll, GUIStyle.none,
                GUILayout.Width(_winRect.width - 4f),
                GUILayout.Height(_winRect.height - 8f));
            if (_activeTab == 0) _scroll = sv;
            else if (_activeTab == 1) _scrollKbm = sv;
            else if (_activeTab == 3) _scrollCtrl = sv;
            else if (_activeTab == 2) _scrollAccount = sv;
            else if (!string.IsNullOrEmpty(activeExternalId)) _externalTabScroll[activeExternalId] = sv;
            GUILayout.Space(6f);
            ...
            GUILayout.Space(4f);
            GUILayout.BeginHorizontal();
            GUILayout.Label("Master  [" + Mathf.RoundToInt(_audioMaster * 100f) + "%]", LabelStyle(), GUILayout.Width(pw * 0.42f));
            float newAM = DrawSlider(_audioMaster, 0f, 1f);
            GUILayout.EndHorizontal();
            if (Mathf.Abs(newAM - _audioMaster) > 0.005f)
            {
                _audioMaster = Mathf.Round(newAM * 100f) / 100f;
                PlayerPrefs.SetFloat("CNRMod_AudioMaster", _audioMaster);
            }
            GUILayout.Label("  Scales all game audio categories.", HintStyle());
            GUILayout.Space(6f);

            GUILayout.BeginHorizontal();
            GUILayout.Label("Music  [" + Mathf.RoundToInt(_audioMusic * 100f) + "%]", LabelStyle(), GUILayout.Width(pw * 0.42f));
            float newMusic = DrawSlider(_audioMusic, 0f, 1f);
            GUILayout.EndHorizontal();
            if (Mathf.Abs(newMusic - _audioMusic) > 0.005f)
            {
                _audioMusic = Mathf.Round(newMusic * 100f) / 100f;
                PlayerPrefs.SetFloat("CNRMod_AudioMusic", _audioMusic);
            }
            GUILayout.Label("  Background music / menu music.", HintStyle());
            GUILayout.Space(6f);

            GUILayout.BeginHorizontal();
            GUILayout.Label("Announcements  [" + Mathf.RoundToInt(_audioAnnounce * 100f) + "%]", LabelStyle(), GUILayout.Width(pw * 0.42f));
            float newAnn = DrawSlider(_audioAnnounce, 0f, 1f);
            GUILayout.EndHorizontal();
            if (Mathf.Abs(newAnn - _audioAnnounce) > 0.005f)
            {
                _audioAnnounce = Mathf.Round(newAnn * 100f) / 100f;
                PlayerPrefs.SetFloat("CNRMod_AudioAnnounce", _audioAnnounce);
            }
            GUILayout.Label("  Headshot / kill / hit announcer clips.", HintStyle());
            GUILayout.Space(6f);

            GUILayout.BeginHorizontal();
            GUILayout.Label("SFX  [" + Mathf.RoundToInt(_audioSfx * 100f) + "%]", LabelStyle(), GUILayout.Width(pw * 0.42f));
            float newSfx = DrawSlider(_audioSfx, 0f, 1f);
            GUILayout.EndHorizontal();
            if (Mathf.Abs(newSfx - _audioSfx) > 0.005f)
            {
                _audioSfx = Mathf.Round(newSfx * 100f) / 100f;
                PlayerPrefs.SetFloat("CNRMod_AudioSfx", _audioSfx);
            }
            GUILayout.Label("  Shooting, reloading, footsteps, and other effects.", HintStyle());
            GUILayout.Space(14f);
            ...
            GUILayout.Space(4f);
            {
                bool newValue = SettingsUI.Toggle("Unscope after firing", _unscopeOnFire);
                if (newValue != _unscopeOnFire)
                {
                    _unscopeOnFire = newValue;
                    PlayerPrefs.SetInt("CNRMod_UnscopeOnFire", _unscopeOnFire ? 1 : 0);
                    if (_unscopeOnFire) CancelScopeLock(true);
                }
            }
            ...
            GUILayout.Space(4f);
            {
                bool newValue = SettingsUI.Toggle("Damage numbers", _damageNumbers);
                if (newValue != _damageNumbers)
                {
                    _damageNumbers = newValue;
                    PlayerPrefs.SetInt("CNRMod_DamageNumbers", _damageNumbers ? 1 : 0);
                    PlayerPrefs.Save();
                }
            }
            GUILayout.Label("  Client-side damage dealt. OFF by default; shotgun pellets are combined.", HintStyle());
            GUILayout.Space(14f);
            ...
            GUILayout.Space(4f);
            {
                bool newValue = SettingsUI.Toggle("Wide camera angle", _wideCam);
                if (newValue != _wideCam)
                {
                    _wideCam = newValue;
            ...
            {
                bool newValue = SettingsUI.Toggle(VIS_ITEMS[i].displayName, _visOn[i]);
                if (newValue != _visOn[i])
                {
                    _visOn[i] = newValue;
            ...
            {
                GUIStyle warnSt = new GUIStyle(HintStyle()); warnSt.normal.textColor = new Color(1f, 0.6f, 0.3f); warnSt.fontSize = 13;
            ...
            {
                string externalId = ActiveExternalTabId();
                if (!string.IsNullOrEmpty(externalId)) SettingsExternalTabs.Draw(externalId, pw);
            }
        ...
        private void ApplySwipeDelta(float dy)
        {
            // dy = currY - prevY in scaled IMGUI space (Y=0 is top of screen).
            // Swipe up (dy < 0) => scroll.y increases (see content below).
            // Swipe down (dy > 0) => scroll.y decreases (see content above).
            if      (_activeTab == 0) _scroll.y        = Mathf.Max(0f, _scroll.y        - dy);
            else if (_activeTab == 1) _scrollKbm.y     = Mathf.Max(0f, _scrollKbm.y     - dy);
            else if (_activeTab == 3) _scrollCtrl.y    = Mathf.Max(0f, _scrollCtrl.y    - dy);
            else if (_activeTab == 2) _scrollAccount.y = Mathf.Max(0f, _scrollAccount.y - dy);
            else
            {
                string id = ActiveExternalTabId();
                if (!string.IsNullOrEmpty(id))
                {
                    Vector2 scroll = Vector2.zero;
                    _externalTabScroll.TryGetValue(id, out scroll);
                    scroll.y = Mathf.Max(0f, scroll.y - dy);
                    _externalTabScroll[id] = scroll;
                }
            }
        }
        ...
        private void DrawControllersTabContent(float pw)
        {
            ...
            if (!anyConnected)
            {
                GUIStyle noCtrl = new GUIStyle(HintStyle());
            ...
            GUILayout.Space(4f);
            GUIStyle axStyle = new GUIStyle(HintStyle()); axStyle.fontSize = 11;
            ...
            GUILayout.Space(3f);
            GUIStyle axStyle2 = new GUIStyle(HintStyle()); axStyle2.fontSize = 10;
            ...
            GUILayout.Space(4f);
            {
                bool newValue = SettingsUI.Toggle("Gamepad controls enabled", _gamepadEnabled);
                if (newValue != _gamepadEnabled)
                {
                    _gamepadEnabled = newValue;
        ...
        private void DrawAccountTabContent(float pw)
        {
            ...
            string loginUser = EcoGetString("LoginUsername");
            if (!string.IsNullOrEmpty(loginUser))
                GUILayout.Label("Login:  " + loginUser, LabelStyle());
            else
                GUILayout.Label("Login:  Guest account", LabelStyle());
            GUILayout.Space(4f);
            ...
            GUIStyle stLbl = new GUIStyle(LabelStyle()); stLbl.normal.textColor = statusCol;
            ...
            SectionHeader("Create Login For This Guest");
            GUILayout.Space(4f);
            GUILayout.Label("This keeps your current guest progress and adds username/password login for other devices.", HintStyle());
            GUILayout.Space(4f);
            GUILayout.BeginHorizontal();
            GUILayout.Label("Username:", LabelStyle(), GUILayout.Width(92f));
            _createLoginUser = GUILayout.TextField(_createLoginUser, 24, GUI.skin.textField, GUILayout.Width(pw - 102f));
            GUILayout.EndHorizontal();
            GUILayout.Space(4f);
            GUILayout.BeginHorizontal();
            GUILayout.Label("Password:", LabelStyle(), GUILayout.Width(92f));
            _createLoginPassword = GUILayout.PasswordField(_createLoginPassword, '*', 32, GUI.skin.textField, GUILayout.Width(pw - 102f));
            GUILayout.EndHorizontal();
            GUILayout.Space(6f);
            if (GUILayout.Button("Create Login", BtnStyle(18, new Color(0.4f, 0.8f, 1f))))
            {
                if (!ValidLoginName(_createLoginUser))
                    _accountMsg = "Username must be 3-24 letters, numbers, or underscores.";
                else if (_createLoginPassword.Length < 6)
                    _accountMsg = "Password must be at least 6 characters.";
                else
                {
                    EcoSetString("AccountResultMsg", null);
                    EcoCallStatic("RequestCreateLogin", new object[]{ _createLoginUser, _createLoginPassword });
                    _accountMsg = "Creating login...";
                    _createLoginPassword = "";
                }
            }
            GUILayout.Space(10f);

            SectionHeader("Log In Existing Account");
            GUILayout.Space(4f);
            GUILayout.Label("Preview both accounts first. Nothing changes until you choose merge or use login only.", HintStyle());
            GUILayout.Space(4f);
            GUILayout.BeginHorizontal();
            GUILayout.Label("Username:", LabelStyle(), GUILayout.Width(92f));
            _loginUser = GUILayout.TextField(_loginUser, 24, GUI.skin.textField, GUILayout.Width(pw - 102f));
            GUILayout.EndHorizontal();
            GUILayout.Space(4f);
            GUILayout.BeginHorizontal();
            GUILayout.Label("Password:", LabelStyle(), GUILayout.Width(92f));
            _loginPassword = GUILayout.PasswordField(_loginPassword, '*', 32, GUI.skin.textField, GUILayout.Width(pw - 102f));
            GUILayout.EndHorizontal();
            GUILayout.Space(6f);
            if (GUILayout.Button("Preview Login", BtnStyle(18, new Color(1f, 0.7f, 0.3f))))
            {
                if (!ValidLoginName(_loginUser))
                    _accountMsg = "Username must be 3-24 letters, numbers, or underscores.";
                else if (_loginPassword.Length < 6)
                    _accountMsg = "Password must be at least 6 characters.";
                else
                {
                    _pendingLoginUser = _loginUser;
                    _pendingLoginPassword = _loginPassword;
                    EcoSetString("AccountResultMsg", null);
                    EcoCallStatic("RequestLoginStart", new object[]{ _pendingLoginUser, _pendingLoginPassword });
                    _accountMsg = "Checking login...";
                }
            }
            GUILayout.Space(10f);

            if (EcoGetBool("LoginPreviewReady"))
            {
                GUILayout.BeginHorizontal();
                DrawAccountStatsBox("Current Guest", EcoGetInt("LoginGuestCoins"), EcoGetInt("LoginGuestGems"),
                    EcoGetInt("LoginGuestOwnedItems"), EcoGetInt("LoginGuestLevel"), (pw - 12f) * 0.5f);
                GUILayout.Space(8f);
                DrawAccountStatsBox("Login Account", EcoGetInt("LoginAccountCoins"), EcoGetInt("LoginAccountGems"),
                    EcoGetInt("LoginAccountOwnedItems"), EcoGetInt("LoginAccountLevel"), (pw - 12f) * 0.5f);
                GUILayout.EndHorizontal();
                GUILayout.Space(8f);

                if (EcoGetBool("LoginSameAccount"))
                {
                    GUILayout.Label("This device is already linked to that login.", HintStyle());
                }
                else
                {
                    if (GUILayout.Button("Merge Guest Into Login", BtnStyle(18, new Color(0.35f, 1f, 0.45f))))
                    {
                        EcoSetString("AccountResultMsg", null);
                        EcoCallStatic("RequestLoginFinish", new object[]{ _pendingLoginUser, _pendingLoginPassword, "merge" });
                        _accountMsg = "Merging accounts...";
                    }
                    GUILayout.Space(4f);
                    if (GUILayout.Button("Use Login Only", BtnStyle(18, new Color(1f, 0.65f, 0.25f))))
                    {
                        EcoSetString("AccountResultMsg", null);
                        EcoCallStatic("RequestLoginFinish", new object[]{ _pendingLoginUser, _pendingLoginPassword, "switch" });
                        _accountMsg = "Switching accounts...";
                    }
                }
                GUILayout.Space(10f);
            }

            string accountResult = EcoGetString("AccountResultMsg");
            if (accountResult != null)
            {
                _accountMsg = accountResult;
                EcoSetString("AccountResultMsg", null);
            }
        ...
        private bool ValidLoginName(string s)
        {
        ...
        private void DrawAccountStatsBox(string title, int coins, int gems, int owned, int level, float width)
        {
            GUILayout.BeginVertical(GUI.skin.box, GUILayout.Width(width));
            GUILayout.Label(title, LabelStyle());
            GUILayout.Label("Coins: " + coins, HintStyle());
            GUILayout.Label("Gems: " + gems, HintStyle());
            GUILayout.Label("Owned: " + owned, HintStyle());
            GUILayout.Label("Level: " + level, HintStyle());
        ...
        private void DrawKbmTabContent(float pw)
        {
            ...
            GUILayout.Space(4f);
            {
                bool newValue = SettingsUI.Toggle("KBM controls enabled", _kbmEnabled);
                if (newValue != _kbmEnabled)
                {
                    _kbmEnabled = newValue;
        ...
        private static GUIStyle TabBtnStyle(bool active)
        {
            ...
            s.normal.textColor = active ? Color.white : new Color(0.72f, 0.72f, 0.72f);
            s.hover.textColor = Color.white;
            s.active.textColor = Color.white;
            Texture2D bg = active
                ? (_texTabActiveBg ?? (_texTabActiveBg = MakeTex(2, 2, new Color(0.28f, 0.28f, 0.28f, 1f))))
                : (_texGhostHover ?? (_texGhostHover = MakeTex(2, 2, new Color(0.20f, 0.20f, 0.20f, 1f))));
            s.normal.background = bg;
            s.hover.background = active ? bg : MakeTex(2, 2, new Color(0.24f, 0.24f, 0.24f, 1f));
            s.active.background = MakeTex(2, 2, new Color(0.30f, 0.30f, 0.30f, 1f));
        ...
        private void KbmSetCursorLocked(bool locked)
        {
            ...
            int rmbSerial = _capListener != null ? _capListener.rmbPressSerial : 0;
            _rmbAimPressSerialSeen = rmbSerial;
            _rmbEscapePressSerialSeen = rmbSerial;
            _escapeUnlockQueuedSerial = rmbSerial;
        }
            ...
            if (_gpRAxisX != null || _joyProxy != null)
            {
                ...
                if (_sliderotate != null)
                {
                    ...
                    if (rmag > _controllerDeadzone)
                    {
                        ...
                        float camSens = _modAimed ? (_controllerCamSens * _controllerAimMult) : _controllerCamSens;
        ...
        private void KbmInjectMouseLook(float mx, float my, float sensOverride = -1f)
        {
            ...
            float sens = sensOverride >= 0f ? sensOverride
                       : (_modAimed ? (_mouseSensNgl * _mouseAdsMult) : _mouseSensNgl);
        ...
        // Style helpers
        // =====================================================================
        // Public rendering surface used by SettingsUI. Native Settings content and
        // external tabs share these exact primitives so they cannot visually drift.
        public static void UISection(string title)
        {
            SectionHeader(title ?? "");
        }
        ...
        public static void UIHint(string text)
        {
            GUILayout.Label("  " + (text ?? ""), HintStyle());
        }

        public static bool UIToggle(string label, bool value)
        {
            GUILayout.Space(3f);
            Rect row = GUILayoutUtility.GetRect(10f, 46f, GUILayout.ExpandWidth(true));
            float boxSize = 34f;
            Rect box = new Rect(row.x + 5f, row.y + (row.height - boxSize) * 0.5f, boxSize, boxSize);

            if (Event.current.type == EventType.Repaint)
            {
                Texture2D chkTex = value
                    ? (_spSelectKuang ?? MakeTex(2, 2, Color.white))
                    : (_spPropKuang ?? MakeTex(2, 2, new Color(0.35f, 0.35f, 0.35f)));
                GUI.DrawTexture(box, chkTex, ScaleMode.ScaleToFit);
                GUI.Label(new Rect(box.xMax + 10f, row.y, Mathf.Max(0f, row.xMax - box.xMax - 12f), row.height), label ?? "", LabelStyle());
            }

            // Only the checkbox itself is interactive. Labels/descriptions are text,
            // matching conventional settings behavior and preventing accidental toggles.
            if (GUI.Button(box, GUIContent.none, GUIStyle.none)) value = !value;
            return value;
        }

        public static bool UIDisclosure(string label, bool expanded)
        {
            GUILayout.Space(4f);
            string arrow = expanded ? "v" : ">";
            if (GUILayout.Button(arrow + "  " + (label ?? ""), BtnStyle(17, Color.white), GUILayout.Height(40f)))
                expanded = !expanded;
            return expanded;
        }
        ...
        private static GUIStyle LabelStyle()
        {
            ...
            s.fontSize = 19; s.normal.textColor = Color.white;
            if (_gameFont != null) s.font = _gameFont;
            return _gsLabel = s;
        }
        ...
        private static GUIStyle GhostBtnStyle()
        {
            ...
            GUIStyle s = new GUIStyle(GUI.skin.button);
            s.normal.background = MakeTex(2, 2, new Color(0.19f, 0.19f, 0.19f, 1f));
            s.hover.background  = _texGhostHover  ?? (_texGhostHover  = MakeTex(2, 2, new Color(0.24f, 0.24f, 0.24f, 1f)));
            s.active.background = _texGhostActive ?? (_texGhostActive = MakeTex(2, 2, new Color(0.29f, 0.29f, 0.29f, 1f)));
        ...
        private static void SectionHeader(string title)
        {
            if (_gsSectionHdr == null)
            {
                ...
                _gsSectionHdr.fontStyle = FontStyle.Bold;
                _gsSectionHdr.fontSize = 18;
                _gsSectionHdr.normal.textColor = Color.white;
                _gsSectionHdr.alignment = TextAnchor.MiddleLeft;
                if (_gameFont != null) _gsSectionHdr.font = _gameFont;
            }
            GUILayout.Space(2f);
            GUILayout.Label(title.ToUpperInvariant(), _gsSectionHdr);
            Rect r = GUILayoutUtility.GetLastRect();
            if (Event.current.type == EventType.Repaint)
            {
        ...
        private void PatchFireButton()
        {
            if (_dragGOs[0] == null) return;
            GameObject go = _dragGOs[0];
            if (go == null) return;

            FireBtnClick proxy = go.GetComponent<FireBtnClick>() ?? go.AddComponent<FireBtnClick>();
            proxy.hook = this;
            ...
            for (int i = 0; i < comps.Length; i++)
            {
                ...
                if (tn == "UIButtonEventKit" || tn == "UIButton" || tn == "UIButtonMessage" || tn == "UIButtonKeys" || tn == "UIButtonScale" || tn.ToLower().Contains("fire") || tn.ToLower().Contains("press") || tn.ToLower().Contains("click"))
                    ((Behaviour)mb).enabled = false;
            }

            Behaviour[] beh = go.GetComponents<Behaviour>();
            for (int i = 0; i < beh.Length; i++)
            {
                Behaviour b = beh[i];
                if (b == null) continue;
                string tn = b.GetType().Name.ToLower();
                if (tn.Contains("button") || tn.Contains("press") || tn.Contains("click") || tn.Contains("fire"))
                    b.enabled = false;
            }
        }
        ...
        private class CapturedPointerListener : AndroidJavaProxy
        {
            ...
            public volatile int  rmbPressSerial;
            ...
            public void Reset() { _dx = 0f; _dy = 0f; lmbHeld = false; rmbHeld = false; rmbPressSerial = 0; _loggedFirst = false; FireCount = 0; }
            ...
            bool onCapturedPointer(AndroidJavaObject view, AndroidJavaObject e)
            {
                ...
                try
                {
                    ...
                    if (action == ACTION_MOVE || action == ACTION_HOVER_MOVE)
                    {
                        ...
                        bool secondaryHeld = (bs & BUTTON_SECONDARY) != 0;
                        if (secondaryHeld && !rmbHeld) rmbPressSerial++;
                        rmbHeld = secondaryHeld;
                    }
                    ...
                    else if (action == ACTION_BUTTON_PRESS)
                    {
                        ...
                        if (btn == BUTTON_SECONDARY)  { if (!rmbHeld) rmbPressSerial++; rmbHeld = true; }
                    }
        ...
        private void LoadPrefs()
        {
            ...
            _unscopeOnFire  = PlayerPrefs.GetInt("CNRMod_UnscopeOnFire", 0) == 1;
            _damageNumbers  = PlayerPrefs.GetInt("CNRMod_DamageNumbers", 0) == 1;
        ...
        private void ReloadExternalMods()
        {
            ...
            try
            {
                MethodInfo loadMods = typeof(MainMenuDirector).GetMethod("LoadMods", BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
                if (loadMods != null) loadMods.Invoke(null, null);
                else SettingsModEntry.Log("ReloadExternalMods: LoadMods not available in this game build");
            }
        ...
        private void CacheMenuSystemSprites()
        {
            ...
            _gsKeyLabelCtrl = _gsKeyLabelKbm = _gsMatchTabText = _gsMatchFooterText = _gsMatchTitle = null;
            _gsBtnCache.Clear();
    ...
    public class FireBtnClick : MonoBehaviour
    {
        public SettingsModHook hook;
        private void OnPress(bool pressed)
        {
            if (hook != null) hook.OnFireButtonProxy(pressed);
        }
    }

    public class AimBtnClick : MonoBehaviour
    {
        public SettingsModHook hook;
        private void OnPress(bool pressed)
        {
            // Intentionally ignored: aim should be tap-to-toggle, not hold-to-aim.
        }
