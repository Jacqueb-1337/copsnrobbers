...
namespace CNRSettingsMod
{
    ...
    public static class SettingsModEntry
    {
        ...
        public  const string Version = "3.1.106";
    ...
    public class SettingsModHook : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            ...
            {
                // (UICamera is disabled in OpenSettings(), so NGUI click-through is already blocked.
                // A fullscreen GUI.Button here would be rendered last at depth=-100 and would
                // consume ALL touch events before the settings window can process them.)
                // Heavy dim overlay
                GUI.color = new Color(0.16f, 0.16f, 0.16f, 0.98f);
                GUI.DrawTexture(new Rect(0, 0, vw, vh), Texture2D.whiteTexture);
                GUI.color = Color.white;

                float w = Mathf.Min(vw * 0.96f, 440f);
                float h = Mathf.Min(vh * 0.94f, 555f);
                _winRect = new Rect((vw - w) * 0.5f, (vh - h) * 0.5f, w, h);

                if (_gsWinBg == null)
                {
                    _gsWinBg = new GUIStyle(GUI.skin.window);
                    // Match Settings uses a flat charcoal panel rather than the old textured utility skin.
                    _gsWinBg.normal.background = MakeTex(2, 2, new Color(0.16f, 0.16f, 0.16f, 1f));
                ...
                {
                    // Don't clear _swipeDragging here — DrawSettingsWindow needs it to consume
                    // the MouseUp so no button fires at the end of a scroll gesture.
                    _swipeGuiFingerDown = false;
                    _swipeIsScrolling   = false;
                }
                // Draw the panel background then content as regular GUI calls (not GUI.Window).
                // GUI.Window uses a deferred rendering pass that renders behind the dim overlay
                // when GUIUtility.ScaleAroundPivot is active. GUI.Box + GUILayout.BeginArea
                // are regular GUI calls that always respect call order and GUI.matrix.
                GUI.Box(_winRect, "CNR SETTINGS", _gsWinBg);
                GUILayout.BeginArea(new Rect(_winRect.x + 4f, _winRect.y + 22f, _winRect.width - 8f, _winRect.height - 26f));
                DrawSettingsWindow(0);
                GUILayout.EndArea();
                // Clear _swipeDragging NOW, after DrawSettingsWindow has consumed the MouseUp.
        ...
        private void DrawSettingsWindow(int id)
        {
            ...
            GUILayout.BeginHorizontal();
            GUILayout.Space(2f);
            if (GUILayout.Button("GENERAL", TabBtnStyle(_activeTab == 0), GUILayout.Width(tbW), GUILayout.Height(34f)))
            ...
            if (GUILayout.Button("SAVE & CLOSE", BtnStyle(22, Color.white), GUILayout.Height(38f)))
            {
                _showSettings = false;
                if (_nguiUICam != null) _nguiUICam.enabled = true;
                try { Type t = EcoType(); if (t != null) { var fi = t.GetField("ModManagerOpen", BindingFlags.Public | BindingFlags.Static); if (fi != null) fi.SetValue(null, false); var fi2 = t.GetField("SettingsModOpen", BindingFlags.Public | BindingFlags.Static); if (fi2 != null) fi2.SetValue(null, false); } } catch { }
