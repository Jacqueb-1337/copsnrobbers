...
namespace CNRSettingsMod
{
    ...
    public class SettingsModHook : MonoBehaviour
    {
        ...
        private float _keepScopeTimer = -1f;  // legacy fallback; kept disabled so fire never re-presses aim
        ...
        private void ApplySensitivity()
        {
            ...
            UpdateScopeWidget(_isAiming || _keepScopeTimer > 0f);

            // If vanilla bowReload() forces an un-scope after a shot, request aim again.
            // This is intentionally a fallback; true no-un-scope requires patching bowReload().
            if (wasAiming && !_isAiming && !_unscopeOnFire)
            {
                float timeSinceFire = Time.time - _lastFireTime;
                if (timeSinceFire < 0.35f)
                {
                    _keepScopeTimer = 0.35f;
                    SettingsModEntry.Log("KeepScope: vanilla fire unscoped (dt=" + timeSinceFire.ToString("F3") + "s), requesting aim again");
                    DumpScopeCandidates();
                }
                else
                {
