...
namespace CNRSettingsMod
{
    ...
    public class SettingsModHook : MonoBehaviour
    {
        ...
        private bool  _modAimHeld    = false;
        ...
        private void ApplySensitivity()
        {
            ...
            bool scopedOrLocked = isNowScoped || (_scopeLockUntil > Time.time);
            bool wasAiming = _modAimed;
            _modAimed = scopedOrLocked;
            UpdateScopeWidget(_modAimed);
            if (_modAimed && !_unscopeOnFire)
            {
            ...
            if (wasAiming && !_modAimed && !_unscopeOnFire)
            {
            ...
            if (_modAimed) _scopeLockWeapon = GetSelectedWeapon();
        ...
        private void PreventFireUnscope()
        {
            ...
            try
            {
                ...
                CRWeaponScript weapon = GetSelectedWeapon();
                if (weapon == null) weapon = GetActiveAimWeapon();
                if (weapon == null) return;
                ...
                {
                    _scopePreventLogBudget--;
                    SettingsModEntry.Log("PreventFireUnscope: held " + weapon.weaponName
        ...
        private void SyncWeaponAimState()
        {
            try
            {
                ...
                if (weapon == null) weapon = GetActiveAimWeapon();
                if (weapon == null) return;
                if (weapon.aimed != _modAimed)
                    weapon.aimed = _modAimed;
            }
            catch (Exception ex)
            {
        ...
        private void CacheAimBtn()
        {
            ...
            if (_dragGOs[4] != null) { _aimBtn = _dragGOs[4]; PatchAimButton(); return; }
            UnityEngine.Object[] all = Resources.FindObjectsOfTypeAll(typeof(GameObject));
            foreach (UnityEngine.Object obj in all)
            {
                GameObject go = obj as GameObject;
                if (go != null && go.name == "Image Button(Aim)")
                { _aimBtn = go; SettingsModEntry.Log("CacheAimBtn found: " + go.name); PatchAimButton(); return; }
            }
        }
    ...
    public class AimBtnClick : MonoBehaviour
    {
        ...
        {
            _held = pressed;
            if (hook != null) hook.SetAimHeld(pressed);
        }
        private void OnClick()
        {
            if (hook == null) return;
            hook.SetAimHeld(true);
        }
        private void LateUpdate()
        {
            if (hook == null || !_held) return;
            hook.SetAimHeld(true);
        }
    }
