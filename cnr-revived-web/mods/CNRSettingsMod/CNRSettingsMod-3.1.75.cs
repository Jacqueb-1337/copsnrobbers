...
namespace CNRSettingsMod
{
    ...
    public class SettingsModHook : MonoBehaviour
    {
        ...
        // Scan for BulletEnemy (trigger-collider, no Rigidbody) instances every frame.
        // Problem: BulletEnemy.OnTriggerEnter fires against the local player's CharacterController
        // when a bullet spawns INSIDE the CC capsule (happens when firing steeply downward --
        // muzzle is inside the capsule, bullet immediately overlaps it).
        // Physics.IgnoreCollision has no effect on CC trigger events in old Unity.
        //
        // Key insight for distinguishing self-bullets from remote bullets:
        //   Self-hitting bullets spawn INSIDE the local CC capsule (muzzle is inside when
        //   looking steeply down). Remote player bullets spawn at the REMOTE player's muzzle,
        //   which is always outside our capsule (two CharacterControllers can't physically overlap).
        // Fix: for any NEW BulletEnemy whose spawn position is inside our CC capsule, disable
        // its trigger collider (bc.enabled=false). The bullet still moves and self-destructs
        // normally -- only the damage trigger is silenced. No timing tricks needed.
        private void ScanForLocalBullets()
        {
            if (_localCC == null || _localPlayerRoot == null) return;

            BulletEnemy[] bullets = (BulletEnemy[])UnityEngine.Object.FindObjectsOfType(typeof(BulletEnemy));
            if (bullets == null) return;

            // Build CC capsule in world space once per scan.
            Vector3 ccWorld = _localPlayerRoot.transform.TransformPoint(_localCC.center);
            float   r       = _localCC.radius + 0.15f; // 0.15 m tolerance: catches bullets spawned just at capsule surface
            float   halfH   = Mathf.Max(0f, _localCC.height * 0.5f - _localCC.radius);
            Vector3 top     = ccWorld + Vector3.up * halfH;
            Vector3 bottom  = ccWorld - Vector3.up * halfH;

            foreach (BulletEnemy be in bullets)
            {
                int id = ((UnityEngine.Object)(object)be).GetInstanceID();
                if (_knownBulletIds.Contains(id)) continue;
                _knownBulletIds.Add(id);

                Vector3 bPos = ((Component)(object)be).transform.position;
                if (!BulletInsideCapsule(bPos, top, bottom, r)) continue; // remote bullet -- leave it alone

                // Local self-hitting bullet: disable its trigger so OnTriggerEnter never fires.
                Collider bc = ((Component)(object)be).GetComponent<Collider>();
                if (bc != null) bc.enabled = false;
            }
        }

        // Returns true if 'point' is within 'radius' of the capsule axis defined by top/bottom.
        private static bool BulletInsideCapsule(Vector3 point, Vector3 top, Vector3 bottom, float radius)
        {
            Vector3 axis  = top - bottom;
            float   lenSq = Vector3.Dot(axis, axis);
            float   t     = (lenSq > 0f) ? Mathf.Clamp01(Vector3.Dot(point - bottom, axis) / lenSq) : 0f;
            return  (point - (bottom + t * axis)).magnitude <= radius;
        }
