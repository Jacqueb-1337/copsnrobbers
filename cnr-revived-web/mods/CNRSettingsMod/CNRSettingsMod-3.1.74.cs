...
namespace CNRSettingsMod
{
    ...
    public class SettingsModHook : MonoBehaviour
    {
        ...
        private HashSet<int>         _knownBulletIds        = new HashSet<int>();  // BulletEnemy instances seen; suppressed only during fire window
        ...
        private float                 _bulletSuppressTimer   = 0f;     // short window: newly-seen BulletEnemy = local bullet, suppress it
        private const float           BULLET_SUPPRESS_SECS   = 0.2f;  // 0.2 s covers local bullet spawn latency (~12 frames @ 60 fps)
        ...
        private void LateUpdate()
        {
            ...
            if (_bulletSuppressTimer > 0f)
                _bulletSuppressTimer -= Time.deltaTime;
            ...
            if (_inGameScene)
            {
                ...
                if (fireNow) { _projectileScanTimer = PROJECTILE_SCAN_SECS; _bulletSuppressTimer = BULLET_SUPPRESS_SECS; }
        ...
        // Scan for BulletEnemy (trigger-collider, no Rigidbody) instances.
        // BulletEnemy is used for BOTH locally-fired bullets AND bullets spawned by
        // machineGunShot / shotGunShot RPCs from remote players, so we must NOT
        // suppress all of them -- only ones that spawn during our own fire window.
        // Physics.IgnoreCollision does NOT work for CharacterController trigger events
        // in old Unity; instead we disable the trigger collider entirely for local bullets.
        private void ScanForLocalBullets()
        {
            if (_bulletSuppressTimer <= 0f) return; // no active fire window -- skip entirely
            ...
            foreach (BulletEnemy be in bullets)
            {
                ...
                // Disable the trigger collider so BulletEnemy.OnTriggerEnter can never
                // fire against our CharacterController. The bullet's Update() still runs
                // (it keeps moving and self-destructs normally) -- only the damage trigger
                // is silenced. Physics.IgnoreCollision is intentionally NOT used here
                // because it has no effect on CharacterController trigger events in PUN v1 Unity.
