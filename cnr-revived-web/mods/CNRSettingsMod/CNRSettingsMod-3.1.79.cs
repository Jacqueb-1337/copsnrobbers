...
namespace CNRSettingsMod
{
    ...
    public static class SettingsModEntry
    {
        ...
        public  const string Version = "3.1.79";
    ...
    public class SettingsModHook : MonoBehaviour
    {
        ...
        private void ApplySensitivity()
        {
            ...
            if (_prevFpsOnFire != -1 && curFpsOnFire != _prevFpsOnFire)
            {
                ...
                if (!_unscopeOnFire && _isAiming)
                    _scopeLockUntil = Time.time + 0.45f;
            }
            ...
            // ---- Keep-scope: if scope just ended due to firing, keep CRWeaponScript.aimed true ----
            ...
            if (wasAiming && !_isAiming && !_unscopeOnFire)
            {
                ...
                {
                    _scopeLockUntil = Time.time + 0.75f;
                    _keepScopeTimer = 1.0f;
                    SettingsModEntry.Log("KeepScope: fire-triggered unscope (dt=" + timeSinceFire.ToString("F3") + "s), preserving scope");
                }
                else
                {
            ...
            {
                SettingsModEntry.Log("KeepScope: fallback re-scope SUCCESS");
                _keepScopeTimer = -1f;
            }
            if (_unscopeOnFire)
            {
        ...
        private void PreventFireUnscope()
        {
            ...
            try
            {
                ...
                if (weapon == null)
                {
                    ...
                    {
                        if (weapons[i] != null && ((Component)weapons[i]).gameObject.activeInHierarchy)
                        {
                            weapon = weapons[i];
                            break;
                        }
                ...
                if (!weapon.canAim || weapon.WeaponType != CRWeaponScript.weaponType.BOW) return;
                ...
                if (_scopePreventLogBudget > 0)
                {
                    ...
                    SettingsModEntry.Log("PreventFireUnscope: preserved " + weapon.weaponName);
                }
            }
            catch (Exception ex)
            {
