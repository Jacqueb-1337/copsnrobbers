...
namespace CNRSettingsMod
{
    ...
    public class SettingsModHook : MonoBehaviour
    {
        ...
        private void LateUpdate()
        {
            ...
            ApplySensitivity();
            PreventFireUnscope();
            if (!_unscopeOnFire && Time.time <= _scopeLockUntil && !_scopeLockCoroutineActive)
        ...
        private void ApplySensitivity()
        {
            ...
            if (!_unscopeOnFire && Time.time <= _scopeLockUntil)
            {
                PreventFireUnscope();
                isNowScoped = true;
        ...
        {
            if (_isAiming || Time.time <= _scopeLockUntil) return true;
            if (_mainCam == null) CacheMainCam();
            if (_mainCam == null) return false;
            return _mainCam.fieldOfView < _defaultFov - 3f;
        }

        private void BeginScopeLock(float seconds)
        {
            if (_mainCam == null) CacheMainCam();
            if (_mainCam != null)
            {
            ...
            _scopeLockUntil = Mathf.Max(_scopeLockUntil, Time.time + seconds);
            _keepScopeTimer = -1f;
            PreventFireUnscope();
        }

        private void CancelScopeLock()
        {
            _scopeLockUntil = -1f;
            _scopeHoldFov = -1f;
            _keepScopeTimer = -1f;
        ...
        private void PreventFireUnscope()
        {
            ...
            if (Time.time > _scopeLockUntil) return;
            ...
            try
            {
                ...
                {
                    Component[] tweens = _mainCam.GetComponents(typeof(TweenFOV));
                    for (int i = 0; i < tweens.Length; i++)
                    {
                        TweenFOV tf = tweens[i] as TweenFOV;
                        if (tf != null) ((Behaviour)tf).enabled = false;
                    }
                    _mainCam.fieldOfView = _scopeHoldFov;
                }

                CRWeaponScript weapon = GetActiveAimWeapon();
                if (weapon == null) return;

                weapon.aimed = true;
                if (weapon.Aim != null) weapon.Aim.playAnimation = false;
        ...
        private void DrawSettingsWindow(int id)
        {
            ...
            GUILayout.Space(4f);
            {
                ...
                if (clicked)
                {
                    ...
                    if (_unscopeOnFire) CancelScopeLock();
                }
            }
