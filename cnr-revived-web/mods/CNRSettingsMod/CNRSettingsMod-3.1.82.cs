...
namespace CNRSettingsMod
{
    ...
    public class SettingsModHook : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (_scopeLockWeapon != null && (!_scopeLockWeapon.gameObject.activeInHierarchy || !_scopeLockWeapon.canAim))
        ...
        private void LateUpdate()
        {
            ...
            KbmJumpDetect();
            OwnJumpPhysics();
            if (_scopeLockWeapon != null)
            {
                CRWeaponScript selected = GetSelectedWeapon();
                if (selected != null && selected != _scopeLockWeapon)
                    CancelScopeLock(true);
            }
            PreventFireUnscope();
            ApplySensitivity();
            PreventFireUnscope();
            if (!_unscopeOnFire && _scopeLockUntil > 0f && !_scopeLockCoroutineActive)
        ...
        private void ApplySensitivity()
        {
            ...
            if (_prevFpsOnFire != -1 && curFpsOnFire != _prevFpsOnFire)
            {
                ...
                if (!_unscopeOnFire && IsLikelyScopedNow())
                    BeginScopeLock();
            }
            ...
            if (!_unscopeOnFire && _scopeLockUntil > 0f)
            {
                PreventFireUnscope();
                isNowScoped = true;
                if (_mainCam != null) currentFov = _mainCam.fieldOfView;
            }
