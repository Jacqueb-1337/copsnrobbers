// VoiceChat.cs - v0.5.27 WebView diagnostic build
//
...
namespace CNRVoiceChat
{
    ...
    public class VoiceChatHook : MonoBehaviour
    {
        ...
        private const int WebPcmFramesPerPost = 5;
        private const bool EnablePhpDiagnostics = false;
        ...
        private readonly byte[] _webPcmBuffer = new byte[FrameSamples * WebPcmFramesPerPost];
        private int _webPcmOffset;
        private bool _webPcmPostInFlight;
        ...
        void Start()
        {
            ...
            _webRtcActive = false;
            _browserVisible = SceneName() == "MainMenu";
        ...
        void Update()
        {
            ...
            MaybeReportState(inGame);
            MaybeBroadcastVoiceRoom(inGame);
        ...
        void OnGUI()
        {
            ...
            {
                DrawInGameBrowserButton(pad, size);
            }
        ...
        private void DrawInGameBrowserButton(float pad, float size)
        {
            float btnW = 168f;
            float btnH = 50f;
            Rect btnRect = new Rect(Screen.width - btnW - pad, pad + 8f, btnW, btnH);
            ...
            if (GUI.Button(btnRect, _browserVisible ? "Hide Voice" : "Show Voice", style))
            {
                _browserVisible = !_browserVisible;

                if (_browserVisible && !_webrtcBridge.HasRequested())
                {
                    TryStartWebRtc();
                }

                _webrtcBridge.SetVisible(_browserVisible);
                VoiceChatEntry.Log("Browser visible=" + _browserVisible + " in-game");
            }
        }
        ...
        private void DrawMenuBrowserButton()
        {
            ...
            Rect btnRect = new Rect(pad, pad + 8f, btnW, btnH);
        ...
        private void CaptureAndSend()
        {
            ...
            RaiseVoiceEvent(ht);
            MaybePostWebPcm(_encoded);
        }

        private void MaybePostWebPcm(byte[] frame)
        {
            try
            {
                if (frame == null || frame.Length == 0) return;
                if (!IsInGameRoom()) return;
                if (_webrtcBridge == null || !_webrtcBridge.HasRequested()) return;
                if (_webPcmPostInFlight) return;

                int copy = Math.Min(frame.Length, _webPcmBuffer.Length - _webPcmOffset);
                Array.Copy(frame, 0, _webPcmBuffer, _webPcmOffset, copy);
                _webPcmOffset += copy;
                if (_webPcmOffset < _webPcmBuffer.Length) return;

                byte[] payload = new byte[_webPcmBuffer.Length];
                Array.Copy(_webPcmBuffer, payload, payload.Length);
                _webPcmOffset = 0;
                StartCoroutine(PostWebPcm(payload));
            }
            catch (Exception ex)
            {
                VoiceChatEntry.Log("MaybePostWebPcm error: " + ex.Message);
            }
        }

        private IEnumerator PostWebPcm(byte[] payload)
        {
            _webPcmPostInFlight = true;
            try
            {
                string room = _webrtcBridge != null ? _webrtcBridge.RoomName : null;
                if (string.IsNullOrEmpty(room)) room = GetVoiceRoomName();
                string peer = LocalPeerId();
                string pcm = Convert.ToBase64String(payload);
                string json = "{\"room\":\"" + EscapeJson(room) +
                    "\",\"peer\":\"" + EscapeJson(peer) +
                    "\",\"type\":\"pcm_native\",\"data\":{\"rate\":" + CaptureRate +
                    ",\"pcm\":\"" + pcm +
                    "\",\"seq\":" + _seq +
                ...
                using (WWW www = new WWW("https://play.jacqueb.me/voicechat/api.php?action=send", body, headers))
                {
                    yield return www;
                    if (!string.IsNullOrEmpty(www.error))
                        VoiceChatEntry.Log("PostWebPcm error: " + www.error);
                }
            }
            finally
            {
                _webPcmPostInFlight = false;
            }
        }
        ...
        private void MaybeBroadcastVoiceRoom(bool inGame)
        {
            try
            {
                if (!inGame) return;
                ...
                if (string.IsNullOrEmpty(room) || room == "offline") return;
                Hashtable ht = new Hashtable();
                ht["id"] = LocalPeerId();
                ht["kind"] = "voice_room";
                ht["room"] = room;
        ...
        {
            if (!EnablePhpDiagnostics) return;
            try
            {
        ...
        private void MaybeReportState(bool inGame)
        {
            try
            {
                if (!EnablePhpDiagnostics) return;
                if (!inGame && !_webRtcActive) return;
    ...
    public class WebRtcBridge
    {
        ...
        private MethodInfo _miIsVisible;
        private AndroidJavaObject _activity;
        private AndroidJavaObject _nativeWebView;
        private AndroidJavaObject _nativeWebContainer;
        private AndroidJavaObject _nativePopupWindow;
        private bool _nativeWebViewAdded;
        private string _nativeUrl;
        private bool _lastMicMuted;
        ...
        public bool ProbeSupport()
        {
            ...
            _probed = true;
            _supported = Application.platform == RuntimePlatform.Android;

            VoiceChatEntry.Log("WebView diagnostic support=" + _supported +
                " native=" + (UseNativeWebView() ? "1" : "0"));
        ...
        {
            if (micMuted == _lastMicMuted && speakerMuted == _lastSpeakerMuted) return;
            _lastMicMuted = micMuted;
            _lastSpeakerMuted = speakerMuted;
        }
        ...
        public void SetVisible(bool visible)
        {
            ...
            if (visible)
            {
                if (UseNativeWebView())
                {
                    ShowNativeWebView(BuildDebugPageUrl());
                }
            }
            else
            {
        ...
        public string GetStatusLine()
        {
            return "WebView supported=" + _supported +
                ...
                " visible=" + _visible +
                " native=" + (_nativeWebViewAdded ? "1" : "0") +
        ...
        private void ApplyMediatorMargins()
        {
            int left = 12;
            int top = 74;
            int right = 12;
            int bottom = 12;
            TryInvoke("SetMargin", _miSetMargin, left, top, right, bottom);
            VoiceChatEntry.Log("WebView diagnostic mediator margins l=" + left + " t=" + top + " r=" + right + " b=" + bottom);
        }

        private string MediatorVisible()
        {
            try
            {
                if (_miIsVisible == null) return "unknown";
                object value = _miIsVisible.Invoke(null, null);
        ...
        private void ShowNativeWebView(string url)
        {
            if (Application.platform != RuntimePlatform.Android) return;
            if (string.IsNullOrEmpty(url)) url = DebugPageUrl;
            _nativeUrl = url;
            ...
            try
            {
                ...
                if (activity == null)
                {
                    VoiceChatEntry.Log("Native WebView show skipped: activity null");
                    return;
                }
                ...
                activity.Call("runOnUiThread", new AndroidJavaRunnable(() =>
                {
                    try
                    {
                        AndroidShowNativeWebView(url);
                    }
                    catch (Exception ex)
                    {
                        VoiceChatEntry.Log("Native WebView UI show failed: " + FullException(ex));
                    }
                }));
            }
            catch (Exception ex)
            {
                VoiceChatEntry.Log("Native WebView show failed: " + FullException(ex));
            }
        }

        private void AndroidShowNativeWebView(string url)
        {
            AndroidJavaObject activity = CurrentActivity();
            ...
            {
                _nativeWebView = new AndroidJavaObject("android.webkit.WebView", activity);
                SafeCall(_nativeWebView, "setWebViewClient", new AndroidJavaObject("android.webkit.WebViewClient"));
                SafeCall(_nativeWebView, "setWebChromeClient", new AndroidJavaObject("android.webkit.WebChromeClient"));
                AndroidJavaObject settings = _nativeWebView.Call<AndroidJavaObject>("getSettings");
                if (settings != null)
                {
                    SafeCall(settings, "setJavaScriptEnabled", true);
                    SafeCall(settings, "setDomStorageEnabled", true);
                    SafeCall(settings, "setMediaPlaybackRequiresUserGesture", false);
                    SafeCall(settings, "setLoadWithOverviewMode", true);
                    SafeCall(settings, "setUseWideViewPort", true);
                    SafeCall(settings, "setMixedContentMode", 0);
                }
                SafeCall(_nativeWebView, "setFocusable", true);
                SafeCall(_nativeWebView, "setFocusableInTouchMode", true);
                SafeCall(_nativeWebView, "setClickable", true);
                SafeCall(_nativeWebView, "setLongClickable", true);
                SafeCall(_nativeWebView, "setVerticalScrollBarEnabled", true);
                SafeCall(_nativeWebView, "setHorizontalScrollBarEnabled", true);
                SafeCall(_nativeWebView, "setOverScrollMode", 2);
                SafeCall(_nativeWebView, "setWillNotDraw", false);
                SafeCall(_nativeWebView, "setBackgroundColor", unchecked((int)0xffffffff));
                SafeCall(_nativeWebView, "setVisibility", 0);
                VoiceChatEntry.Log("Native WebView created");
            }

            if (_nativeWebContainer == null)
            {
                _nativeWebContainer = new AndroidJavaObject("android.widget.FrameLayout", activity);
                SafeCall(_nativeWebContainer, "setClickable", true);
                SafeCall(_nativeWebContainer, "setFocusable", true);
                SafeCall(_nativeWebContainer, "setFocusableInTouchMode", true);
                SafeCall(_nativeWebContainer, "setDescendantFocusability", 262144);
                SafeCall(_nativeWebContainer, "setBackgroundColor", unchecked((int)0x00000000));
                SafeCall(_nativeWebContainer, "setPadding", 12, 0, 12, 12);
                VoiceChatEntry.Log("Native WebView container created");
            }

            if (_nativeWebContainer != null && _nativeWebView != null)
            {
                try
                {
                    AndroidJavaObject childLp = CreateMatchLayoutParams();
                    SafeCall(_nativeWebContainer, "removeAllViews");
                    SafeCall(_nativeWebContainer, "addView", _nativeWebView, childLp);
                }
                catch (Exception ex)
                {
                    VoiceChatEntry.Log("Native WebView container addView failed: " + FullException(ex));
                }
            }

            if (!_nativeWebViewAdded && _nativeWebContainer != null)
            {
                ShowPopupWebView(activity);
            }

            SafeCall(_nativeWebContainer, "bringToFront");
            SafeCall(_nativeWebView, "bringToFront");
            SafeCall(_nativeWebView, "requestFocusFromTouch");
            SafeCall(_nativeWebView, "requestFocus");
            SafeCall(_nativeWebContainer, "requestFocusFromTouch");
            SafeCall(_nativeWebContainer, "requestFocus");
            SafeCall(_nativeWebContainer, "setClickable", true);
            SafeCall(_nativeWebContainer, "setFocusable", true);
            SafeCall(_nativeWebContainer, "setFocusableInTouchMode", true);
            SafeCall(_nativeWebContainer, "setVisibility", 0);
            SafeCall(_nativeWebView, "setVisibility", 0);
            _nativeWebView.Call("loadUrl", url);
            VoiceChatEntry.Log("Native WebView loadUrl=" + url);
        }

        private void ShowPopupWebView(AndroidJavaObject activity)
        {
            try
            {
                int matchParent = -1;
                int topOffset = Mathf.CeilToInt(104f);
                int popupHeight = Mathf.Max(1, Screen.height - topOffset - 8);
                _nativePopupWindow = new AndroidJavaObject("android.widget.PopupWindow", _nativeWebContainer, matchParent, popupHeight, true);
                SafeCall(_nativePopupWindow, "setTouchable", true);
                SafeCall(_nativePopupWindow, "setFocusable", true);
                SafeCall(_nativePopupWindow, "setOutsideTouchable", false);
                SafeCall(_nativePopupWindow, "setClippingEnabled", false);
                SafeCall(_nativePopupWindow, "setBackgroundDrawable", new AndroidJavaObject("android.graphics.drawable.ColorDrawable", 0));

                AndroidJavaObject window = activity.Call<AndroidJavaObject>("getWindow");
                AndroidJavaObject decor = window != null ? window.Call<AndroidJavaObject>("getDecorView") : null;
                if (decor == null)
                {
                    VoiceChatEntry.Log("Native WebView popup skipped: decor null");
                    return;
                }

                _nativePopupWindow.Call("showAtLocation", decor, 51, 0, topOffset);
                _nativeWebViewAdded = true;
                VoiceChatEntry.Log("Native WebView popup shown top=" + topOffset + " height=" + popupHeight);
            }
            catch (Exception ex)
            {
                VoiceChatEntry.Log("Native WebView popup show failed: " + FullException(ex));
            }
        }

        private AndroidJavaObject CreateNativeLayoutParams()
        {
            int matchParent = -1;
            AndroidJavaObject lp = new AndroidJavaObject("android.widget.FrameLayout$LayoutParams", matchParent, matchParent);
            SafeCall(lp, "setMargins", 12, 74, 12, 12);
            return lp;
        }

        private AndroidJavaObject CreateMatchLayoutParams()
        {
            int matchParent = -1;
            return new AndroidJavaObject("android.widget.FrameLayout$LayoutParams", matchParent, matchParent);
        }

        private void HideNativeWebView()
        {
            if (Application.platform != RuntimePlatform.Android || _nativeWebView == null) return;
            try
            {
                AndroidJavaObject activity = CurrentActivity();
                if (activity == null) return;
                activity.Call("runOnUiThread", new AndroidJavaRunnable(() =>
                {
                    try
                    {
                        if (_nativePopupWindow != null)
                        {
                            SafeCall(_nativePopupWindow, "dismiss");
                            _nativePopupWindow.Dispose();
                            _nativePopupWindow = null;
                            _nativeWebViewAdded = false;
                        }
                        if (_nativeWebContainer != null)
                            SafeCall(_nativeWebContainer, "setVisibility", 8);
                        if (_nativeWebView != null)
                            SafeCall(_nativeWebView, "setVisibility", 8);
                        VoiceChatEntry.Log("Native WebView hidden");
                    }
                    catch (Exception ex)
                    {
                        VoiceChatEntry.Log("Native WebView UI hide failed: " + FullException(ex));
                    }
                }));
            }
            catch (Exception ex)
            {
                VoiceChatEntry.Log("Native WebView hide failed: " + FullException(ex));
            }
        }

        private void DestroyNativeWebView()
        {
            if (Application.platform != RuntimePlatform.Android || _nativeWebView == null) return;
            try
            {
                ...
                activity.Call("runOnUiThread", new AndroidJavaRunnable(() =>
                {
                    try
                    {
                        ...
                        if (_nativeWebContainer != null)
                        {
                            SafeCall(_nativeWebContainer, "removeAllViews");
                            SafeCall(_nativeWebContainer, "setVisibility", 8);
                            _nativeWebContainer.Dispose();
                            _nativeWebContainer = null;
                        }
                        if (_nativeWebView != null)
                        {
                            SafeCall(_nativeWebView, "loadUrl", "about:blank");
                            SafeCall(_nativeWebView, "destroy");
                            _nativeWebView.Dispose();
                            _nativeWebView = null;
                        }
                        _nativeWebViewAdded = false;
                        VoiceChatEntry.Log("Native WebView destroyed");
                    }
                    catch (Exception ex)
                    {
                        VoiceChatEntry.Log("Native WebView UI destroy failed: " + FullException(ex));
                    }
                }));
            }
            catch (Exception ex)
            {
                VoiceChatEntry.Log("Native WebView destroy failed: " + FullException(ex));
            }
        }

        private static void SafeCall(AndroidJavaObject target, string method, params object[] args)
        {
            try
            {
                if (target != null) target.Call(method, args);
            }
            catch (Exception ex)
            {
                VoiceChatEntry.Log("Android call failed " + method + ": " + ex.Message);
            }
        }
