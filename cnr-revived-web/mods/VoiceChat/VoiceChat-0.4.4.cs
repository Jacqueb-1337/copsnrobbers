...
namespace CNRVoiceChat
{
    public class VoiceChatEntry
    {
        ...
        private static void EnableWebViewDebugging()
        {
            ...
            {
                if (Application.platform != RuntimePlatform.Android) return;
                using (AndroidJavaClass webView = new AndroidJavaClass("android.webkit.WebView"))
                {
                    using (AndroidJavaClass unityPlayer = new AndroidJavaClass("com.unity3d.player.UnityPlayer"))
                    {
                        using (AndroidJavaObject activity = unityPlayer.GetStatic<AndroidJavaObject>("currentActivity"))
                        {
                            activity.Call("runOnUiThread", new AndroidJavaRunnable(() =>
                            {
                                try
                                {
                                    webView.CallStatic("setWebContentsDebuggingEnabled", true);
                                    Log("WebView debugging enabled.");
                                }
                                catch (Exception ex)
                                {
                                    Log("EnableWebViewDebugging UI thread failed: " + ex.Message);
                                }
                            }));
                        }
                    }
            ...
            {
                Log("EnableWebViewDebugging failed: " + ex.Message);
            }
        }
    ...
    public class WebRtcBridge
    {
        ...
        public bool TryConnect(string localPeerId, bool inGame, string roomName, string displayName)
        {
            ...
            TryInvoke(_miShow);
            VoiceChatEntry.Log("WebView voice bridge connected peer=" + localPeerId + " room=" + _roomName);
            return true;
        }
