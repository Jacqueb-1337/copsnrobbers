...
namespace CNRVoiceChat
{
    ...
    public class VoicePlaybackFilter : MonoBehaviour
    {
        ...
        private float _playbackRateFactor = 0.22f;
        private bool _hasCurrent;
        private bool _hasNext;
        private float _current;
        private float _next;
        private const int MaxQueuedSamples = 44100 * 6;
        ...
        public void Enqueue(float[] samples)
        {
            ...
            lock (_lock)
            {
                ...
                while (_queue.Count > MaxQueuedSamples) _queue.Dequeue();
            }
        }
        ...
        private void DropExcessBacklog()
        {
            int target = _dstRate * 2;
            while (_queue.Count > target)
            {
                _queue.Dequeue();
            }
            if (_queue.Count == 0)
