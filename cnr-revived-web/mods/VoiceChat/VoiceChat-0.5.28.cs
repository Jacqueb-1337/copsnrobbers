...
namespace CNRVoiceChat
{
    ...
    public class VoiceChatHook : MonoBehaviour
    {
        ...
        private void DrawInGameBrowserButton(float pad, float size)
        {
            float btnW = 108f;
            float btnH = 34f;
            Rect btnRect = new Rect(Screen.width - btnW - pad, pad, btnW, btnH);
    ...
    public class WebRtcBridge
    {
        ...
        public bool ProbeSupport()
        {
            ...
            if (_webMediatorType != null)
            {
                ...
                _miIsVisible = GetStaticMethod("IsVisible");
        ...
        public bool TryConnect(string localPeerId, bool inGame, string roomName, string displayName)
        {
            ...
            if (UseNativeWebView())
            {
                TryInvoke("HideNativeFallbackMediator", _miHide);
                ShowNativeWebView(url);
            }
            else
            {
                ApplyMediatorMargins();
                TryInvoke("LoadUrl", _miLoadUrl, url);
                TryInvoke("Show", _miShow);
                VoiceChatEntry.Log("WebMediator IsVisible after Show=" + MediatorVisible());
            }
        ...
        public void SetVisible(bool visible)
        {
            ...
            VoiceChatEntry.Log("WebView diagnostic visible=" + visible + " mediatorVisible=" + MediatorVisible());
        }
        ...
        private void AndroidShowNativeWebView(string url)
        {
            ...
            if (!_nativeWebViewAdded)
            {
                AndroidJavaObject lp = CreateNativeLayoutParams();
                activity.Call("addContentView", _nativeWebView, lp);
