...
namespace CNRVoiceChat
{
    ...
    public class VoiceChatHook : MonoBehaviour
    {
        ...
        private void TryStartWebRtc()
        {
            ...
            if (_webrtcBridge.TryConnect(LocalPeerId(), IsInGameRoom()))
            {
                _webRtcActive = true;
    ...
    public class WebRtcBridge
    {
        ...
        private readonly Queue<float[]> _localFrames = new Queue<float[]>();
        private bool _probed;
        ...
        public bool ProbeSupport()
        {
            ...
            _probed = true;
            _supported = FindType("Unity.WebRTC.RTCPeerConnection") != null
                && FindType("Unity.WebRTC.AudioStreamTrack") != null
                && FindType("Unity.WebRTC.RTCIceCandidate") != null;
            VoiceChatEntry.Log("WebRTC support probe: " + _supported);
            if (_supported)
            {
                VoiceChatEntry.Log("WebRTC bridge scaffolding enabled: signaling only for now; PCM fallback remains active.");
            }
            return _supported;
        }

        public bool TryConnect(string localPeerId, bool inGame)
        {
            if (!_supported || !inGame) return false;
            _connected = true;
            _localPeerId = localPeerId;
            VoiceChatEntry.Log("WebRTC bridge connected for peer=" + localPeerId);
            return true;
        }
        ...
        public void CaptureLocalAudioFrame()
        {
            ...
            _localFrames.Enqueue(new float[160]);
            while (_localFrames.Count > 12) _localFrames.Dequeue();
        }
        ...
        public void SendSignal(string kind, string sdp, string candidate, int? mline)
        {
            ...
            ht["kind"] = "webrtc";
            ht["sig"] = kind;
            if (!string.IsNullOrEmpty(sdp)) ht["sdp"] = sdp;
            if (!string.IsNullOrEmpty(candidate)) ht["candidate"] = candidate;
            if (mline.HasValue) ht["mline"] = mline.Value;
            VoiceChatEntry.Log("WebRTC signal queued kind=" + kind + " from=" + _localPeerId);
        }
