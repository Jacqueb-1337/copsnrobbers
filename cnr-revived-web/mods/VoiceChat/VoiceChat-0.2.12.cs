...
namespace CNRVoiceChat
{
    ...
    public class VoiceChatHook : MonoBehaviour
    {
        ...
        private const float MicGain = 2.0f;
        private const float RemoteGain = 1.8f;
    ...
    public class RemoteVoice
    {
        ...
        private const int ClipSamples = Rate * 5;
        ...
        public RemoteVoice(string sender)
        {
            ...
            _clip = AudioClip.Create("VoiceChat_" + sender, ClipSamples, 1, Rate, true, true,
                new AudioClip.PCMReaderCallback(OnReadSamples),
                new AudioClip.PCMSetPositionCallback(OnSetPosition));
            _source.clip = _clip;
        ...
        public void Push(byte[] data)
        {
            ...
            try
            {
                lock (_queueLock)
                {
                    for (int i = 0; i < decoded.Length; i++) _queue.Enqueue(decoded[i]);
                }
        ...
        private void OnReadSamples(float[] data)
        {
            lock (_queueLock)
            {
                for (int i = 0; i < data.Length; i++)
                {
                    if (_queue.Count > 0) data[i] = _queue.Dequeue();
                    else data[i] = 0f;
                }
            }
        }

        private void OnSetPosition(int newPosition)
        {
            // Keep queued streaming data in order; no special seek handling needed.
        }
        ...
        public void StopPlayback()
        {
            try
            {
                ...
                if (_clip != null && _source != null) _source.clip = null;
