...
namespace CNRVoiceChat
{
    ...
    public class VoiceChatHook : MonoBehaviour
    {
        ...
        private string LocalPeerId()
        {
            try
            {
                if (PhotonNetwork.player != null)
                    return Convert.ToString(PhotonNetwork.player.ID);
            }
            catch { }
            return "local";
        }
        ...
        private Hashtable ExtractPayload(EventData ev)
        {
            try
            {
                ...
                if (ev.Parameters.ContainsKey((byte)245))
                {
                    ...
                    if (dict != null) return FlattenDict(dict);
                }
        ...
        private object GetEntryValue(object entry)
        {
            ...
            try
            {
                ...
                {
                    de = (DictionaryEntry)entry;
                    return de.Value;
                }
                PropertyInfo p = entry.GetType().GetProperty("Value");
        ...
        private Hashtable FlattenDict(IDictionary dict)
        {
            Hashtable flat = new Hashtable();
            foreach (DictionaryEntry de in dict)
                if (de.Key != null) flat[de.Key] = de.Value;
            return flat;
        }
