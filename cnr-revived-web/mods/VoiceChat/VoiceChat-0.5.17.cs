...
namespace CNRVoiceChat
{
    public class VoiceChatEntry
    {
        ...
        public const byte VoiceRoomEvent = 198;
    ...
    public class VoiceChatHook : MonoBehaviour
    {
        ...
        private void DrawMenuBrowserButton()
        {
            ...
            float size = 56f;
            float pad = 10f;
            Rect btnRect = new Rect(pad, pad, size * 2.2f, size);
            GUIStyle style = new GUIStyle(GUI.skin.button);
            style.fontSize = 18;
            style.fontStyle = FontStyle.Bold;
            style.normal.textColor = Color.white;
            style.alignment = TextAnchor.MiddleCenter;
            if (GUI.Button(btnRect, _browserVisible ? "Hide Voice" : "Voice Rooms", style))
            {
        ...
        private void TryStartWebRtc()
        {
            ...
            if (_webrtcBridge.TryConnect(LocalPeerId(), IsInGameRoom(), voiceRoom, LocalDisplayName()))
            {
                _webRtcActive = true;
    ...
    public class WebRtcBridge
    {
        ...
        public void OnSignal(string sender, Hashtable payload)
        {
            ...
            peer.State = kind;
            VoiceChatEntry.Log("WebRTC signal from " + sender + " kind=" + kind + " room=" + (_roomName ?? "null") + " local=" + (_localPeerId ?? "null") + " keys=" + payload.Keys.Count);
        }
        ...
        public void SendSignal(string kind, string sdp, string candidate, int? mline)
        {
            ...
            string js = "if(window.__cnrVoice&&window.__cnrVoice.onSignal){window.__cnrVoice.onSignal(" +
                        ...
                        ");}";
            ExecuteJs(js);
            VoiceChatEntry.Log("WebView signal queued kind=" + kind + " from=" + _localPeerId + " room=" + (_roomName ?? "null"));
        }
        ...
        {
            if (!_supported) return;
            if (visible) TryInvoke(_miShow);
            else TryInvoke(_miHide);
        }
