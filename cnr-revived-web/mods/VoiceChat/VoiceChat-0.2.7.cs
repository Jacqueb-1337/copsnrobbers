    ...
    public class RemoteVoice
    {
        ...
        public void Push(byte[] data)
        {
            ...
            try
            {
                ...
                _clip = AudioClip.Create("VoiceChatPkt_" + _sender, decoded.Length, 1, Rate, false, false);
                _clip.SetData(decoded, 0);
                _source.Stop();
                _source.clip = _clip;
                _source.Play();
                VoiceChatEntry.Log("Remote voice packet played sender=" + _sender + " samples=" + decoded.Length);
            }
            catch (Exception ex)
            {
