...
namespace CNRMods
{
    ...
    public class CNRIPRedirectMod : MonoBehaviour
    {
        ...
        private void ApplyPhotonOverride()
        {
            try
            {
                // Get PhotonNetwork.PhotonServerSettings via reflection
                Type photonNetType = GetPhotonNetworkType();
                if (photonNetType == null)
                {
                    Debug.LogError("[IPRedirect] PhotonNetwork type not found");
                    return;
                }

                // PhotonNetwork.PhotonServerSettings is a static property/field
                object settings = GetPhotonServerSettings(photonNetType);
                if (settings == null)
                {
                    Log("PhotonServerSettings is null — retrying next frame");
                    return;
                }

                Type settingsType = settings.GetType();

                // Override ServerAddress (without port — port is a separate field)
                ...
                // Also set HostType to SelfHosted (enum value 2) so SDK doesn't override
                SetField(settingsType, settings, "HostType", (int)2);

                _overrideApplied = true;
                Debug.Log($"[IPRedirect] Photon settings → {_serverIp}:{_serverPort}  AppID={_appId}");
                Log($"Override applied: {_serverIp}:{_serverPort}  AppID={_appId}");

                // If Photon is already connecting / connected, force-reconnect to our server
