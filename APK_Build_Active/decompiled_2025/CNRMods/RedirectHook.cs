...
public class RedirectHook : MonoBehaviour
{
	...
	public void SetRoomProp(Type pnt, string key, string value)
	{
		try
		{
			...
			PropertyInfo property2 = value2.GetType().GetProperty("customProperties", BindingFlags.Instance | BindingFlags.Public);
			if ((object)property2 == null)
			{
				ModEntry.Log("SetRoomProp: no customProperties");
				return;
			}
			object value3 = property2.GetValue(value2, null);
			if (value3 == null)
			{
				ModEntry.Log("SetRoomProp: customProperties is null");
				return;
			}
			if (!(value3 is IDictionary dictionary))
			{
				ModEntry.Log("SetRoomProp: customProperties not IDictionary, type=" + value3.GetType().FullName);
				return;
			}
			StringBuilder stringBuilder = new StringBuilder();
			foreach (object key2 in dictionary.Keys)
			{
				stringBuilder.Append(string.Concat(key2, "(", key2.GetType().Name, ") "));
			}
			ModEntry.Log(string.Concat("SetRoomProp: existing keys=[", stringBuilder, "]"));
			...
			{
				ModEntry.Log("SetRoomProp: no SetCustomProperties(1 param) on " + value2.GetType().Name);
				return;
			}
			methodInfo.Invoke(value2, new object[1] { value3 });
